...
// Κοινή συνάρτηση επαναφοράς που εξασφαλίζει ότι τα μενού επιστρέφουν ΜΟΝΟ αφού τελειώσει πλήρως η εκτύπωση
  const restoreMenusAfterPrint = () => {
    if (DOM.shareMenu) DOM.shareMenu.style.display = '';
    if (DOM.aMenu) DOM.aMenu.style.display = '';
    window.removeEventListener('afterprint', restoreMenusAfterPrint);
  };

  safeBind('btn-do-print', 'click', (e) => {
    e?.preventDefault(); 
    if (DOM.shareMenu) DOM.shareMenu.style.display = 'none'; 
    DOM.shareMenu?.classList.remove('show');
    
    window.addEventListener('afterprint', restoreMenusAfterPrint);
    window.print(); 
    // ΔΙΟΡΘΩΣΗ: Αφαιρέθηκε το αναξιόπιστο setTimeout των 1000ms
  });

safeBind(DOM.aPrint, 'click', (e) => {
    e?.preventDefault(); // Προστασία από ακούσιο refresh σελίδας
    if (DOM.aMenu) DOM.aMenu.style.display = 'none'; 
    DOM.aMenu?.classList.remove('is-open');
    
    // Ασφαλής επαναφορά μετά την εκτύπωση αντί για setTimeout
    window.addEventListener('afterprint', restoreMenusAfterPrint);
    window.print(); 
  });
...
const isTtsSupported = 'SpeechSynthesisUtterance' in window;
  ...
  if (isTtsSupported) {
    ...
    // ΔΙΟΡΘΩΣΗ: Ακύρωση της φωνής όταν ο browser ελαχιστοποιείται, η οθόνη κλειδώνει ή αλλάζει η καρτέλα
...
if (iframe.dataset.url !== url) {
    ...
    // ΔΙΟΡΘΩΣΗ: Αφαίρεση του allow-scripts για αποτροπή εκτέλεσης διπλών widget, διαφημίσεων και κρασαρίσματος
