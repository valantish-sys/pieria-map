  ...
  const LightboxManager = {
    ...
    init: () => {
     ...
     LM.el.addEventListener('touchend', (e) => {
        // --- ΝΕΑ ΠΡΟΣΘΗΚΗ: Αγνοεί το άφημα αν ήταν pinch-to-zoom (2+ δάχτυλα) ---
...
const { imgs, text: fullText, widgetHtml } = Utils.extractData(postContent);
   ...
   // --- ΕΔΩ ΤΟΠΟΘΕΤΕΙΣ ΤΟΝ ΑΚΡΟΑΤΗ ---
      window.addEventListener('reactionSync', (e) => {
          ...
          const postId = safePostId.replace(/_/g, '/').replace('html', '.html');
          document.querySelectorAll(`.floating-reactions[data-postid*="${postId}"]`).forEach(rDiv => {
              rDiv.classList.add('voted');
              const countSpan = rDiv.querySelector(`.count-${type}`);
              if (countSpan && countSpan.textContent !== "..") countSpan.textContent = newCount;
          });
      });
