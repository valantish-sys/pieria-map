...
// Φτιάχνουμε ένα Promise για να μπορεί ο κώδικας να "περιμένει" τη σύνδεση αν καθυστερήσει
let authPromise = new Promise((resolve, reject) => {
  // 1. Δοκιμάζουμε κανονική αποθήκευση (για να θυμάται ο browser το όριο 2 φωτογραφιών)
  authPub.setPersistence(firebase.auth.Auth.Persistence.LOCAL)
    .catch(() => {
      // 2. Αν ο Edge ή η Ανώνυμη Περιήγηση το μπλοκάρει, χρησιμοποιούμε τη μνήμη RAM (NONE)
      return authPub.setPersistence(firebase.auth.Auth.Persistence.NONE);
    })
    .then(() => {
      authPub.onAuthStateChanged(user => {
        if (user) {
          currentUserUid = user.uid; // Αποθηκεύουμε το κρυφό ID
          resolve(user.uid);
        } else {
...
window.canUserUpload = async function() {
  if (!currentUserUid) {
    // Δείχνουμε ένα loading popup και περιμένουμε αντί να ακυρώνουμε κατευθείαν το ανέβασμα
    Swal.fire({ 
      title: 'Ασφαλής σύνδεση...', 
      text: 'Γίνεται προετοιμασία, παρακαλώ περιμένετε...', 
    ...
    try {
      await authPromise; // Περιμένουμε να τελειώσει η αόρατη σύνδεση του Firebase
      Swal.close(); // Κλείνουμε το loading
    } catch (err) {
      Swal.close();
      Swal.fire({ 
        icon: 'error', 
        title: 'Αποτυχία Σύνδεσης', 
        text: 'Ο browser σας μπλοκάρει τη σύνδεση λόγω αυστηρών ρυθμίσεων απορρήτου. Δοκιμάστε από κανονικό παράθυρο.', 
  ...
  // Αν πάλι δεν υπάρχει UID (σπάνιο), σταματάμε
  ...
  if (snap.exists()) {
    snap.forEach(child => {
      let time = child.val();
      // Κρατάμε για έλεγχο ΜΟΝΟ όσα έγιναν το τελευταίο 24ωρο. Τα παλιά απλά τα αγνοούμε!
      if (now - time < ONE_DAY) {
      ...
      canvas.toBlob(blob => {
        ...
        .then(data => {
          if(data.secure_url) {
            ...
            // ΠΡΟΣΘΗΚΗ: Κλείνει το loading αν το Cloudinary γυρίσει error
            Swal.fire({ icon: 'error', title: 'Σφάλμα', text: 'Αποτυχία ανεβάσματος (Cloudinary). Δοκιμάστε ξανά.', confirmButtonColor: '#e74c3c' });
          }
        })
        .catch(err => {
