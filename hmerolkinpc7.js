...
// =========================================================
    // 🔥 ADVANCED ENGINE (Mobile Edition - Χωρίς Μαγνήτη)
    // =========================================================
    const AdvancedEngine = {
        ...
        playRoulette: () => {
            ...
            const spinInterval = setInterval(() => {
                ...
                if (navigator.vibrate) navigator.vibrate(5); // Δόνηση "τακ-τακ" όσο γυρνάει
                ...
                if (spins >= maxSpins) {
                    ...
                    if (navigator.vibrate) navigator.vibrate([20, 40, 20]); // Jackpot δόνηση νίκης
    ...
    const CalendarEngine = {
        ...
        init: () => {
            ...
            CalendarEngine.calendar = new window.FullCalendar.Calendar(DOM.calendarEl, {
                ...
                datesSet: (info) => {
                    ...
                    // Αποτροπή αλλαγής τίτλου αν είμαστε σε Zoom
       ...
       setupEvents: () => {
            ...
            const changeMonthAnimated = (direction) => {
                ...
                isAnimating = true;
                // ... (ο υπόλοιπος κώδικας από κάτω μένει άθικτος)
            ...
            // --- ΠΡΟΣΘΗΚΗ ΛΕΙΤΟΥΡΓΙΑΣ SWIPE (Για οθόνες αφής στο PC/Laptop) ---
            let touchstartX = 0;
            let touchendX = 0;
