...
// ==========================================
    ...
    class FeedbackEngine {
        static scan() {
            document.querySelectorAll(CONFIG.SELECTORS.ARTICLE_CONTAINER).forEach(container => {
                ...
                // Φόρτωση δεδομένων σε Πραγματικό Χρόνο
        ...
        static handleReaction(btn) {
            ...
            // Ορίζουμε τις δύο διαδρομές στο Firebase
            const reactionRef = ref(db, 'reactions/' + safePostId + '/' + type);
            const totalRef = ref(db, 'stats/total_reactions');
            
            // Τρέχουμε και τις 2 καταγραφές (Στο άρθρο ΚΑΙ στο γενικό σύνολο)
            Promise.all([
    ...
    } // <--- ΕΔΩ ΕΙΝΑΙ Η 3Η ΑΓΚΥΛΗ ΠΟΥ ΕΛΕΙΠΕ ΚΑΙ ΚΛΕΙΝΕΙ ΤΗΝ CLASS
// ==========================================
