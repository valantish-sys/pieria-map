...
(() => {
  ...
  const ReactionsManager = {
    observer: new IntersectionObserver((entries, observer) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // REAL-TIME LISTENER από Firebase
    ...
    handleVote: (btn) => {
      ...
      countSpan.textContent = ".."; // Οπτικό feedback αναμονής
      
      // ΑΣΦΑΛΗΣ ΚΑΤΑΓΡΑΦΗ ΜΕ TRANSACTION (Firebase)
      // ΑΣΦΑΛΗΣ ΚΑΤΑΓΡΑΦΗ ΜΕ TRANSACTION (Firebase)
      const reactionRef = ref(db, 'reactions/' + safePostId + '/' + type);
      const totalRef = ref(db, 'stats/total_reactions'); // ΝΕΟ: Γενικό σύνολο
      ...
      .then(() => {
        ...
        // Τα νούμερα ενημερώνονται αυτόματα από τον onValue observer!
      })
      .catch((error) => {
