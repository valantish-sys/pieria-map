   ...
   // --- 4. ΖΩΝΤΑΝΟΣ ΠΑΛΜΟΣ (ΑΛΗΘΙΝΟ FIREBASE PRESENCE) ---
    ...
    setupSwipeSupport: () => {
     ...
     const animateSwipeOut = (direction) => {
         isAnimatingSwipe = true; // Κλειδώνουμε το UI
         ...
         setTimeout(() => {
            ...
            setTimeout(() => {
               ...
               setTimeout(() => {
                  container.style.transition = '';
                  container.style.transform = '';
                  isAnimatingSwipe = false; // Ξεκλειδώνουμε το UI στο τέλος
               }, 500);
            }, 50);
         }, 300);
      };
       ...
       if (typeof JukeboxManager.setupMediaSession === 'function') JukeboxManager.setupMediaSession(name);
        ...
        els.player.play().then(() => {
          ...
          if (err.name === 'AbortError') {
              STATE.isFading = false; // <--- ΚΡΙΣΙΜΟ: Ξεκλειδώνουμε το State, αλλιώς "κολλάει" η ένταση!
              return;
          }
