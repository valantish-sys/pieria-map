...
// Εισαγωγή Emojis
window.insertTextAtCursor = (text) => {
    let t = document.getElementById('p-content');
    let start = t.selectionStart;
    let end = t.selectionEnd;
    t.innerHTML = t.innerHTML.substring(0, start) + text + t.innerHTML.substring(end);
...
window.saveDraft = () => {
    ...
    const draft = {
        author: document.getElementById('p-author').innerHTML,
        title: document.getElementById('p-title').innerHTML,
        content: document.getElementById('p-content').innerHTML,
        yt: document.getElementById('p-youtube').innerHTML
    };
...
const loadDraft = () => {
    try {
        ...
        if (draft) {
            if(draft.author) document.getElementById('p-author').innerHTML = draft.author;
            if(draft.title) document.getElementById('p-title').innerHTML = draft.title;
            if(draft.content) document.getElementById('p-content').innerHTML = draft.content;
            if(draft.yt) {
                document.getElementById('p-youtube').innerHTML = draft.yt;
...
window.handleYouTubeInput = () => {
    const url = document.getElementById('p-youtube').innerHTML;
...
window.removePreviewData = (event) => {
    ...
    document.getElementById('p-image').innerHTML = '';
    document.getElementById('p-youtube').innerHTML= ''; 
    selectedFile = null;
    activeYouTubeId = null;
    currentRotation = 0;
...
window.clearAllData = () => {
    if(confirm('Θέλετε να καθαρίσετε όλα τα πεδία και το προσχέδιο;')) {
        ...
        document.getElementById('p-author').innerHTML=''; 
        window.removePreviewData();
...
window.showLivePreview = () => {
    document.getElementById('modal-author').innerText = document.getElementById('p-author').innerHTML.trim() || 'Ανώνυμος';
    document.getElementById('modal-title').innerText = document.getElementById('p-title').innerHTML.trim() || '[Χωρίς Τίτλο]';
...
window.submitParentPost = async () => {
    ...
    const rawAuthor = document.getElementById('p-author').innerHTML.trim();
    const rawTitle = document.getElementById('p-title').innerHTML.trim();
...
const initPostDragAndDrop = () => {
    ...
    dropZone.addEventListener('drop', e => {
        if (isSubmitting || document.getElementById('p-youtube').innerHTML) return; 
...
const initCharCounters = () => {
    ...
    for (const [id, max] of Object.entries(limits)) {
        ...
        el.addEventListener('input', () => {
            const remain = max - el.innerText.length;
...
window.addEventListener('paste', e => {
    ...
    if (pastedText && (pastedText.includes('youtube.com') || pastedText.includes('youtu.be'))) {
        document.getElementById('p-youtube').innerHTML = pastedText;
        window.handleYouTubeInput();
        return; 
    }
    ...
    if (file && !document.getElementById('p-youtube').innerHTML) {
        const dataTransfer = new DataTransfer();
        dataTransfer.items.add(file);
        const fileInput = document.getElementById('p-image');
        fileInput.files = dataTransfer.files;
