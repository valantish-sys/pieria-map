...
window.saveEdit = (day, uid) => {
 ...
 // Κλείνουμε το πλαίσιο 
    document.getElementById(`edit-${day}-${uid}`).style.display = 'none';
    
    // Επαναφέρουμε χειροκίνητα το κείμενο και τα κουμπιά για να μην εξαφανιστεί η κάρτα,
    // σε περίπτωση που η renderAll() είναι μπλοκαρισμένη από άλλη ανοιχτή επεξεργασία.
    const tBox = document.getElementById(`txt-${day}-${uid}`);
    const aBox = document.getElementById(`act-${day}-${uid}`);
  ...
  update(ref(db, `answers/${day}/${uid}`), { answer: newTxt, status: 'approved' })
      .catch(err => alert("Αποτυχία: Το κείμενο απορρίφθηκε (π.χ. ξεπερνά τους 600 χαρακτήρες)."));
};
