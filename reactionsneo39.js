...
const mediaNodes = Array.from(wrapper.querySelectorAll('.post-body img, .post-body iframe, .scrollable-article-area img, .scrollable-article-area iframe'));
    ...
    class ArticleEngine {
        static scan() {
            document.querySelectorAll(CONFIG.SELECTORS.ARTICLE_CONTAINER).forEach(post => {
              ...
              // Δημιουργία Κουμπιού
                const btn = document.createElement('a'); // Αλλαγή σε Link για να δουλεύει η ροδέλα και το δεξί κλικ
                btn.href = linkElem.href;
      ...
      static close(post, btn, isPopState = false) {
            ...
            if (wrapper) {
              ...
              // Σταματάμε το βίντεο ΟΡΙΣΤΙΚΑ για να μην "καίει" δεδομένα 4G/5G στο παρασκήνιο
                wrapper.querySelectorAll('iframe').forEach(ifr => {
                    ...
                    entries.forEach(entry => {
                        if (entry.isIntersecting) {
                          ...
                          get(ref(db, 'reactions/' + safePostId)).then((snapshot) => {
                                // Αν ο χρήστης πρόλαβε και ψηφίσει όσο περιμέναμε, διακόπτουμε την αντικατάσταση
                                if (div.classList.contains('voted')) return;
