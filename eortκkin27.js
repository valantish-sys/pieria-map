                    ...
                    if (i === 0) {
                     ...
                     // --- ΝΕΟ: ΕΞΥΠΝΟΣ ΣΥΜΒΟΥΛΟΣ (Μπήκε ΜΕΣΑ στο if (i === 0) !) ---
                        ...
                        if (advisorBox) {
                            ...
                            // Κρύβεται ΜΟΝΟ αν είναι πριν τις 8 το πρωί Ή από τις 4 το απόγευμα (16:00) και μετά
                            if (hourNow < 8 || hourNow >= 16) {
                                advisorBox.style.display = 'none';
                            } else {
                                // Ελέγχουμε τον καιρό για όλες τις ώρες που είναι ενεργός ο σύμβουλος (8, 9... 15)
                                const activeHours = [8, 9, 10, 11, 12, 13, 14, 15];
                                ...
                                const maxDayTemp = Math.max(...activeHours.map(h => data.hourly.apparent_temperature[baseIndex + h]));

                                let msg = "", icon = "", borderColor = "", bgColor = "";
                                
                                if (hasSnow) {
                                    icon = "⛄"; msg = "Ίσως χιόνια! Ντύσου σαν κρεμμύδι 🧅!"; 
