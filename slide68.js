        ...
        if (!ticking) {
          window.requestAnimationFrame(() => {
           ...
           // Ενημέρωση θέσης (το <= 0 αποτρέπει λάθη με το bounce του Safari στα iPhone)
            ...
            window.scrollEndTimer = setTimeout(() => { isScrollingDown = false; }, 200);
          });
          ticking = true;
        }
      }, PASSIVE);
  ...
  if (LM.video) LM.video.removeAttribute('src'); 
    ...
    LM.lastFocus?.focus(); 
      if (!STATE.isPausedByTitle) {
          window.SliderManager?.resetProgress(true); // Επανεκκίνηση του animation
   ...
   if (ytMatch?.[1]) {
        ...
        LM.download.onclick = async (e) => {
          ...
          try {
            ...
            // Δίνουμε 1 δευτερόλεπτο περιθώριο στον browser να ξεκινήσει τη λήψη (ειδικά για Safari/iOS)
            setTimeout(() => {
...
const { imgs, text: fullText, widgetHtml } = Utils.extractData(postContent);
      ...
      slide.setAttribute("aria-hidden", "true"); 
        slide.setAttribute("tabindex", "-1"); // Όλα τα slides ξεκινούν κλειδωμένα
        ...
        if (widgetHtml) {
            ...
            if (idMatches) {
               ...
               uniqueIds.forEach(oldId => {
                    ...
                    // --- Η ΔΙΟΡΘΩΣΗ: Μπήκε το |$ στο τέλος της παρένθεσης ---
                 // Προσθήκη της τελείας (\.) στο lookahead για να εντοπίζει σωστά τα classes
const regexCss = new RegExp(`#${oldId}(?=[\\s\\:\\,\\[\\{>~\\.]|$)`, 'g');
     ...
     if (SM.slides.length > 0) {
        ...
        SM.slides[0].el.removeAttribute("tabindex"); // Ξεκλειδώνει μόνο το ενεργό
    ...
    document.addEventListener('keydown', e => {
     ...
     // Αρκεί το slider να είναι έστω και μερικώς ορατό στην οθόνη
