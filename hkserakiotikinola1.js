(() => {
 ...
 // ==========================================
  ...
  const CONFIG = Object.freeze({
    // Βάλε εδώ το JSON. Θα κατέβει ΜΟΝΟ μια φορά και θα τροφοδοτήσει PC και Κινητό!
  ...
  const STATE = {
    factsArray: [] // Εδώ αποθηκεύονται τα δεδομένα (1 φορά)
  };
  ...
  const Utils = {
    ...
    // Προσθήκη ασφαλούς αποκωδικοποίησης (από το PC)
    decodeHTML: (str) => {
 ...
 // ==========================================
  ...
  const FlipManager = {
    init: async () => {
      // --- Λήψη του JSON (Fetch API) ΜΙΑ ΦΟΡΑ ---
     // --- Λήψη του JSON (Fetch API) ΜΙΑ ΦΟΡΑ ---
      try {
        ...
        // 100% Ασφαλής ανάγνωση: Ψάχνει να βρει τον πίνακα (Array) όπου κι αν είναι κρυμμένος στο JSON!
        if (Array.isArray(data)) {
          STATE.factsArray = data;
        } else {
          // Αν είναι Object, βρίσκει την πρώτη λίστα που περιέχει
          STATE.factsArray = data.kidsFactsMob || data.kidsFactsDesk || Object.values(data).find(val => Array.isArray(val)) || [];
        }
      } catch (error) {
        console.error("Αποτυχία φόρτωσης δεδομένων:", error);
      }

      // --- Η ΕΞΥΠΝΗ ΛΟΥΠΑ (Ενεργοποιεί PC & Mobile) ---
    ...
    buildWidget: (suffix) => {
      const el = {
        ...
        // ΔΙΟΡΘΩΣΗ: Πιο σωστή στόχευση (να μην ψάχνει για "null")
        flipCard: suffix === "-desk" ? document.getElementById(`my-flip-card${suffix}`) : null
      };

      // Αν δεν βρει τη συγκεκριμένη κάρτα στη σελίδα, απλά το αγνοεί!
      ...
      // --- ΤΟΠΙΚΗ ΜΝΗΜΗ ΓΙΑ ΚΑΘΕ ΚΑΡΤΑ ΞΕΧΩΡΙΣΤΑ ---
      const localState = {
      ...
      const updateDOM = () => {
        ...
        if (nextFact) {
          // ΔΙΟΡΘΩΣΗ: Χρησιμοποιούμε innerHTML παντού για να μη σπάνε τα <br> 
          // και τα έντονα γράμματα στο PC.
      ...
      el.flipInner.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") {
          ...
          // Στο Desktop ανανεώνει το κείμενο καθώς κλείνει, για να είναι έτοιμο στο επόμενο hover
          if (suffix === "-desk") {
            setTimeout(updateDOM, CONFIG.flipMidpointMs);
          }
        }
      ...
      // --- HOVER (MouseLeave) ΜΟΝΟ ΓΙΑ ΤΟ DESKTOP ---
