...
if (DOM.shareMenu?.classList.contains('show') && !DOM.shareMenu.contains(t) && !t.closest('#share-master-fab, #share-master-fab-mob')) DOM.shareMenu.classList.remove('show');
  ...
  const initDraggableDial = () => {
    ...
    // 3. Όταν σηκώσει το δάχτυλο
    mainBtn.addEventListener('touchend', () => {
      // ΑΝ ΗΤΑΝ ΑΠΛΟ ΤΑΠ (δεν σύρθηκε ποτέ): Τερματίζουμε εδώ.
      // Το CSS παραμένει άθικτο και το κουμπί δεν χαλάει στο Scroll!
      if (!isDragging) return;

      // Αν είχε συρθεί, κάνουμε το μαγνητικό snap
     ...
     // Ελέγχουμε αν η οθόνη είναι έως 768px (κινητό) ή μεγαλύτερη (tablet)
const isMobile = window.innerWidth <= 768;

// Ορίζουμε τις αποστάσεις ανάλογα με τη συσκευή (για να ταιριάζουν με το CSS σου)
const paddingX = isMobile ? 9 : 15;      // 9px αριστερά στο κινητό, 15px στο tablet
const paddingBottom = isMobile ? 12 : 55; // 12px από κάτω στο κινητό, 55px στο tablet

// Υπολογισμός τελικής θέσης
const snapX = isRight ? screenW - rect.width - paddingX : paddingX;
const snapY = Math.max(80, Math.min(rect.top, screenH - rect.height - paddingBottom));
