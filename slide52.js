    ...
    init: () => {
      ...
      document.addEventListener('click', (e) => {
        ...
        if (!nav.contains(e.target)) {
          // 1. Αν έκανες κλικ εντελώς ΕΞΩ από το μενού (Τα κλείνει όλα όπως πριν)
          ...
          // 2. Αν έκανες κλικ ΜΕΣΑ στο μενού (Μετάβαση από το ένα παράθυρο στο άλλο)
          const clickedItem = e.target.closest('li'); // Βρίσκει την κατηγορία (tab) που μόλις πάτησες
          
          if (clickedItem) {
            allDropdowns.forEach(d => {
              // Αν το ανοιχτό παράθυρο ΔΕΝ ανήκει στην κατηγορία που πάτησες, το κλείνει ακαριαία!
              if (!clickedItem.contains(d)) {
                d.style.display = 'none';
                setTimeout(() => { d.style.display = ''; }, 100); // Reset στα 100ms για να λειτουργεί το hover μετά
              }
            });
          }
        }
  ...
  window.SliderManager = {
    ...
    buildDOM: (entries) => {
     ...
     entries.forEach((entry, idx) => {
        ...
        const { imgs, text: fullText, widgetHtml } = Utils.extractData(entry.content?.$t || "");
