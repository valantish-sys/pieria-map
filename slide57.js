...
(() => {
 ...
 const NavManager = {
    init: () => {
      ...
      // --- ΔΙΟΡΘΩΣΗ: GHOST FOCUS ΚΑΙ STICKY HOVER ---
      nav.addEventListener('mousemove', (e) => {
        if (document.activeElement && nav.contains(document.activeElement)) {
            ...
            // Ξεθολώνει (blur) τον προηγούμενο σύνδεσμο για να κλείσει το μενού
            if (activeLi && activeLi !== targetLi) {
      ...
      document.addEventListener('click', (e) => {
        ...
        if (!nav.contains(e.target)) {
          ...
          // Κλικ μέσα στο μενού (για κινητά)
          const clickedLi = e.target.closest('li');
          if (clickedLi) {
             allDropdowns.forEach(d => {
               if (!clickedLi.contains(d) && !d.contains(clickedLi)) {
                  d.style.display = 'none';
                  setTimeout(() => { d.style.display = ''; }, 100);
               }
             });
  ...
  const ReactionsManager = {
   ...
   handleVote: (btn) => {
      ...
      .then((results) => {
        ...
        // --- Η ΔΙΟΡΘΩΣΗ ΕΙΝΑΙ ΕΔΩ: Ενημερώνουμε την οθόνη ακαριαία! ---
        const txResult = results[0]; // Το αποτέλεσμα της πρώτης συναλλαγής (η ψήφος)
        if (txResult && txResult.committed) {
            // Παίρνουμε το νέο νούμερο κατευθείαν από τη βάση δεδομένων!
            countSpan.textContent = txResult.snapshot.val();
        } else {
            // Εφεδρικό (fallback) σε περίπτωση που κάτι κολλήσει
            countSpan.textContent = (parseInt(prevCount, 10) || 0) + 1;
        }
