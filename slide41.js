...
(() => {
  ...
  const Utils = {
   ...
   extractData: (htmlStr) => {
      ...
      // ΝΕΟ: Εντοπίζει αν η ανάρτηση είναι "έξυπνο" widget!
      const widgetNode = doc.querySelector('.smart-widget');
      let widgetHtml = null;
      if (widgetNode) {
          const styleNode = doc.querySelector('style');
          widgetHtml = (styleNode ? styleNode.outerHTML : '') + widgetNode.outerHTML;
          widgetNode.remove();
  ...
  window.SliderManager = {
    ...
    buildDOM: (entries) => {
      ...
      entries.forEach((entry, idx) => {
        ...
        // ΝΕΟ: Αν είναι widget, γίνεται <div> για να δουλεύουν τα Links χωρίς σφάλματα
        ...
        // ΝΕΟ: Η λογική εμφάνισης του Μενού
        if (widgetHtml) {
            const widgetOverlay = Object.assign(document.createElement("div"), { className: "widget-overlay" });
            widgetOverlay.innerHTML = widgetHtml;
            widgetOverlay.style.cssText = "position: absolute; inset: 0; background: rgba(255, 255, 255, 0.95); z-index: 10; overflow-y: auto; overflow-x: hidden; padding: 15px; pointer-events: auto; display: flex; flex-direction: column;";
            slide.appendChild(widgetOverlay);
        } else {
            // Ο παλιός σου κώδικας για κανονικές αναρτήσεις
            if (!isPinned && idx < 6) {
        ...
        if (postId?.length > 3) {
          ...
          if (widgetHtml) reactDiv.style.zIndex = "20"; // Για να φαίνεται πάνω από το μενού!
