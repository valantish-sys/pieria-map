...
const setVisibility = (show) => {
      ...
      if (DOM.speedDialMenu?.classList.contains('is-open') || 
          ...
          DOM.sPop?.classList.contains('is-open')) { // ΠΡΟΣΤΕΘΗΚΕ: Προστασία για την Αναζήτηση
          return; 
      }
...
if (iframe.dataset.url !== url) {
  ...
  // 2. Σκανάρουμε για Επικεφαλίδες (Μόνο H2 και H3)
    ...
    // Κατασκευή αόρατου ID αν δεν υπάρχει (για να ξέρει πού να πάει το link)
      if (!heading.id) {
        ...
        const safeName = heading.textContent.trim().toLowerCase().replace(/[\s\W]+/g, '-').replace(/^-|-$/g, '');
        heading.id = safeName ? 'toc-' + safeName : 'smart-toc-heading-' + index;
      }
 ...
 // ΝΕΟ: Αν το URL περιέχει #hash (π.χ. από κοινοποίηση συνδέσμου), κάνουμε ομαλό scroll 
    ...
    if (window.location.hash) {
      // Προσθήκη αποκωδικοποίησης (decodeURIComponent) για υποστήριξη Ελληνικών Anchor Links
...
window.triggerContactMenu = (e) => {
  e?.preventDefault();
  const isMobile = window.innerWidth <= 1368; 
