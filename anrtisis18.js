...
// Εφαρμογή μορφοποίησης στο επιλεγμένο κείμενο του textarea
window.applyTextFormat = (type) => {
  const t = document.getElementById('p-content');
  const start = t.selectionStart;
  const end = t.selectionEnd;
  const selected = t.value.substring(start, end);
  let replacement = "";
  let cursorOffset = 0;

  if (type === 'bold') {
    replacement = `**${selected || 'κείμενο'}**`;
  ...
  t.value = t.value.substring(0, start) + replacement + t.value.substring(end);
  t.selectionStart = t.selectionEnd = start + cursorOffset;
...
const loadDraft = () => {
    try {
        ...
        if (draft) {
            ...
            if(draft.content) document.getElementById('p-content').value = draft.content;
            if(draft.yt) {
...
window.showLivePreview = () => {
  ...
  const rawText = document.getElementById('p-content').value.trim();
