...
// --- ΝΕΟ: ΣΥΝΔΕΣΗ ΜΕ ΤΗ ΒΑΣΗ ΤΩΝ ΓΟΝΕΩΝ ---
const appParents = initializeApp({
  databaseURL: "https://anartiseis-7cad1-default-rtdb.europe-west1.firebasedatabase.app"
}, "ParentsAppSlider"); // Ονομάζουμε την εφαρμογή για να μην υπάρξει σύγκρουση
const dbParents = getDatabase(appParents); // Το 'dbParents' φέρνει τα άρθρα!
// ------------------------------------------
...
LM.lastFocus?.focus(); 
  ...
  window.SliderManager = {
   ...
   try {
        ...
        // --- ΝΕΟ: FETCH ΑΡΘΡΩΝ ΓΟΝΕΩΝ ΜΕ ΕΠΙΣΗΜΟ SDK (Bypass CORS) ---
        try {
            const fbSnap = await get(ref(dbParents, 'parent_posts'));
            if (fbSnap.exists()) {
                const fbPosts = [];
