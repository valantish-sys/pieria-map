      ...
      // ΔΙΟΡΘΩΣΗ BUG 2: Ασφαλής ανάγνωση του href ακόμα και αν το επιλεγμένο κουμπί είναι div
        ...
        try {
          ...
          const newDocContainer = doc.querySelector('.widget.Blog') || doc.querySelector('.blog-posts') || doc.querySelector('#main') || doc.querySelector('.main-inner');
           ...
           // ΚΡΙΣΙΜΟ BUG FIX: ΜΟΝΙΜΟ μπλοκάρισμα του document.write. 
             // Αν επαναφερθεί, τα ασύγχρονα scripts (πχ AdSense) που κατεβαίνουν με καθυστέρηση
             // θα καλέσουν το document.write και θα διαγράψουν (wipe out) ΟΛΗ τη σελίδα!
                ...
                // ΠΡΟΣΟΧΗ: Η "Επαναφορά αμέσως μετά την εισαγωγή" ΔΙΑΓΡΑΦΗΚΕ. Το document.write ΠΡΕΠΕΙ να μείνει νεκρό.
