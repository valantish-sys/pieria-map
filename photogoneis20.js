        ...
        .then(data => {
          if(data.secure_url) {
           ...
           // --- ΑΡΧΗ ΕΠΑΝΑΦΟΡΑΣ WEB3FORMS & ANTI-BOT ΠΡΟΣΤΑΣΙΑΣ ---
            ...
            let emailData = JSON.parse(localStorage.getItem(emailKey) || '{"date":"","count":0}');
            
            if (emailData.date !== today) {
                emailData = { date: today, count: 0 };
            }
            
            // Επιτρέπουμε το πολύ 3 emails τη μέρα από τον ίδιο browser
            if (emailData.count < 3) { 
                // 3. Τεχνητή καθυστέρηση 1 δευτερολέπτου που "σπάει" τα αυτοματοποιημένα loops
                setTimeout(() => { 
                    fetch('https://api.web3forms.com/submit', {
                        ...
                        body: JSON.stringify({
                            ...
                            botcheck: false // 4. Το κρυφό πεδίο που απαιτεί το Web3forms για να ξέρει ότι είσαι άνθρωπος
                        })
                    }).then(() => {
                        emailData.count++;
            ...
            // Καταγραφή ανεβάσματος στον Server (Firebase) για το όριο
            if (currentUserUid) {
