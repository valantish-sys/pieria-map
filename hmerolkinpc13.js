    ...
    const UIEngine = {
      ...
      fadeTimeout: null,
       ...
       showTooltip: (cellFrame, posts, isModal) => {
         ...
         clearTimeout(UIEngine.fadeTimeout); // <--- ΝΕΟ: Σκοτώνει τα παλιά κλεισίματα-"φαντάσματα"!
        ...
        closeTooltip: () => {
            ...
            // ΔΙΟΡΘΩΣΗ: Αποθήκευση στη μεταβλητή για να μπορούμε να το ακυρώσουμε
  ...
  // =========================================================
    ...
    const CalendarEngine = {
       ...
       setupEvents: () => {
           ...
           // --- ΠΡΟΣΘΗΚΗ ΛΕΙΤΟΥΡΓΙΑΣ "ΕΞΥΠΝΟΥ" SWIPE (Για οθόνες αφής στο PC) ---
            let isTouchMode = false; // ΝΕΟ: Διακόπτης αφής
            let touchstartX = 0;
            let startY = 0;

           DOM.container.addEventListener('touchstart', (e) => {
                isTouchMode = true; // ΝΕΟ: Μόλις νιώσει δάχτυλο το θυμάται!
            ...
            const handleCellInteraction = (frame, type) => {
              if (typeof AdvancedEngine !== 'undefined' && (AdvancedEngine.isSpinning || AdvancedEngine.isYearView)) return;
               ...
               if (type === 'click') {
                    // ΝΕΟ: Αν ΔΕΝ είναι οθόνη αφής (!isTouchMode) και υπάρχει μόνο 1 άρθρο, άνοιξέ το κατευθείαν (για PC).
                    // Αν είναι αφή (τάμπλετ/κινητό), άνοιξε ΠΑΝΤΑ το αναδυόμενο πλαίσιο!
                    if (posts && posts.length === 1 && !isTouchMode) {
           ...
           DOM.container.addEventListener('mouseover', (e) => {
                if (isTouchMode) return; // ΝΕΟ: Αν είναι αφή, σταματάει το Hover!
                const frame = e.target.closest('.fc-daygrid-day-frame');
                if (frame) {
                    clearTimeout(UIEngine.hideTimeout); 
           ...
           // Περιμένει το global script του FullCalendar να φορτώσει
            const waitForCalendar = setInterval(async () => {
                if (window.FullCalendar) {
                    ...
                    // ΝΕΟ: Τα κατεβάζει ΤΑΥΤΟΧΡΟΝΑ. Το Ημερολόγιο φορτώνει πολύ πιο γρήγορα!
                    await Promise.all([
