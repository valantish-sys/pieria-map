  ...
  window.SliderManager = {
   ...
   init: async () => {
      ...
      try {
        ...
        let entries = data.feed?.entry || []; // (ΠΡΟΣΟΧΗ: Το 'const' έγινε 'let' για να μπορέσουμε να προσθέσουμε δεδομένα)
        
        // --- ΝΕΑ ΠΡΟΣΘΗΚΗ: Φόρτωση Άρθρων Γονέων από Firebase (μέσω REST API) ---
        try {
            // Τραβάμε μόνο τα εγκεκριμένα (approved) άρθρα από την βάση anartiseis
            const fbRes = await fetch('https://anartiseis-7cad1-default-rtdb.europe-west1.firebasedatabase.app/parent_posts.json?orderBy="status"&equalTo="approved"');
            const fbData = await fbRes.json();
            
            if (fbData && typeof fbData === 'object' && !fbData.error) {
                const fbEntries = Object.keys(fbData).map(key => {
                    const post = fbData[key];
                    
                    // Καθαρισμός του περιεχομένου (αφαιρούμε το tag [READ_MORE] κλπ) όπως κάνει και ο 2ος κώδικας
                    let safeContent = post.content ? post.content.replace(/<o:p>.*?<\/o:p>/g, '') : "";
                    if (!/<[a-z][\s\S]*>/i.test(safeContent)) safeContent = safeContent.replace(/\n/g, '<br>');
                    safeContent = safeContent.replace(/\[READ_MORE\]/g, ' ');
                    
                    // Αν υπάρχει εικόνα, την βάζουμε μέσα στο HTML για να την εξάγει ο Slider αυτόματα
                    let contentHtml = safeContent;
                    if (post.imageUrl) {
                        contentHtml = `<img src="${post.imageUrl}" /> <br> ${contentHtml}`;
                    }
                    
                    return {
                        isParentPost: true, // Σημάδι ότι είναι άρθρο της Firebase
                        firebaseId: key,
                        title: { $t: `👨‍👩‍👧‍👦 ${post.title || 'Άρθρο Γονέα'}` },
                        content: { $t: contentHtml },
                        published: { $t: new Date(post.timestamp || 0).toISOString() },
                        // Link προς την αρχική σελίδα ώστε αν το πατήσει να κάνει scroll κατευθείαν στο άρθρο του γονέα!
                        link: [{ rel: "alternate", href: `/#fb-post-${key}` }]
                    };
                });
                
                // Ενώνουμε τα άρθρα του Blogger με αυτά των γονέων
                entries = [...entries, ...fbEntries];
                
                // Ταξινομούμε τη λίστα χρονολογικά (τα πιο πρόσφατα πρώτα)
                entries.sort((a, b) => new Date(b.published?.$t || 0) - new Date(a.published?.$t || 0));
            }
        } catch (e) {
            console.warn("Σφάλμα φόρτωσης Άρθρων Γονέων στο Slider:", e);
        }
        ...
        // --- ΝΕΑ ΛΟΓΙΚΗ: ΕΛΕΓΧΟΣ 15 ΗΜΕΡΩΝ (ΣΕΝΑΡΙΟ Β) ---
        const now = Date.now();
        let lastSeen = null;
        // Ασφαλής ανάγνωση από τη μνήμη του browser
        try { lastSeen = localStorage.getItem('welcome_post_last_seen'); } catch (e) {}
        
        const daysInMs = CONFIG.welcomeIntervalDays * 24 * 60 * 60 * 1000; // Μετατροπή ημερών σε milliseconds
        
        // Έλεγχος: Αν ΔΕΝ υπάρχει καθόλου στο ιστορικό Ή αν πέρασαν οι 15 μέρες
        const shouldShowWelcome = !lastSeen || (now - parseInt(lastSeen, 10) > daysInMs);

        if (shouldShowWelcome) {
            // 1. Ο χρήστης πρέπει να το δει (1η φορά ή πέρασε το 15ήμερο). Αποθηκεύουμε τη σημερινή στιγμή.
            try { localStorage.setItem('welcome_post_last_seen', now.toString()); } catch (e) {}
            
            // 2. Το υπάρχον σύστημα (Καρφίτσωμα στην αρχή Ή Λήψη από το δίκτυο)
            if (pinnedIndex > 0) {
...
const regexCss = new RegExp(`#${oldId}(?=[\\s\\:\\,\\[\\{>~\\.\\+\\)]|$)`, 'g');
      ...
      // --- ΝΕΟ: Η κόκκινη μπάρα τίτλου προστίθεται ΠΑΝΤΑ (και στα menus) ---
        ...
        // --- ΝΕΑ ΠΡΟΣΘΗΚΗ: Ορισμός ID για τον τέλειο συγχρονισμό των Emojis ---
        let postId = "";
        if (entry.isParentPost) {
            // Για τα άρθρα γονέων, δίνουμε ακριβώς το ίδιο ID με τον 2ο κώδικα (π.χ. parent_post_123)
            postId = 'parent_post_' + entry.firebaseId;
        } else {
            // Για τα άρθρα του Blogger, παίρνουμε το path
            postId = (href && href !== "#") ? new URL(href, window.location.origin).pathname : "";
        }
        // --- ΤΕΛΟΣ ΝΕΑΣ ΠΡΟΣΘΗΚΗΣ ---
...
const closeAllMenus = (exceptNode = null) => {
   ...
   menus.forEach(({ node, cls }) => {
      if (node && node !== exceptNode && node.classList.contains(cls)) {
        node.classList.remove(cls);
        // ΝΕΟ: Απεγκλωβισμός εικονικού πληκτρολογίου όταν κλείνει η αναζήτηση από εξωτερική εντολή
 ...
 // ΠΡΟΣΘΗΚΗ: Έξυπνο Auto-Focus στο πεδίο κειμένου
      ...
      if (isOpening) {
        if (inputField) {
            // ΔΙΟΡΘΩΣΗ: Ακύρωση Auto-Focus σε συσκευές αφής (Tablet/Κινητά) για αποφυγή βίαιου Zoom και ανοίγματος πληκτρολογίου
            if (!window.matchMedia('(pointer: coarse)').matches) {
                inputField.focus({ preventScroll: true }); 
                setTimeout(() => inputField.focus({ preventScroll: true }), 100); 
            }
        }
      } else if (inputField) {
        inputField.blur(); // ΔΙΟΡΘΩΣΗ: Απεγκλωβισμός πληκτρολογίου αν πατήσει το κουμπί για κλείσιμο
      }
    }
  };
...
// Share Event
  ...
  try {
      ...
      if (err.name !== 'AbortError') {
        ...
        // ΠΡΟΣΘΗΚΗ: Δίχτυ ασφαλείας. Αν το Web Share API καταρρεύσει στο Desktop, κάνουμε Fallback σε αντιγραφή!
        if (navigator.clipboard && window.isSecureContext) {
            try {
                await navigator.clipboard.writeText(shareData.url);
                alert('Ο σύνδεσμος αντιγράφηκε στο πρόχειρο (εναλλακτική μέθοδος)!');
            } catch (copyErr) { console.error('Clipboard Fallback Error:', copyErr); }
        }
      }
    }
  };
...
// Κοινή συνάρτηση επαναφοράς που εξασφαλίζει ότι τα μενού επιστρέφουν ΜΟΝΟ αφού τελειώσει πλήρως η εκτύπωση
// ΠΡΟΣΘΗΚΗ: Καθολική απόκρυψη του Widget στην εκτύπωση (καλύπτει εγγυημένα και το Ctrl+P)
  const printStyle = document.createElement('style');
  printStyle.textContent = '@media print { #a11y-menu, #search-pop, #share-options, #lib-options, #contact-options, #mobile-speed-dial, #fab-overlay, #smart-toc-wrapper, #smart-link-preview, #smart-glossary-tooltip { display: none !important; } }';
  document.head.appendChild(printStyle);

  safeBind('btn-do-print', 'click', (e) => {
    e?.preventDefault(); 
    DOM.shareMenu?.classList.remove('show'); // Απλό κλείσιμο του UI
    window.print(); 
  });

  safeBind(DOM.aPrint, 'click', (e) => {
    e?.preventDefault();
    DOM.aMenu?.classList.remove('is-open'); // Απλό κλείσιμο του UI
    window.print(); 
  });
...
DOM.aBtns?.forEach(btn => {
    ...
    // ΔΙΟΡΘΩΣΗ: Αποτροπή βίαιης ανανέωσης της σελίδας (refresh) αν το widget έχει μπει μέσα σε φόρμα
    safeBind(btn, 'click', (e) => {
        e?.preventDefault();
        applySetting(btn, cls, key, !DOM.html.classList.contains(cls));
    });
  });

safeBind(DOM.aReset, 'click', (e) => {
    e?.preventDefault();
 ...
 // Προστασία: Ακύρωση της φωνής αν ο χρήστης αλλάξει σελίδα, για αποφυγή ανάγνωσης στο παρασκήνιο
  if (isTtsSupported) {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
          window.speechSynthesis.cancel();
          // ΠΡΟΣΘΗΚΗ: Χειροκίνητη επαναφορά του εικονιδίου σε 🔊, καθώς το onend αγνοείται στο background
...
safeBind(DOM.aTts, 'click', (e) => {
    e?.preventDefault(); // Προστασία από ακούσιο refresh σελίδας
...
const activeLang = document.querySelector('select.goog-te-combo')?.value;
 ...
 // ΔΙΟΡΘΩΣΗ: Ιεραρχικός έλεγχος. Δίνουμε προτεραιότητα στο κυρίως κείμενο (.post-body) 
  // ώστε η ύπαρξη του tag <article> (ως wrapper ή σχόλια) να μην προκαλεί διπλή καταμέτρηση και false positive.
  const postBodies = document.querySelectorAll('.post-body, .entry-content');
  const fallbackArticles = document.querySelectorAll('article');
  
  const articles = postBodies.length > 0 ? postBodies : fallbackArticles;

  if (articles.length > 1) {
      alert('Για την ανάγνωση κειμένου, παρακαλώ ανοίξτε ένα συγκεκριμένο άρθρο (πατώντας στον τίτλο του).');
  ...
  const article = articles[0]; // Εφόσον είναι ένα, το επιλέγουμε με ασφάλεια
...
if (lang === 'el') {
    ...
    // ΠΡΟΣΘΗΚΗ: Καθαρισμός του cookie και από το Root Domain αν η διεύθυνση ξεκινά με "www."
    if (location.hostname.startsWith('www.')) {
        document.cookie = "googtrans=; expires=Thu, 01 Jan 1970 00:00:00 UTC; domain=" + location.hostname.substring(3) + "; path=/;";
    }
...
const cleanUrl = new URL(window.location.href);
  ...
  if (DOM.sPop?.classList.contains('is-open') && !DOM.sPop.contains(t) && !t.closest('#search-fab, #search-fab-mob')) {
        DOM.sPop.classList.remove('is-open');
        // ΔΙΟΡΘΩΣΗ: Απεγκλωβισμός πληκτρολογίου όταν κάνει κλικ στο κενό (outside click)
        const inputField = DOM.sPop.querySelector('input');
        if (inputField) inputField.blur();
    }
    ...
    const handleActivity = (e) => {
      ...
      if (!ticking) {
       ...
       // Προστασία: Αν φτάσαμε τέρμα πάνω (στην αρχή της σελίδας), ακυρώνουμε τον αποκλεισμό
         ...
         // ΠΡΟΣΘΗΚΗ: Ξεκλείδωμα του μενού όταν ο χρήστης τερματίσει τη σελίδα και εγκλωβιστεί!
            ...
            // ΔΙΟΡΘΩΣΗ: Ένα άγγιγμα/κλικ στην οθόνη ΠΡΕΠΕΙ να "ξεκλειδώνει" το μενού 
            // ακυρώνοντας τον εγκλωβισμό του scroll (Tap-to-Wake).
            if (e && e.type === 'click') {
               isHiddenByScrollDown = false; 
            } else if (isHiddenByScrollDown) {
               ticking = false;
               return; 
            }

            // Επαναφέρει το κουμπί κανονικά.
            setVisibility(true);
            clearTimeout(hideTimeout);
            hideTimeout = setTimeout(() => setVisibility(false), CONFIG.FAB_HIDE_DELAY);
          }
...
mainBtn.addEventListener('touchstart', (e) => {
      ...
      // ΔΙΟΡΘΩΣΗ: Απενεργοποίηση της διάδρασης στα iframes κατά το σύρσιμο για να μην "κλέψουν" το event της αφής
...
const handleDragEnd = () => {
      ...
      // ΔΙΟΡΘΩΣΗ: Επαναφορά της διάδρασης στα iframes
   ...
   // Ενοποίηση των timers για να αποφευχθεί το race condition σε απανωτά αγγίγματα
      dragResetTimer = setTimeout(() => {
          ...
          if (!isDragging) { // Ο έλεγχος παραμένει για ασφάλεια
            ...
            // ΠΡΟΣΘΗΚΗ: Καθαρισμός του inline transition μετά το τέλος της κίνησης, 
            // ώστε να μην καταστρέφονται μόνιμα τα μελλοντικά CSS animations του μενού
            setTimeout(() => {
                if (!isDragging) dialWrapper.style.removeProperty('transition');
            }, 850);
...
let tickingRuler = false;
  const updateRuler = (e) => {
    if (DOM.html.classList.contains('a11y-ruler') && rulerMask) {
      if (!tickingRuler) {
        // ΔΙΟΡΘΩΣΗ: Χρήση requestAnimationFrame. Μηδενίζει το lag χωρίς να χαλάσει τις συντεταγμένες!
        window.requestAnimationFrame(() => {
...
// ==========================================
  ...
  // ==========================================
  let glossaryTooltip = document.getElementById('smart-glossary-tooltip');
  
  // ΠΡΟΣΘΗΚΗ: Δυναμική δημιουργία του δοχείου αν λείπει από το HTML, αποτρέποντας το "σκάσιμο" της Javascript
  if (!glossaryTooltip) {
      glossaryTooltip = document.createElement('div');
      glossaryTooltip.id = 'smart-glossary-tooltip';
  ...
  const forbiddenWords = [
   ...
   "πορνο", "πορνε", "πορνη",     // πιάνει: πορνό, πορνογραφία
 ...
 // ΝΕΟΣ ΜΗΧΑΝΙΣΜΟΣ: Ενημερώνει το κείμενο ΚΑΙ υπολογίζει τα όρια της οθόνης
  ...
  // Μετατροπή ειδικών συμβόλων (όπως <, >) ώστε να μην εκληφθούν ως HTML κώδικας
   ...
   if (searchData.query && searchData.query.search.length > 0) {
        ...
        // ΑΣΦΑΛΗΣ ΜΕΘΟΔΟΣ: Χρήση DOMParser για να μην εκτελεστεί τυχόν κακόβουλος κώδικας (XSS)
        const parser = new DOMParser();
        const doc = parser.parseFromString(bestMatch.snippet, 'text/html');
      ...
      // ΠΡΟΣΘΗΚΗ: Αν στο μεταξύ ο χρήστης ζήτησε άλλη λέξη, ακυρώνουμε το "ξεκάρφωτο" μήνυμα λάθους της προηγούμενης!
      if (currentWordRequested !== word) return;
...
const handleSelection = () => {
    ...
    // ΔΙΟΡΘΩΣΗ: Αποτροπή αναζήτησης αν ο χρήστης μαρκάρει κείμενο ΜΕΣΑ στα λειτουργικά μενού του widget!
    if (selection.rangeCount > 0 && selection.anchorNode) {
      const targetNode = selection.anchorNode.nodeType === 3 ? selection.anchorNode.parentNode : selection.anchorNode;
      if (targetNode.closest && targetNode.closest('#smart-toc-wrapper, #a11y-menu, #search-pop, #contact-options, #mobile-speed-dial, #share-options, #lib-options')) return;
    }
 ...
 let word = selection.toString().trim();
    // ΠΡΟΣΘΗΚΗ: Συμπερίληψη των τυπογραφικών εισαγωγικών (“ ” ‘ ’)
    word = word.replace(/^[.,:;"'«»()!\?“”‘’]+|[.,:;"'«»()!\?“”‘’]+$/g, '');
...
const applySliderPatch = (forceResume = false) => {
 ...
 const previewBox = document.createElement('div');
   ...
   <div class="preview-loader">⏳ Φόρτωση...</div>
      <!-- ΔΙΟΡΘΩΣΗ: Αφαίρεση του allow-scripts για αποτροπή αναδρομικής εκτέλεσης σεναρίων και κρασαρίσματος -->
      <iframe class="preview-iframe" sandbox="allow-same-origin" style="pointer-events: none;" src="about:blank" tabindex="-1"></iframe>
    `;
...
let isSamePageAnchor = false;
      try {
         ...
         // ΔΙΟΡΘΩΣΗ: Απαραίτητη η σύγκριση ΚΑΙ των παραμέτρων (.search), αλλιώς μπλοκάρονται οι σελίδες αναζήτησης/σελιδοποίησης
          isSamePageAnchor = cUrl.pathname === lUrl.pathname && cUrl.search === lUrl.search && cUrl.origin === lUrl.origin;
      } catch (e) {
          // ΔΙΟΡΘΩΣΗ: Ασφαλέστερο Fallback που αγνοεί και τυχόν παραμέτρους (π.χ. ?m=1)
          isSamePageAnchor = url.split('#')[0].split('?')[0] === window.location.href.split('#')[0].split('?')[0]; 
      }

      const isOnlyImage = link.children.length > 0 && link.textContent.trim() === '';
 ...
 // ΝΕΟ: Άμεση απόκρυψη του παλιού συννεφακίου αν το ποντίκι πήγε σε ΔΙΑΦΟΡΕΤΙΚΟ link!
      if (currentLink && currentLink !== link && previewBox.classList.contains('is-visible')) {
          ...
          // Ολική αντικατάσταση στοιχείου για να μην "μολυνθεί" το Ιστορικό του Browser (Back button fix)
            const cleanIframe = document.createElement('iframe'); // (Σημείωση: Ή const newIframe...)
            cleanIframe.className = 'preview-iframe';
            cleanIframe.sandbox = 'allow-same-origin';
            cleanIframe.style.pointerEvents = 'none';
            cleanIframe.tabIndex = -1;
            cleanIframe.dataset.url = '';
...
if (iframe.dataset.url !== url) {
    ...
    // Η ανάθεση του src ΠΡΙΝ την εισαγωγή στο DOM αποτρέπει πλήρως τη δημιουργία ανεπιθύμητου ιστορικού!
    newIframe.src = url;
...
let hidePreviewTimer; // ΠΡΟΣΘΗΚΗ: Χρονόμετρο απόκρυψης
   ...
   // ΠΡΟΣΘΗΚΗ: Ακύρωση της προεπισκόπησης ακαριαία, αν ο χρήστης κάνει ενεργό κλικ στον σύνδεσμο!
   document.addEventListener('click', (e) => {
       const link = e.target.closest('.post-body a, .entry-content a');
       if (link) {
           clearTimeout(hoverTimer);
...
const initAutoToC = () => {
  ...
  const rawHeadings = Array.from(article.querySelectorAll('h2, h3'));
    ...
    // Αφαιρέθηκε ο περιορισμός "offsetParent !== null". Επιτρέπει στο widget να εντοπίζει τίτλους 
    // ακόμα κι αν το Blogger template καθυστερεί να δομήσει το layout ή αν υπάρχουν "Διαβάστε Περισσότερα".
    const headings = rawHeadings.filter(h => h.textContent.trim().length > 1);
    ...
    // Ομαλό Σύρσιμο (Smooth Scroll)
     ...
     // Ενημερώνουμε το URL hash αθόρυβα (ReplaceState), ώστε αν ο χρήστης κάνει 
        ...
        if (window.history && window.history.replaceState) {
          ...
          } catch (err) { /* Ασπίδα Προστασίας από Fatal Error σε cross-origin iframes (e-class) */ }
        }
...
const observerOptions = {
      // ΔΙΟΡΘΩΣΗ: Πολύ αυστηρότερο κάτω όριο (-70%). Η ενεργή ζώνη ανάγνωσης περιορίζεται 
      // αποκλειστικά στο πάνω 30% της οθόνης. Αποτρέπει τη λανθασμένη επισήμανση (κλοπή) αν 
      // υπάρχουν πολλά μικρά κεφάλαια/ασκήσεις ορατά ταυτόχρονα.
      rootMargin: '-79px 0px -70% 0px', 
      threshold: 0
    };
 ...
 let visibleHeadings = new Set();
   const headingObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        entry.isIntersecting ? visibleHeadings.add(entry.target.id) : visibleHeadings.delete(entry.target.id);
      });

      if (visibleHeadings.size > 0) {
        // ΔΙΟΡΘΩΣΗ: Επιλογή ΠΑΝΤΑ του πρώτου ορατού τίτλου (ορθή ιεραρχική σειρά ανάγνωσης από πάνω προς τα κάτω)
        const activeItem = tocLinks.find(item => visibleHeadings.has(item.heading.id));
        if (activeItem) {
...
document.getElementById('lib-options')?.classList.remove('show');
 ...
 const sPop = document.getElementById('search-pop');
  if (sPop && sPop.classList.contains('is-open')) {
      sPop.classList.remove('is-open');
      const inputField = sPop.querySelector('input');
      if (inputField) inputField.blur();
  }
...
window.handleSearchFromLib = (e) => {
 ...
 const sPop = document.getElementById('search-pop');
  if (sPop) {
   ...
   if (inputField) {
            // ΔΙΟΡΘΩΣΗ: Ακύρωση Auto-Focus στα κινητά (iOS) για αποφυγή βίαιου Zoom
            if (!window.matchMedia('(pointer: coarse)').matches) {
                inputField.focus({ preventScroll: true }); 
                setTimeout(() => inputField.focus({ preventScroll: true }), 100); 
            }
...
;(() => {
 ...
 // eortologio
    const CONFIG = Object.freeze({
        ...
        jsonUrl: 'https://cdn.jsdelivr.net/gh/valantish-sys/pieria-map@main/eortvasi5.json'
    });
 const DataEngine = {
...
if (dayOffset === 0 && currentHour >= 14) {
    ...
    if (isEven) {
        return `${wStart} 
                <span class="sch-time">🍝 Φαγητό</span> 
                <span class="sch-time">📚 Διάβασμα</span> 
                <span class="sch-time">⚽ Παιχνίδι</span> 
                <span class="sch-time" style="color:#a90e0e; margin-top:3px;">🎒 Τσάντα</span> 
                ${wEnd}`;
    } else {
        return `${wStart} 
                <span class="sch-time">🛋️ Χαλάρωση</span> 
                <span class="sch-time">📝 Φροντιστήριο</span> 
                <span class="sch-time">🚲 Βόλτα</span> 
                <span class="sch-time" style="color:#a90e0e; margin-top:3px;">🛌 Ύπνος</span> 
                ${wEnd}`;
    }
  ...
  // ==========================================
        ...
        getLandmarkIcon: (date) => {
          ...
          if (m === 6 && d === 30) events.push({ icon: '📜', name: 'Σύναξις Αγίων 12 Αποστόλων' });
           ...
           if (m === 7 && d === 26) events.push({ icon: '👁️', name: 'Αγίας Παρασκευής' });
           if (m === 7 && d === 28) events.push({ icon: '🍎', name: 'Αγίας Ειρήνης Χρυσοβαλάντου' });
            if (m === 8 && d === 6) events.push({ icon: '☀️', name: 'Μεταμόρφωση του Σωτήρος' });
...
if (daysDiff === 7)   events.push({ icon: '☝️', name: 'Κυριακή του Θωμά' });
...
;(() => {
 ...
 getPostPath: (container) => {
            try { 
                // [ΝΕΟ] Αν είναι άρθρο γονέων από τη Firebase, δίνουμε το δικό του ID για τα Emojis!
                if (container.dataset.firebaseId) {
                    return 'parent_post_' + container.dataset.firebaseId;
                }
...
static processFetchedContent(fetchedBody, summaryWrap) {
    ...
    static closeAllOpened(e) {
            ...
            document.querySelectorAll('.fetched-content-wrapper.is-open').forEach(wrapper => {
                ...
                if (isPopState && btn && btn.dataset.url && btn.dataset.url.split('#')[0] === window.location.href.split('#')[0]) return;
                
                // [ΝΕΟ] Αν είναι τοπικό άρθρο Firebase, έκλεισέ το αθόρυβα
                if (wrapper.classList.contains('local-content-wrapper')) {
...
mainContainer.dataset.justVoted = "true";
    ...
    // ==========================================
    // 6. PARENT POSTS ENGINE (Άρθρα Γονέων)
    // ==========================================
    class ParentPostsEngine {
        static cachedPosts = [];
       ...
       static escapeHTML(str) {
            if (!str) return "";
            // [ΝΕΟ] Αποκωδικοποίηση σε περίπτωση που η Φόρμα το έστειλε ήδη κωδικοποιημένο (αποφυγή διπλού escaping)
            let decoded = str.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&#39;/g, "'").replace(/&quot;/g, '"');
            // Ξανά κωδικοποίηση (Μέγιστη ασφάλεια αν κάποιος χάκερ παρακάμψει τη φόρμα)
            return decoded.replace(/[&<>'"]/g, tag => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[tag]));
        }

        static fetchAndInject() {
            if (this.isFetching) return;
            this.isFetching = true;
            
            const injApp = !firebase.apps.find(a => a.name === "AnartiseisApp") 
                ? firebase.initializeApp(this.appConfig, "AnartiseisApp") 
                : firebase.app("AnartiseisApp");

            injApp.database().ref('parent_posts').orderByChild('status').equalTo('approved').once('value').then(snap => {
                this.cachedPosts = [];
                if (snap.exists()) {
                    snap.forEach(child => { this.cachedPosts.push({ id: child.key, ...child.val() }); });
                    this.cachedPosts.sort((a, b) => b.timestamp - a.timestamp);
                }
                this.placePostsChronologically();
                this.isFetching = false;
            }).catch(e => {
                console.error("ParentPosts Fetch Error:", e);
                this.isFetching = false;
            });
        }

        static placePostsChronologically() {
            const blogContainer = document.querySelector('.widget.Blog') || document.querySelector('.blog-posts') || document.querySelector('#main');
            if (!blogContainer) return;

          const visibleBloggerPosts = Array.from(document.querySelectorAll('.date-outer:not(.fb-injected), .post-outer:not(.fb-injected), article.post:not(.fb-injected), .post:not(.fb-injected), .mobile-date-outer:not(.fb-injected), .mobile-post-outer:not(.fb-injected)'))
                .filter(post => !post.parentElement.closest('.date-outer, .post-outer, article.post, .post, .mobile-date-outer, .mobile-post-outer'));

            if (visibleBloggerPosts.length === 0) return;

            let oldestTime = 0, newestTime = Date.now();
            ...
            if (timestamps.length > 0) {
                oldestTime = Math.min(...timestamps);
                newestTime = Math.max(...timestamps);
            }

         const hasOlderLink = (document.querySelector('.blog-pager-older-link') && document.querySelector('.blog-pager-older-link').style.display !== 'none') || document.getElementById('infinite-scroll-sentinel') !== null;
            const isHomePage = window.location.pathname === '/' || window.location.pathname === '';
            let hasNewInsertions = false;

            this.cachedPosts.forEach(fbPost => {
                if (document.getElementById('fb-post-' + fbPost.id)) return;
                if (fbPost.timestamp < oldestTime && hasOlderLink) return;
                if (!isHomePage && fbPost.timestamp > newestTime) return;

              const dateStr = new Date(fbPost.timestamp).toLocaleDateString('el-GR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
                
              const imgHtml = fbPost.imageUrl ? `<a href="${fbPost.imageUrl}" class="parent-post-img"><img src="${fbPost.imageUrl}" data-src="${fbPost.imageUrl}" loading="lazy" style="max-width: 100%; border-radius: 8px; margin: 15px auto; display: block; box-shadow: 0 4px 10px rgba(0,0,0,0.1); cursor: zoom-in;"></a>` : '';

              // [ΝΕΟ] Έλεγχος και δημιουργία HTML για το βίντεο (εφόσον υπάρχει youtubeUrl)
              let ytHtml = '';
              if (fbPost.youtubeUrl) {
                  const ytMatch = fbPost.youtubeUrl.match(/(?:youtube(?:-nocookie)?\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?|shorts)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i);
                  if (ytMatch && ytMatch[1]) {
                      ytHtml = `<div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; margin: 15px auto; max-width: 100%; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
                ...
                let safeContent = fbPost.content ? fbPost.content.replace(/<o:p>.*?<\/o:p>/g, '') : "";
                
               // Προστασία: Αν το κείμενο γράφτηκε σε απλό κουτί και ΔΕΝ έχει HTML, μετατρέπουμε τα enter σε <br>
                if (!/<[a-z][\s\S]*>/i.test(safeContent)) {
                    safeContent = safeContent.replace(/\n/g, '<br>');
                }

                // [ΝΕΟ 1] Διόρθωση "ορφανών" λιστών: Η φόρμα στέλνει <li> αλλά ξεχνάει να τα βάλει μέσα σε <ul>
                if (safeContent.includes('<li>')) {
                    safeContent = safeContent.replace(/(<li>.*?<\/li>)/gi, '<ul style="margin: 8px 0 8px 25px; padding: 0;">$1</ul>');
                    // Ενώνει τα συνεχόμενα <ul> μεταξύ τους για να μην έχουν αντιαισθητικά κενά
                    safeContent = safeContent.replace(/<\/ul>\s*(<br>\s*)*<ul[^>]*>/gi, ''); 
                }

                // [ΝΕΟ 2] Αυτόματα Links: Εντοπίζει "νεκρά" URLs και τα κάνει πατητά, να ανοίγουν σε νέα καρτέλα
                safeContent = safeContent.replace(/(^|[^"'])(https?:\/\/[^\s<]+)/gi, '$1<a href="$2" target="_blank" rel="noopener noreferrer" style="color: #1e6cff; text-decoration: underline;">$2</a>');

                let firstPart = safeContent, secondPart = "", hasMore = false;
                
                if (safeContent.includes('[READ_MORE]')) {
                    hasMore = true;
                    let parts = safeContent.split('[READ_MORE]');
                ...
                // Δημιουργούμε το HTML ώστε να ταιριάζει απόλυτα και να νομίζει ο κώδικας ότι είναι άρθρο Blogger
                fbEl.innerHTML = `
                  <h2 class="date-header"><span>${dateStr}</span></h2>
                  <div class="date-posts">
                    <!-- Η κλάση 'article-processed' το προστατεύει από το ArticleEngine (αφού δημιουργούμε μόνοι μας το Read More) -->
                    <div class="post-outer parent-post article-processed" data-firebase-id="${fbPost.id}">
                      <div class="post hentry uncustomized-post-template" style="border-left: 5px solid #1e6cff !important; padding: 20px; background: rgba(30,108,255,0.02); border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); margin-bottom: 25px; animation: fadeUpModern 0.6s ease forwards;">
                        <h3 class="post-title entry-title" style="margin-bottom: 10px;">
                          <span style="color: #1e6cff !important; font-weight:bold;">👨‍👩‍👧‍👦 ${this.escapeHTML(fbPost.title)}</span>
                        </h3>
                        <div class="post-header">
                          <div class="post-header-line-1">
                              <span class="post-author" style="color: #555; font-weight: bold; font-size: 13px;">Από: ${this.escapeHTML(fbPost.author)}</span>
                     ...
                     <div class="post-body entry-content summary-wrapper" style="margin-top: 15px; font-size: 110%; line-height: 1.5;">
                            ${imgHtml}
                            ${ytHtml}
                            <!-- ΔΙΟΡΘΩΣΗ 2: Αλλαγή από <p> σε <div> και αφαίρεση του white-space: pre-wrap -->
                            <!-- [ΝΕΟ 3] Προστέθηκε word-break για να μη "σπάνε" την οθόνη του κινητού τα μεγάλα links! -->
                            <div style="margin:0; word-break: break-word; overflow-wrap: anywhere;">${firstPart}</div>
                            ${hasMore ? `
                            <div class="fetched-content-wrapper local-content-wrapper" style="display:none;">
                                <div class="fetched-content-inner">
                                    <div class="scrollable-article-area">
                                        <!-- ΔΙΟΡΘΩΣΗ 3: Αφαίρεση του pre-wrap και από το δεύτερο μέρος -->
                                        <div style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed #ddd; word-break: break-word; overflow-wrap: anywhere;">${secondPart}</div>
                                    </div>
                                </div>
                            </div>
                            <!-- Το 'local-read-more' ενημερώνει το σύστημα να μην ψάξει για Fetch URL -->
                            <div class="custom-read-more local-read-more">${CONFIG.LABELS.READ_MORE}</div>
                            ` : ''}
                        </div>
            ...
            // Αν μπήκαν νέα άρθρα Firebase, δίνουμε εντολή στη μηχανή να βάλει τα Emojis τους!
            if (hasNewInsertions) FeedbackEngine.scan();
        }
    }
  ...
  const AppController = {
        init: () => {
            ...
            // [ΝΕΟ] Εκκίνηση της μηχανής για τα άρθρα γονέων!
            ParentPostsEngine.init();
            ...
            document.addEventListener('newPostsLoaded', () => {
                FeedbackEngine.scan(); 
                ArticleEngine.scan(); 
                // [ΝΕΟ] Επανατοποθέτηση άρθρων γονέων αν γίνει Infinite Scroll (Load More)
                setTimeout(() => ParentPostsEngine.placePostsChronologically(), 500);
            });
             ...
             // 2. Click σε Read More Button
                ...
                if (readMoreBtn) {
                   ...
                   // Κλείσιμο των υπόλοιπων ανοιχτών άρθρων για εξοικονόμηση μνήμης (RAM) στο κινητό!
                    document.querySelectorAll('.fetched-content-wrapper.is-open').forEach(wrapper => {
                        ...
                        if (post && !post.contains(readMoreBtn)) { 
                            if (wrapper.classList.contains('local-content-wrapper')) {
                                wrapper.style.display = 'none';
                                wrapper.classList.remove('is-open');
                                const pb = post.querySelector('.custom-read-more');
                                if (pb) pb.innerHTML = CONFIG.LABELS.READ_MORE;
                    ...
                    // [ΝΕΟ] Αν είναι τοπικό άρθρο Firebase (των γονέων), ανοιγοκλείνει ακαριαία χωρίς Fetch!
                    if (readMoreBtn.classList.contains('local-read-more')) {
                        const post = readMoreBtn.closest(CONFIG.SELECTORS.ARTICLE_CONTAINER);
                        const wrapper = post.querySelector('.local-content-wrapper');
                        
                        if (wrapper.classList.contains('is-open')) {
                            wrapper.style.display = 'none';
                            wrapper.classList.remove('is-open');
                            readMoreBtn.innerHTML = CONFIG.LABELS.READ_MORE;
                            post.classList.remove('is-expanded');
                        } else {
                            wrapper.style.display = 'block';
                            wrapper.classList.add('is-open');
                            readMoreBtn.innerHTML = CONFIG.LABELS.CLOSE;
                            post.classList.add('is-expanded');
                            wrapper.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                        }
                        return;
               ...
               if (lightboxImg) {
                  const parentLink = lightboxImg.closest('a');
                    // [ΝΕΟ] Επιτρέπουμε στο Lightbox να ανοίγει και τις εικόνες των γονέων (parent-post-img)
                    const isBloggerImage = parentLink && parentLink.href && (parentLink.href.match(/\.(jpe?g|png|gif|webp)(\?.*)?$/i) || parentLink.href.includes('googleusercontent.com/img/') || parentLink.href.includes('bp.blogspot.com/') || parentLink.classList.contains('parent-post-img'));
       ...
       fetchPosts: async (retryCount = 0) => {
          ...
          let entry = finalData.feed?.entry?.[0];
                ...
                if (diffDays <= 365) {
                    ...
                    if (Math.random() > 0.15 && retryCount < 10) {
