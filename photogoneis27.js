...
window.startWatchdog = function() {
  ...
  // Ορίζουμε 8 δευτερόλεπτα όριο ανοχής
  loadingWatchdog = setTimeout(() => {
    console.warn("Αργή σύνδεση: Διακοπή Firebase για εξοικονόμηση μνήμης (RAM) & CPU.");
    
    // 1. "Σκοτώνουμε" εντελώς τη σύνδεση της Firebase για να ελευθερωθεί η συσκευή!
    try { dbPub.goOffline(); } catch(e) {}
    dbPub.ref("album_settings").off();
    if (activeQuery) activeQuery.off();
    
    // 2. Κρύβουμε το αιώνιο "Φόρτωση..." και τυχόν κουμπιά
    ...
    // 3. Εμφανίζουμε φιλικό μήνυμα σφάλματος και απεγκλωβισμού
    if (emptyState) {
      ...
      emptyState.innerHTML = `
        <div style="text-align: center; padding: 25px 15px; border: 1px dashed #ccc; border-radius: 8px; background: #fffaf0;">
          ...
          <h3 style="color: #333; margin: 0 0 10px 0; font-family: sans-serif;">Αδύναμο Σήμα</h3>
          <p style="color: #666; font-size: 14px; margin-bottom: 20px; font-family: sans-serif;">
            Η φόρτωση διακόπηκε για να μην "κολλήσει" η συσκευή σας από την προσπάθεια σύνδεσης.
          </p>
...
window.stopWatchdog = function() {
  clearTimeout(loadingWatchdog); // Η φόρτωση πέτυχε! Ακυρώνουμε το χρονόμετρο.
};
...
// Περιμένουμε σίγουρα το DOM να είναι έτοιμο!
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initAlbumSettings);
} else {
  initAlbumSettings();
}
...
window.checkUploadLimit = async function() {
  ...
  // 3. Συνεχίζει κανονικά τον έλεγχο για το ανέβασμα...
