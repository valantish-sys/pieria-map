...
if (isMarkdown) {
      if (typeof DOMPurify !== 'undefined' && !window.isPurifyHookAdded) {
...
if (window.visualViewport) {
    ...
    window.visualViewport.addEventListener('resize', () => {
      ...
      // ΝΕΟ: Scroll κάτω ΜΟΝΟ αν το ύψος μειωθεί δραματικά (π.χ. >30px, άνοιγμα ψηφιακού πληκτρολογίου).
      // Αποτρέπει τον εγκλωβισμό του χρήστη από τις αυξομειώσεις της μπάρας του Safari!
      if (DOM.window.classList.contains('is-open') && viewportHeight < lastVpHeight - 30) {
        DOM.messagesBox.scrollTop = DOM.messagesBox.scrollHeight;
      }
