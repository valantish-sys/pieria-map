(() => {
   ...
   const CONFIG = Object.freeze({
            ...
            while (hasMore) {
                try {
                    ...
                    if (data.feed?.entry && data.feed.entry.length > 0) {
                        ...
                        // Αν φέρει κάτω από 150, σημαίνει ότι φτάσαμε στο 1ο σου άρθρο!
    ...
    const UIEngine = {
       ...
       init: () => {
            // ΔΙΟΡΘΩΣΗ: Πρέπει να ελέγχει το ίδιο ακριβώς ID που φτιάχνει από κάτω!
            ...
            UIEngine.overlay.addEventListener('click', UIEngine.closeTooltip);
        ...
        showTooltip: (cellFrame, posts, isModal, isPC) => {
            ...
            if (isModal || !isPC) {
                ...
                const topPos = rect.top + window.scrollY - UIEngine.tooltip.offsetHeight + 10;
                const leftPos = rect.left + window.scrollX + (rect.width / 2) - (UIEngine.tooltip.offsetWidth / 2);
    ...
    const CalendarWidget = (suffix) => {
        ...
        const self = {
            ...
            self.els.yearOverlay = document.createElement('div');
                ...
                // ΔΙΟΡΘΩΣΗ 1: Δίνουμε μοναδικό ID προσθέτοντας το suffix
                self.els.yearOverlay.id = `year-view-overlay${suffix}`; 
                
                // ΔΙΟΡΘΩΣΗ 2: Του βάζουμε κλάση για να το "πιάνει" το CSS σου!
                self.els.yearOverlay.className = 'year-view-overlay-class'; 
                
                self.els.container.appendChild(self.els.yearOverlay); // Μόνο 1 φορά (Σβήσαμε τη διπλή)
                ...
                self.calendar = new window.FullCalendar.Calendar(self.els.calendarEl, {
                    ...
                    datesSet: (info) => {
                        ...
                        self.els.monthLabel.textContent = monthName.charAt(0).toUpperCase() + monthName.slice(1);
            ...
            playRoulette: () => {
                ...
                const spinInterval = setInterval(() => {
                    ...
                    self.calendar.gotoDate(new Date(randomYear, randomMonth, 1));
                    ...
                    if (spins >= maxSpins) {
                        ...
                        setTimeout(() => {
                            ...
                            // ΑΣΦΑΛΕΙΑ: Ανοίγει 100% εγγυημένα ακόμα κι αν το "κουτάκι" αργήσει να ζωγραφιστεί λόγω lag (χρησιμοποιώντας το body ως στόχο)
                            if (cell || !isPC) {
            ...
            setupEvents: () => {
                ...
                // Touch & Swipes (Για κινητά ΚΑΙ PC με οθόνη αφής)
                let touchstartX = 0, startY = 0;
                ...
                if (isPC) {
                    ...
                    // Μαγνήτης Ποντικιού (Τρέχει μόνο σε Desktop)
                    self.els.container.addEventListener('mousemove', (e) => {
                        ...
                        self.els.container.querySelectorAll('.post-dot').forEach(dot => {
                            if (frame && frame.contains(dot)) return;
                            dot.style.transform = ''; dot.style.background = ''; dot.style.boxShadow = '';
                        });
    ...
    const AppController = {
        init: () => {
            ...
            suffixes.forEach(suffix => {
                if (document.getElementById(`calendar-container${suffix}`)) {
            ...
            const waitForCalendar = setInterval(async () => {
                if (window.FullCalendar) {
                    ...
                    // Κατεβάζει τα δεδομένα 1 ΦΟΡΑ, ταχύτατα, και τα μοιράζει
                    await Promise.all([
