...
if (DOM.shareMenu?.classList.contains('show') && !DOM.shareMenu.contains(t) && !t.closest('#share-master-fab, #share-master-fab-mob')) DOM.shareMenu.classList.remove('show');
  ...
  const initMobileFabs = () => {
    ...
    let isVisible = true;
    let ticking = false;
    // Χρησιμοποιούμε Math.max για να αποφύγουμε τα αρνητικά νούμερα (bouncing) στα iPhone
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          if (e && e.type === 'scroll') {
            ...
            // Ανοχή 10px για να μην εξαφανίζεται/εμφανίζεται με το παραμικρό τρέμουλο του δαχτύλου
            if (delta > 10) {
              // ⬇️ Scroll προς τα ΚΑΤΩ: Άμεση εξαφάνιση και οριστική ακύρωση του χρονομέτρου
              setVisibility(false);
              clearTimeout(hideTimeout);
              lastScrollY = currentScrollY;
            } else if (delta < -10) {
              // ⬆️ Scroll προς τα ΠΑΝΩ: Άμεση εμφάνιση και έναρξη του χρόνου για να κρυφτεί αργότερα
              setVisibility(true);
              clearTimeout(hideTimeout);
            ...
            // Ανανεώνει τον χρόνο ΜΟΝΟ αν το κουμπί είναι ήδη ορατό. (Δεν εμφανίζεται αν το έχεις κρύψει με scroll down)
            if (isVisible) {
              clearTimeout(hideTimeout);
              hideTimeout = setTimeout(() => setVisibility(false), CONFIG.FAB_HIDE_DELAY);
            }
  ...
  const initDraggableDial = () => {
   ...
   mainBtn.addEventListener('touchend', () => {
     ...
     // Υπολογίζουμε το πραγματικό bottom με βάση το ΣΤΑΘΕΡΟ screenH (όχι αυτό μετά από 400ms)
      ...
      setTimeout(() => { 
        isDragging = false; 
        // BUG FIX: Καθαρίζουμε το hasMoved με καθυστέρηση. Στα κινητά, αν γίνει μεγάλο drag, 
        // ο browser ακυρώνει το 'click' event από μόνος του. Αν δεν καθαριστεί εδώ το hasMoved,
        // το κουμπί μπλοκάρει και νομίζει μόνιμα ότι το σέρνεις (απαιτεί διπλό tap)!
        hasMoved = false; 
      }, 150);
...
// ==========================================
 ...
 // Κεντρική συνάρτηση επιλογής
  const handleSelection = () => {
    ...
    // BUG FIX 1: Αποτροπή εμφάνισης λεξικού όταν ο χρήστης προσπαθεί να γράψει σε φόρμες (π.χ. αναζήτηση/σχόλια)!
    ...
    if (word.length >= 3 && word.length <= 30 && word.split(/\s+/).length <= 2) {
      ...
      // BUG FIX 2: Προσθήκη window.scrollX / window.scrollY. Διαφορετικά, αν ο χρήστης 
      // έχει κάνει scroll κάτω, το συννεφάκι θα εμφανιστεί στον "ουρανό" της σελίδας!
  ...
  const initLinkPreviews = () => {
   ...
   let hidePreviewTimer; // ΠΡΟΣΘΗΚΗ: Χρονόμετρο απόκρυψης
    ...
    document.addEventListener('mouseout', (e) => {
      ...
      // BUG FIX: Δίνουμε χρόνο ανοχής 150ms στο ποντίκι να διασχίσει το "κενό" των 10px 
      // χωρίς να κλείσει ακαριαία το συννεφάκι στο πρόσωπο του χρήστη!
