...
async function submitParentPost() {
    ...
    try {
        ...
        if (selectedFile) {
            if (typeof Swal !== 'undefined') Swal.fire({ title: 'Επεξεργασία & Ανέβασμα...', allowOutsideClick: false, allowEscapeKey: false, didOpen: () => Swal.showLoading() });
        ...
        console.error(error);
        let errorMsg = error.name === 'AbortError' ? 'Αργή σύνδεση! Το ανέβασμα ακυρώθηκε.' : 'Υπήρξε πρόβλημα κατά την αποστολή. Δοκιμάστε ξανά.';
