(() => {
    ...
    e.preventDefault(); // Κρατάμε τον χρήστη στη σελίδα!

    if (DOM.form && DOM.form.action) {
        // 1. Παίρνουμε τα δεδομένα της φόρμας
        const formData = new FormData(DOM.form);
        // 2. Η ΛΥΣΗ: Τα μετατρέπουμε στη μορφή που καταλαβαίνουν οι servers (URL Encoded)
        const urlEncodedData = new URLSearchParams(formData);

        // 3. Στέλνουμε στο παρασκήνιο
        fetch(DOM.form.action, {
            method: DOM.form.method || 'POST',
            body: urlEncodedData, // Χρησιμοποιούμε τα μετασχηματισμένα δεδομένα
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded' // Το "διαβατήριο" για να το δεχτεί η υπηρεσία
            },
            mode: 'no-cors'
        }).catch(() => {});
    }
