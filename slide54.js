...
(() => {
 ...
 // 1. Απλή αναγνώριση Touch (για να ξέρει το site ότι είμαστε σε κινητό)
  window.addEventListener('touchstart', () => { STATE.isTouchDevice = true; }, { passive: true, once: true });
  ...
  const NavManager = {
    init: () => {
      ...
      // --- ΝΕΟ: ΕΞΥΠΝΟΣ ΚΑΘΑΡΙΣΜΟΣ ΜΕΝΟΥ (GHOST HOVER & FOCUS FIX) ---
      const allDropdowns = document.querySelectorAll('.dropdown-content, .sub-dropdown-content');
      
      const cleanMenus = (activeLi = null) => {
        allDropdowns.forEach(d => {
          // Αν δεν υπάρχει επιλογή, ή το μενού ΔΕΝ είναι ούτε γονιός ούτε παιδί της επιλογής μας:
          if (!activeLi || (!activeLi.contains(d) && !d.contains(activeLi))) {
            // Το εξαφανίζουμε ακαριαία για να σπάσει το κολλημένο CSS του browser
            d.style.display = 'none';
            setTimeout(() => { d.style.display = ''; }, 50); // Επαναφορά σε 50ms
          }
        });

        // ΣΗΜΑΝΤΙΚΟ: Αφαιρεί την "εστίαση" (focus) αν άνοιξες link σε νέο παράθυρο!
        if (document.activeElement && nav.contains(document.activeElement)) {
           if (!activeLi || !activeLi.contains(document.activeElement)) {
               document.activeElement.blur();
           }
        }
      };

      // 1. Hover (Ποντίκι / Desktop): Καθαρίζει τα διπλανά μόλις πας πάνω τους
      nav.querySelectorAll('li').forEach(li => {
        li.addEventListener('mouseenter', () => cleanMenus(li), PASSIVE);
      });

      // 2. Click (Αφή / Mobile ή κλικ έξω από το μενού)
      document.addEventListener('click', (e) => {
        if (!nav.contains(e.target)) {
          ...
          if (clickedLi) cleanMenus(clickedLi);
        }
      });
      
      // 3. Αλλαγή Καρτέλας: Τα μαζεύει όλα αν ο χρήστης βγει εντελώς από το παράθυρο
      window.addEventListener('blur', () => cleanMenus(null));
  ...
  const ReactionsManager = {
    observer: new IntersectionObserver((entries, observer) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
         ...
         // ΑΝΤΙΚΑΤΑΣΤΗΣΕ ΤΟ onValue BLOCK ΜΕ ΑΥΤΟ:
    ...
    handleVote: (btn) => {
      ...
      .then(() => {
        ...
        // --- MAGICAL EMOJI FOUNTAIN ΕΔΩ (ΜΕΣΑ ΣΤΟ THEN) ---
        if (navigator.vibrate) navigator.vibrate(20); // Δόνηση επιβεβαίωσης
        ...
        for (let i = 0; i < 5; i++) {
          ...
          setTimeout(() => particle.remove(), 1000); // Καθαρισμός μνήμης
        }
      })
      .catch((error) => {
        console.error("Σφάλμα Firebase:", error);
        rDiv.classList.remove('is-voting', 'voted');
        rDiv.style.pointerEvents = 'auto'; // Το έβαλες (σωστά), αλλά ...
        countSpan.textContent = "0"; // <-- Βάλε 0, όχι "!", για να ξέρει ότι δεν μέτρησε!
      });
    }
  };
    ...
    init: () => {
     ...
     // --- ΝΕΟ: Ενοποιημένο Swipe (Αριστερά/Δεξιά & Κάτω για κλείσιμο) ---
      ...
      LM.el.addEventListener('touchstart', e => {
        if (e.touches.length > 1) return; // Αγνοούμε το multi-touch για pinch-to-zoom
      ...
      LM.el.addEventListener('touchmove', e => {
        ...
        if (diffY > 15 && diffY > diffX) {
          isSwipingDown = true;
          // Ρίχνουμε σταδιακά τη διαφάνεια του μαύρου φόντου
          const opacity = Math.max(0.2, 0.92 - (diffY / window.innerHeight) * 1.5);
          LM.el.style.backgroundColor = `rgba(0, 0, 0, ${opacity})`;
          
          // Μετακινούμε την εικόνα/βίντεο και την μικραίνουμε ελαφρώς
          const activeMedia = LM.video.style.display !== 'none' ? LM.video : LM.img;
          if (activeMedia) activeMedia.style.transform = `translateY(${diffY}px) scale(${1 - (diffY / 2500)})`;
        }
      }, PASSIVE);
      ...
      LM.el.addEventListener('touchend', () => {
        ...
        const diffX = startX - currentX; // Θετικό σημαίνει Swipe Left
        const diffY = currentY - startY;
        const activeMedia = LM.video.style.display !== 'none' ? LM.video : LM.img;

        // Επαναφέρουμε τα CSS animations ("ελατήριο")
        ...
        if (isSwipingDown && diffY > 0) {
          // Αν το τράβηξε αρκετά κάτω (> 120 pixels) -> ΚΛΕΙΣΙΜΟ
          if (diffY > 120) {
            LM.close();
          } else {
            // Αλλιώς -> SNAP BACK (Επιστροφή στην αρχική θέση)
            LM.el.style.backgroundColor = '';
            if (activeMedia) activeMedia.style.transform = '';
          }
        } 
        // Οριζόντιο Swipe (το παλιό σου Left/Right)
        else if (Math.abs(diffX) > 50 && Math.abs(diffX) > Math.abs(diffY)) {
          diffX > 0 ? LM.next() : LM.prev();
        }

        // Καθαρισμός των style rules
        setTimeout(() => {
...
const { imgs, text: fullText, widgetHtml } = Utils.extractData(postContent);
    ...
    setupEvents: () => {
      ...
      SM.container.addEventListener('click', (e) => {
        ...
        // --- ΝΕΟ: Αν πατήσει στο μενού, το slider παγώνει και ΔΕΝ μπλοκάρει το κλικ! ---
       // ΑΝΤΙΚΑΤΑΣΤΗΣΕ ΤΟ ΑΡΧΙΚΟ WIDGET CHECK ΜΕ ΑΥΤΟ:
        if (e.target.closest('.widget-overlay')) {
            ...
            // ΑΦΑΙΡΕΣΑ το return! Αφήνουμε το click να φτάσει στο μενού.
            // Απλώς φροντίζουμε να ΜΗΝ πιαστεί από τα controls του slider από κάτω.
      ...
      SM.container.addEventListener('touchstart', e => {
        if (e.target.closest('.slide-desc-container') || e.target.closest('.widget-overlay')) return;
