...
window.loadPhotos = function(isLoadMore = false) {
 ...
 // ΔΙΟΡΘΩΣΗ: Κατεβάζουμε ΜΟΝΟ τις φωτογραφίες του ενεργού θέματος, εξοικονομώντας >90% δεδομένων!
  activeQuery = dbPub.ref("album_photos").orderByChild("theme_id").equalTo(currentViewId);

 activeQuery.on("value", snap => {
      let allFetched = [];
      snap.forEach(child => {
        // Φιλτράρουμε τις εγκεκριμένες τοπικά (client-side)
