   ...
   setupSensors: () => {
    ...
    // Β. ΓΥΡΟΣΚΟΠΙΚΟ PARALLAX (Κινητά/Tablets)
      ...
      if (window.DeviceOrientationEvent && container) {
        // ΝΕΟ: Σε iOS 13+ απαιτείται ρητή άδεια με click για το Γυροσκόπιο
        ...
        window.addEventListener('deviceorientation', (e) => {
          ...
          // ΑΛΛΑΓΗ: Αν ο χρήστης κάνει Swipe εκείνη τη στιγμή, το Γυροσκόπιο αγνοεί την κλίση!
          if (container.dataset.isSwiping === 'true') return;

          // 1. Διαιρούμε με το 3 για να ρίξουμε την ευαισθησία
    ...
    setupPulse: () => {
     ...
     if (!document.getElementById('pulse-css')) {
        ...
        try {
         ...
         connectedRef.on('value', (snap) => {
            if (snap.val() === true) {
              ...
              // ΝΕΟ: Επαναφορά του ακροατή όταν η καρτέλα επανέρχεται από το background (BFCache)
      ...
      const checkAndConnect = () => {
          // ΔΙΟΡΘΩΣΗ: Πρέπει να βεβαιωθούμε ότι το Firebase app έχει φορτώσει πλήρως (apps.length > 0)
          if (typeof window.radioDb !== 'undefined' || (typeof firebase !== 'undefined' && firebase.apps && firebase.apps.length > 0)) {
              connectFirebase();
    ...
    setupSwipeSupport: () => {
     ...
     // ΜΕΤΑ:
      ...
      // ΜΕΤΑ:
       ...
       // ΜΕΤΑ:
        // ΝΕΟ: Αν κουνάει το δάχτυλο πιο πολύ πάνω-κάτω (Scroll), ακύρωσε το Swipe του Jukebox!
        // ΑΛΛΑΓΗ: Ελέγχουμε την κάθετη καμπύλη ΜΟΝΟ στα πρώτα 15px (Math.abs(currentX) < 15), για να μη σπάει το Swipe στη μέση της κίνησης!
       if (Math.abs(currentX) < 15 && Math.abs(currentY) > Math.abs(currentX) && Math.abs(currentY) > 15) {
     ...
     // ΜΕΤΑ:
      const handleDragEnd = () => {
        ...
        isDragging = false;
        container.dataset.isSwiping = 'false'; // ΑΛΛΑΓΗ: Ελευθερώνουμε ξανά το Γυροσκόπιο
        // ΝΕΟ: Αν ο χρήστης έκανε κάθετο scroll, ακυρώνουμε εντελώς το Swipe!
       // ΜΕΤΑ:
        // ΝΕΟ: Αν ο χρήστης έκανε κάθετο scroll, ακυρώνουμε εντελώς το Swipe!
        if (isVerticalScrolling) {
            currentX = 0;
            container.style.opacity = '1'; // ΑΛΛΑΓΗ: Εξασφαλίζουμε ότι η κάρτα επέστρεψε οπτικά στο κανονικό
            return;
        }
      ...
      if (currentX < -SWIPE_THRESHOLD) {
          // ΑΛΛΑΓΗ: Καλούμε την αλλαγή ΤΩΡΑ συγχρονισμένα για να μην το μπλοκάρει το iOS!
