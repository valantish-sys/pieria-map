           ...
           // Fetch data ONCE for both widgets
            ...
            ['', '-mobile'].forEach(suffix => {
               ...
               const prevBtn = document.getElementById(`eort-prev-day${suffix}`);
                ...
                // ΑΦΑΙΡΕΘΗΚΕ ΤΟ revealSearch() ΠΟΥ ΚΑΤΕΣΤΡΕΦΕ ΤΟ UI
              ...
              eortBox.addEventListener('touchend', (e) => {
                    // Αποτροπή αλλαγής μέρας αν ο χρήστης κάνει swipe-back (iOS) στην άκρη της οθόνης
                    if (touchStartX < 30 || touchStartX > window.innerWidth - 30) return;

                    const diffX = touchStartX - e.changedTouches[0].screenX;
                    const diffY = touchStartY - e.changedTouches[0].screenY;
                    // ΑΦΑΙΡΕΘΗΚΕ ΤΟ revealSearch() ΚΑΙ ΠΡΟΣΤΕΘΗΚΕ * 1.5 ΣΤΟ diffY ΩΣΤΕ ΝΑ ΜΗΝ ΑΛΛΑΖΕΙ ΜΕΡΑ ΚΑΤΑ ΛΑΘΟΣ ΟΤΑΝ ΣΚΡΟΛΑΡΕΙ ΚΑΘΕΤΑ
