...
// --- 1. ΟΜΑΛΗ ΜΕΤΑΒΑΣΗ (FADE IN/OUT) ---
    ...
    // --- 3. ΜΝΗΜΗ & ΣΥΝΕΧΕΙΑ (LOCAL STORAGE) ---
    saveProgress: () => {
      ...
      const data = {
        url: activeBtn.dataset.url,
        name: activeBtn.dataset.name,
        time: els.player.currentTime,
        loopIndex: STATE.loopModeIndex,
        volume: STATE.targetVolume
      };
      localStorage.setItem('jukebox_memory', JSON.stringify(data));
    },

    loadProgress: () => {
      const saved = localStorage.getItem('jukebox_memory');
      if (!saved) return;
      
      try {
        const data = JSON.parse(saved);
        
        // 1. Επαναφορά ρυθμίσεων Επανάληψης (Loop)
        STATE.loopModeIndex = (data.loopIndex + 2) % 3; // Μαθηματικό τρικ για το toggle
        JukeboxManager.toggleLoop();
        
        // 2. Επαναφορά Έντασης (Volume)
        if (data.volume !== undefined) {
           STATE.targetVolume = data.volume;
           if (JukeboxManager.dom.player) JukeboxManager.dom.player.volume = data.volume;
        }

        // 3. Ερώτηση για συνέχιση αν είχε ακούσει πάνω από 10 δευτερόλεπτα
        if (data.time > 10) {
          setTimeout(() => {
            if (confirm(`🎧 Βρήκαμε ότι ακούγατε:\n"${data.name}"\nΘέλετε να συνεχίσετε από εκεί που μείνατε;`)) {
              const btn = document.querySelector(`[data-url="${data.url}"]`);
              if (btn) JukeboxManager.playTrack(btn, data.url, data.name, data.time);
            }
          }, 1000); // 1 δευτερόλεπτο καθυστέρηση για να προλάβει να χτιστεί η σελίδα
        }
      } catch (e) { console.warn("Σφάλμα ανάγνωσης μνήμης"); }
    },
    init: () => {
      ...
      // 2. Αθόρυβη αποθήκευση μνήμης κάθε 5 δευτερόλεπτα (μόνο αν παίζει)
      els.player.addEventListener('timeupdate', () => {
        if (Math.floor(els.player.currentTime) % 5 === 0) JukeboxManager.saveProgress();
      });

      // 3. Ενεργοποίηση μνήμης με το άνοιγμα της σελίδας
      JukeboxManager.loadProgress();
    },

    playTrack: (button, url, name, savedTime = 0) => {
      ...
      const executeChange = () => {
        ...
        if (savedTime > 0) els.player.currentTime = savedTime; // Πήγαινε στο σημείο της μνήμης
    ...
    handleTrackEnd: () => {
      localStorage.removeItem('jukebox_memory');
      const els = JukeboxManager.dom;
