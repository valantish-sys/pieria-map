...
"– Γιατί το βιβλίο των μαθηματικών είναι πάντα λυπημένο;<br>– Επειδή έχει πολλά προβλήματα! 📘😥",
    ...
    // Δηλώνεται άδειο για να μην έχουμε null errors αν το JS φορτώσει στο <head>
    const DOM = {}; 
    ...
    const Utils = {
        ...
        // Ασφαλής προσπέλαση του Storage (Προστατεύει από crashes στο Incognito)
        safeStorageGet: (key) => {
    ...
    const CoreEngine = {
        update: () => {
            ...
            // FIXED Date BUG: Δημιουργία ξεχωριστών Date objects για να μην μολύνεται η μνήμη!
        ...
        updateBattery: (now, isHoliday, easterStart) => {
            ...
            // FIXED BUG: Χρήση του getDayOfYear για ασύγκριτη ακρίβεια χωρίς memory mutations
            const isEaster = (dayOfYear >= easterStartDay && dayOfYear <= easterStartDay + 16);
            ...
            if (isSummer) {
                ...
                if (dayOfYear >= 244 && dayOfYear <= 356) {
                    ...
                    batLevel = 70 - ((dayOfYear - (easterStartDay + 15)) * 1.85); 
                }
            }

            batLevel = Math.max(5, Math.min(100, Math.round(batLevel)));

            // 💡 SENIOR PRACTICE: Η JS βάζει ΜΟΝΟ Data-State. Όλα τα χρώματα τα διαχειρίζεται το CSS.
            DOM.batFill.style.width = `${batLevel}%`;
            DOM.batText.innerHTML = `Μπαταρία Δασκάλων: ${batLevel}% ${isHoliday ? '<span class="charging-icon" aria-hidden="true">⚡</span>' : ''}`;
            
            let state = 'high';
            if (batLevel <= 20) state = 'low';
            else if (batLevel <= 50) state = 'medium';
            DOM.batFill.setAttribute('data-state', state);
        }
    };
    ...
    const MessageManager = {
        ...
        handleInteraction: (e) => {
            // A11y: Επιτρέπει Enter & Space (αν ο χρήστης πλοηγείται με πληκτρολόγιο)
            if (e.type === 'keydown' && e.key !== 'Enter' && e.key !== ' ') return;
            
            e.stopPropagation(); 
            if (e.type === 'keydown') e.preventDefault(); // Αποφυγή scroll με το Spacebar

            if (MessageManager.isShowing || MESSAGES_DB.length === 0) return;
            
            MessageManager.isShowing = true;
            DOM.mainContent.hidden = true;
            DOM.secretBox.hidden = false;
            
            let shown = Utils.safeStorageGet(CONFIG.storageKey);
            if (shown.length >= MESSAGES_DB.length) shown = []; 
            
            const available = MESSAGES_DB.map((_, i) => i).filter(i => !shown.includes(i));
        ...
        hide: (e) => {
            // UX: Αν ο χρήστης προσπαθεί να επιλέξει/αντιγράψει το μήνυμα, ΜΗΝ το κρύψεις!
            if (e && e.target.closest('#holiday-secret-message')) return;

            if (!MessageManager.isShowing) return;
            clearTimeout(MessageManager.timeout);
            DOM.secretBox.hidden = true;
    ...
    const LayoutManager = {
        ...
        moveWidget: () => {
            ...
            if (window.innerWidth <= CONFIG.mobileBreakpoint) {
                const targetWidget = document.getElementById("riddle-glass-base") || document.getElementById("HTML17");
                if (targetWidget) {
                ...
                if (DOM.originalLoc?.parentNode && DOM.widgetBox.parentNode !== DOM.originalLoc.parentNode) {
    ...
    const App = {
        init: () => {
            // Γεμίζουμε τα DOM nodes μόνο όταν φορτώσει η σελίδα!
            ...
            DOM.originalLoc = document.getElementById("holiday-original-location");

            CoreEngine.update();

            // Προσθήκη Keyboard Event Listener
            DOM.widgetBox.addEventListener('click', MessageManager.handleInteraction);
            DOM.widgetBox.addEventListener('keydown', MessageManager.handleInteraction);
