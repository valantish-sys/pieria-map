...
const DataEngine = {
    ...
    const MobileCoreEngine = {
        ...
        updateBattery: (now, isHoliday, easterStart, easterEnd) => {
           ...
           // --- ΝΕΟ: Έλεγχος των Εφέ Μπαταρίας ---
            DOM.batFill.className = "battery-fill"; // Καθαρισμός παλιών κλάσεων (επαναφορά στην αρχική)
            ...
            if (isHoliday) {
                ...
                // Κανονική λειτουργία (Χωρίς animations)
                if (batLevel <= 50) {
    ...
    const MobileMessageManager = {
        ...
        show: (e) => {
            e.stopPropagation(); 
            if (MobileMessageManager.isShowing || DataEngine.messagesArray.length === 0) return;
    ...
    const MobileApp = {
        init: async () => { // <--- Έγινε async
            ...
            // --- ΚΑΤΕΒΑΖΕΙ ΤΑ ΜΗΝΥΜΑΤΑ ΑΠΟ ΤΟ JSON ---
