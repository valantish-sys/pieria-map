...
window.updatePostStatus = (id, status) => {
 ...
 window.deletePost = (id) => {
  ...
  window.editPost = (id) => {
     ...
     // Υπολογισμός τοπικής ώρας για το input type="datetime-local"
...
// ΠΕΡΙΣΤΡΟΦΗ ΕΙΚΟΝΑΣ 90 ΜΟΙΡΕΣ ΔΕΞΙΟΣΤΡΟΦΑ
  window.rotateBlurImage = () => {
    if (!blurCanvas || isUploadingBlur || !blurCtx || !backupCtx) return;

    // 1. Προσωρινή αντιγραφή της τρέχουσας κατάστασης του καμβά
    ...
    // 2. Εναλλαγή διαστάσεων (πλάτος <-> ύψος)
    ...
    // 3. Σχεδίαση με περιστροφή 90 μοιρών στον βασικό καμβά
    blurCtx.save();
    ...
    // 4. Ενημέρωση και του backup καμβά ώστε να συγχρονιστούν οι μελλοντικές θολώσεις
...
window.removePostImage = (id) => {
  ...
  // --- 5 ΝΕΕΣ ΛΕΙΤΟΥΡΓΙΕΣ ADMIN ---

  // 1. ΣΚΟΤΕΙΝΟ ΘΕΜΑ (Dark Mode)
  ...
  window.smartCleanText = () => {
    ...
    // Καθαρίζει διπλά/τριπλά κενά
    text = text.replace(/ {2,}/g, ' ');
    // Διορθώνει κενό ΠΡΙΝ το κόμμα/τελεία/θαυμαστικό (π.χ. "λέξη ," -> "λέξη,")
    text = text.replace(/ \./g, '.').replace(/ ,/g, ',').replace(/ !/g, '!').replace(/ ;/g, ';');
    // Βάζει κενό ΜΕΤΑ το κόμμα/τελεία/ερωτηματικό, αν το ξέχασαν 
    text = text.replace(/([,;.!?])([^\s\d\n])/g, '$1 $2');
    // Αφαιρεί τις υπερβολικές κενές γραμμές
    text = text.replace(/\n{3,}/g, '\n\n');
  ...
  // 5. ΜΕΤΡΗΤΗΣ ΛΕΞΕΩΝ / ΧΡΟΝΟΥ (Προστέθηκε στον κώδικα της κάρτας παραπάνω!)
