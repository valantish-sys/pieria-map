...
import { getDatabase, ref, push, set, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-database.js";
...
// Απομονώνουμε πλήρως το Student Widget με δικό του όνομα, ώστε να μη συγκρούεται ποτέ με τον Τοίχο
const app = getApps().find(a => a.name === "StudentApp") ? getApp("StudentApp") : initializeApp(firebaseConfig, "StudentApp");
const db = getDatabase(app);
  ...
  let QUESTIONS_DB = [];
 try {
      // Ίδια πολιτική cache με τον τοίχο για απόλυτο συγχρονισμό!
...
// Αφαίρεση του 1 για απόλυτο συγχρονισμό με τον αλγόριθμο του Admin Panel
 const safeIndex = QUESTIONS_DB.length > 0 ? (Math.max(0, dayOfSchoolYear - 1) % QUESTIONS_DB.length) : 0;
 const dailyQ = QUESTIONS_DB[safeIndex] || { text: "Αναμονή για νέες ερωτήσεις...", emoji: "⏳" };
 const dayKey = `schoolYear_${schoolYear}_day_${dayOfSchoolYear}`;
  ...
  platforms.forEach(platform => {
      ...
      if (w) {
       ...
       if (btnOpen) {
                  btnOpen.onclick = () => { 
                      // Μπλοκάρισμα της φόρμας αν δεν έχουν φορτωθεί σωστά οι ερωτήσεις
                      if (QUESTIONS_DB.length > 0 && QUESTIONS_DB[0].emoji === "⚠️") {
                          alert("Αδυναμία σύνδεσης. Παρακαλώ ελέγξτε το ίντερνετ και ανανεώστε τη σελίδα.");
                          return;
                      }
           ...
           if (btnSubmit) {
                  ...
                  const handleSubmit = async (e) => {
                    ...
                    set(newAnswerRef, {
                              age: age, answer: text, status: 'pending', timestamp: serverTimestamp(), qIndex: safeIndex
                          }).catch(err => {
                              console.error("Σφάλμα σύνδεσης Firebase:", err);
                              alert("Αποτυχία αποστολής. Παρακαλώ ελέγξτε τη σύνδεσή σας.");
                          }); // Πιάνει σωστά τα ασύγχρονα σφάλματα χωρίς να "παγώνει" το πρόγραμμα.
