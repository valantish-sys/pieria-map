...
window.startWatchdog = function() {
  ...
  loadingWatchdog = setTimeout(() => {
    // ⚠️ ΣΒΗΣΑΜΕ ΤΑ .off() ΓΙΑ ΝΑ ΜΗΝ ΚΟΒΟΥΜΕ ΤΗ ΣΥΝΔΕΣΗ ΤΟΥ FIREFOX ΠΟΤΕ ΞΑΝΑ!
    console.warn("Ο Firefox καθυστερεί, αλλά συνεχίζουμε να περιμένουμε...");
    ...
    // Δείχνουμε απλώς ένα φιλικό μήνυμα αναμονής (ΧΩΡΙΣ ΚΟΥΜΠΙ ΕΠΑΝΑΦΟΡΤΩΣΗΣ)
  // Δείχνουμε απλώς ένα φιλικό μήνυμα αναμονής (ΧΩΡΙΣ ΚΟΥΜΠΙ ΕΠΑΝΑΦΟΡΤΩΣΗΣ)
    if (emptyState) {
      // ΔΙΟΡΘΩΣΗ: Σώζουμε το αρχικό HTML πριν το αντικαταστήσουμε για να μην χαθεί!
      ...
      emptyState.innerHTML = `
        <div style="text-align: center; padding: 25px 15px; border: 1px dashed #ccc; border-radius: 8px; background: #fffaf0;">
          ...
          <h3 style="color: #333; margin: 0 0 10px 0; font-family: sans-serif;">Ο Firefox ελέγχει τη σύνδεση...</h3>
          <p style="color: #666; font-size: 14px; margin-bottom: 20px; font-family: sans-serif;">
            Λόγω ρυθμίσεων ασφαλείας του browser σας, η σύνδεση καθυστερεί.<br>
            <b>Παρακαλώ περιμένετε, οι φωτογραφίες θα εμφανιστούν αυτόματα!</b>
          </p>
        </div>
      `;
    }
        ...
        .then(data => {
          if(data.secure_url) {
           ...
           dbPub.ref("album_photos").push({
              ...
              status: "pending",
              timestamp: Date.now()
            }).then(() => {  // ΔΙΟΡΘΩΣΗ: Ανοίγουμε block .then() για να περιμένουμε την επιβεβαίωση της βάσης!
           ...
           Swal.fire({
              ...
              confirmButtonColor: '#2ecc71'
                ...
                // ΔΙΟΡΘΩΣΗ: Αν αποτύχει η εγγραφή στη Firebase, κλείνουμε σωστά το block
