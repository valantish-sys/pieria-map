...
let currentWordRequested = '';
  ...
  // ΝΕΟΣ ΜΗΧΑΝΙΣΜΟΣ: Ενημερώνει το κείμενο ΚΑΙ υπολογίζει τα όρια της οθόνης
    const updateTooltip = (htmlContent) => {
      ...
      // ΠΡΟΣΘΗΚΗ: Δυναμικός περιορισμός πλάτους για αποφυγή οριζόντιας υπερχείλισης στα κινητά
      glossaryTooltip.style.maxWidth = `${window.innerWidth - 30}px`;
...
const applySliderPatch = (forceResume = false) => {
 ...
 // ΠΡΟΣΘΗΚΗ: Έλεγχος ώστε να μπλοκάρονται μόνο οι άγκυρες της ΙΔΙΑΣ σελίδας (αγνοώντας παραμέτρους όπως ?m=1 ή fbclid)
     ...
     // ΠΡΟΣΘΗΚΗ: Αποτροπή εμφάνισης παραθύρου αν το link περιέχει ΜΟΝΟ εικόνες (Blogger Lightbox / Gallery Links)
      const isOnlyImage = link.children.length > 0 && Array.from(link.children).every(child => child.tagName === 'IMG') && link.textContent.trim() === '';
