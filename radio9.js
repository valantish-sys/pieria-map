...
// --- 1. ΟΜΑΛΗ ΜΕΤΑΒΑΣΗ (FADE IN/OUT) ---
    ...
    // --- 4. ΖΩΝΤΑΝΟΣ ΠΑΛΜΟΣ (LIVE PULSE) ---
    setupPulse: () => {
      const header = document.querySelector('.juke-header');
      if (!header) return;
      
      // Φτιάχνουμε δυναμικά την ένδειξη (χωρίς να πειράξουμε το HTML σου)
      ...
      // Προσθήκη CSS Animation δυναμικά
      ...
      const updatePulse = () => {
         ...
         let base = (hour >= 8 && hour <= 14) ? 28 : 12; // Σχολικές ώρες = περισσότεροι ακροατές
         let current = base + Math.floor(Math.random() * 5); // Τυχαία αυξομείωση +0 εως +4
         pulseDiv.innerHTML = `<span style="animation: pulseAnim 2s infinite; display: inline-block;">🔴</span> ${current} μαθητές συντονισμένοι`;
      };
      
      updatePulse();
      setInterval(updatePulse, 8000); // Η ένδειξη "αναπνέει" ρεαλιστικά κάθε 8 δευτερόλεπτα
    },

    // --- 5. ZΩΝΤΑΝΗ ΚΑΡΤΕΛΑ (TAB MARQUEE) ---
    setupTabEffects: () => {
       let marqueeInterval, favInterval;
       const originalTitle = document.title;
       const originalFavicon = document.querySelector("link[rel~='icon']")?.href || '';

       const startEffects = () => {
          clearInterval(marqueeInterval); clearInterval(favInterval);
          const els = JukeboxManager.dom;
          const title = els.textTarget ? els.textTarget.innerText : 'Ραδιόφωνο';
          let scrollText = `🎵 ON AIR: ${title} • `;
          ...
          // Εικονίδιο Favicon που χορεύει
          let link = document.querySelector("link[rel~='icon']");
          if (!link) { link = document.createElement('link'); link.rel = 'icon'; document.head.appendChild(link); }
          
          const frames = ['🔈', '🔉', '🔊']; let f = 0;
          favInterval = setInterval(() => {
             link.href = `data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2280%22>${frames[f % 3]}</text></svg>`;
             f++;
          }, 400);
       };
       ...
       const stopEffects = () => {
          ...
          let link = document.querySelector("link[rel~='icon']");
          if (link) { if (originalFavicon) link.href = originalFavicon; else link.remove(); }
       };

       // Όταν ο χρήστης αλλάζει καρτέλα στον browser (παει παρασκήνιο)
       document.addEventListener('visibilitychange', () => {
          const els = JukeboxManager.dom;
          if (document.hidden && els.player && !els.player.paused) startEffects();
          else stopEffects();
       });

       // Σταματάει τα εφέ αν πατήσει pause ενώ είναι σε άλλο tab
       if (JukeboxManager.dom.player) {
          JukeboxManager.dom.player.addEventListener('pause', stopEffects);
          JukeboxManager.dom.player.addEventListener('play', () => { if (document.hidden) startEffects(); });
       }
    },
    ...
    init: () => {
      ...
      JukeboxManager.setupTabEffects();
