...
function generateMission(factText, missionsObj) {
  const text = factText.toLowerCase();
  let category = 'default';
  
  // Διάστημα & Αστρονομία
  if (text.includes("σελήνη") || text.includes("διάστημ") || text.includes("πλανήτ") || text.includes("apollo") || text.includes("τηλεσκόπιο") || text.includes("δορυφόρ") || text.includes("άρη") || text.includes("αστρονόμ") || text.includes("ήλιο")) {
    category = 'space';
  
  // Τεχνολογία & Εφευρέσεις
  } else if (text.includes("εφευρίσκει") || text.includes("υπολογιστ") || text.includes("apple") || text.includes("ρεύμα") || text.includes("λάμπα") || text.includes("αυτοκίνητ") || text.includes("windows") || text.includes("εφεύρεση") || text.includes("μηχαν") || text.includes("τηλέφων")) {
    category = 'tech';
  
  // Τέχνες, Κινηματογράφος & Βιβλία
  } else if (text.includes("βιβλ") || text.includes("ταινία") || text.includes("ζωγράφο") || text.includes("μίκυ") || text.includes("χάρι") || text.includes("πικάσο") || text.includes("σκίτσο") || text.includes("συγγραφέ") || text.includes("μουσικ") || text.includes("σινεμά")) {
    category = 'arts';
  
  // Εξερευνήσεις & Γεωγραφία
  } else if (text.includes("εξερευνητ") || text.includes("κολόμβος") || text.includes("μαγγελάνος") || text.includes("ωκεανό") || text.includes("νησ") || text.includes("καράβ") || text.includes("ήπειρ") || text.includes("διώρυγα") || text.includes("ταξί")) {
    category = 'exploration';
    
  // Ιστορία & Μάχες
  } else if (text.includes("επανάστασ") || text.includes("απελευθερώνεται") || text.includes("μάχη") || text.includes("βασιλι") || text.includes("ιστορ") || text.includes("πόλεμ") || text.includes("αυτοκρατορ") || text.includes("σύνταγμα")) {
    category = 'history';

  // Φυσική & Επιστήμη (Science)
  } else if (text.includes("φυσικ") || text.includes("χημικ") || text.includes("νεύτων") || text.includes("αϊνστάιν") || text.includes("βαρύτητ") || text.includes("θεωρία") || text.includes("μάθημα") || text.includes("στοιχείο")) {
    category = 'science';

  // Βιολογία, Ιατρική & Φύση
  ...
  // Επιλογή τυχαίας αποστολής από την κατηγορία που βρήκε
...
// === ΝΕΑ ΚΕΝΑ - ΙΑΝΟΥΑΡΙΟΣ (0) ===
  ...
  "11-31": "31 Δεκεμβρίου: Το 1879 ο Τόμας Έντισον ανάβει δημόσια για πρώτη φορά τους νέους του ηλεκτρικούς λαμπτήρες, φωτίζοντας τη νύχτα."
        ...
        const defaultFact = `Κάθε μέρα είναι μια ευκαιρία να μελετήσουμε το παρελθόν και να χτίσουμε ένα καλύτερο μέλλον.`;
        
        const factElement = document.getElementById("history-fact-mobile");
        if(factElement) {
            factElement.style.opacity = 0;
            
            const currentFact = historyFactsMobile[dateKey] || defaultFact;
            factElement.innerHTML = currentFact;
            
            // Το έξυπνο συμπλήρωμα για το κινητό
            // Το έξυπνο συμπλήρωμα για το κινητό
            const missionTextElementMobile = document.getElementById("mission-text-mobile");
            if (missionTextElementMobile && typeof generateMission === "function" && typeof dynamicMissions !== "undefined") {
              
              // Κλειδί μοναδικό για το κινητό gadget
              const storageKeyMobile = "daily_mission_" + dateKey + "_mobile";
              let savedMissionMobile = localStorage.getItem(storageKeyMobile);
              
              if (!savedMissionMobile) {
                  savedMissionMobile = generateMission(currentFact, dynamicMissions);
                  localStorage.setItem(storageKeyMobile, savedMissionMobile);
              }
              
              missionTextElementMobile.innerHTML = savedMissionMobile;
            }
...
// Η ΝΕΑ ΣΥΝΑΡΤΗΣΗ ΓΙΑ ΤΟ ΚΛΙΚ ΣΤΟ ΚΙΝΗΤΟ (Με προστασία stopPropagation)
function toggleMissionMobile(event) {
  if (event) {
    event.stopPropagation();
  }
  const missionContainer = document.getElementById("mission-container-mobile");
  if(missionContainer) {
    missionContainer.classList.toggle("open");
  }
}

// Συμπλήρωμα: Κλείσιμο ΜΟΝΟ για το mobile συρτάρι όταν κάνουμε κλικ έξω
document.addEventListener("click", function(event) {
  const containerMobile = document.getElementById("history-wrapper-container-mobile");
  const missionContainerMobile = document.getElementById("mission-container-mobile");
  
  // Έλεγχος αν το κλικ έγινε έξω από το mobile container και αν το mobile συρτάρι είναι ανοιχτό
  if (containerMobile && !containerMobile.contains(event.target)) {
    if (missionContainerMobile && missionContainerMobile.classList.contains("open")) {
      missionContainerMobile.classList.remove("open");
    }
  }
});
