(() => {
    ...
    const DOM = {};
   const DataEngine = {
        // ΝΕΟ: Εφεδρικό μήνυμα ώστε το widget να ανταποκρίνεται ακαριαία στο πρώτο κλικ, 
        // ακόμα κι αν το JSON καθυστερήσει ελάχιστα να κατέβει.
    ...
    const CoreEngine = {
        update: () => {
            ...
            DOM.display.innerHTML = `<span class="holiday-days">${daysText}</span> ${next.name}`;
        ...
        updateBattery: (now, isHoliday, easterStart, easterEnd) => {
           ...
           // Καθαρισμός παλιών κλάσεων (επαναφορά)
            // ΝΕΟ: Αφαιρούμε στοχευμένα ΜΟΝΟ τα εφέ της JS, προστατεύοντας τις HTML κλάσεις.
    ...
    const MessageManager = {
        ...
        show: (e) => {
            e.stopPropagation(); 
          if (e.target && e.target.closest && e.target.closest('#holiday-secret-message')) return;
        ...
        init: () => {
          ...
          DataEngine.fetchMessages();
           ...
           DOM.widgetBox.addEventListener('click', MessageManager.show);
            // ΝΕΟ: Προστασία για οθόνες αφής στα Laptops, εμποδίζοντας το touchstart
            // να κλείσει ακαριαία το widget που μόλις προσπάθησε να ανοίξει ο χρήστης.
