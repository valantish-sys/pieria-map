...
shareMenu: el('share-options'),
  ...
  const safeBind = (id, event, cb, opts = false) => {
    ...
    const safeStorage = {
    get: (key) => { try { return safeStorage.getItem(key); } catch(e) { return null; } },
    set: (key, val) => { try { safeStorage.setItem(key, val); } catch(e) {} },
    remove: (key) => { try { safeStorage.removeItem(key); } catch(e) {} }
  };
...
// Speed Dial Events
  safeBind('main-master-fab', 'click', () => {
    ...
    if (t) {
      ...
      safeStorage.setItem('has_seen_school_menu_tooltip', 'true');
    }
  });
...
safeBind('btn-do-print', 'click', () => {
    ...
    DOM.shareMenu?.classList.remove('show');
    // ΠΡΟΣΘΗΚΗ: Ελάχιστη αναμονή (100ms) για να προλάβει ο browser να κρύψει οπτικά το μενού 
    setTimeout(() => {
      window.print(); 
 ...
 const applySetting = (btn, cls, key, active) => {
    ...
    active ? safeStorage.set(key, 'true') : safeStorage.remove(key);
  };

  DOM.aBtns?.forEach(btn => {
    const { class: cls, key } = btn.dataset;
    if (!cls || !key) return;
    if (safeStorage.get(key) === 'true') applySetting(btn, cls, key, true);
...
if (tooltip && !safeStorage.getItem(tooltipKey)) {
    // 1. Περιμένει 2 δευτερόλεπτα πριν το εμφανίσει
    setTimeout(() => {
  
      if (!DOM.speedDialMenu?.classList.contains('is-open') && !safeStorage.getItem(tooltipKey)) {
        tooltip.classList.add('show-tooltip');
       safeStorage.setItem(tooltipKey, 'true'); // ΠΡΟΣΘΗΚΗ: Καταγραφή αμέσως μόλις εμφανιστεί!
      }
    }, 2000);
    ...
    setTimeout(() => {
     ...
     safeStorage.setItem(tooltipKey, 'true');
    }, 12000);
  }
