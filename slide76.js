...
(() => {
  ...
  const REGEX = Object.freeze({
   YT: /(?:embed\/|v=|youtu\.be\/|vi\/|\/v\/|e\/|shorts\/|watch\?v=)([^#\&\?]{11})/,
      ...
      const resetIdleTimer = Utils.throttle(() => {
       ...
       nav.classList.remove('nav-hidden-active');
        ...
        idleTimeout = setTimeout(() => {
          // Αποτρέπει την απόκρυψη αν το ποντίκι βρίσκεται εκείνη τη στιγμή πάνω στο μενού
   ...
   index: 0,
    ...
    LM.el.addEventListener('touchmove', e => {
      ...
      const diffX = Math.abs(currentX - startX);
        ...
        // Αποτρέπει την πλοήγηση Back/Forward του Browser στα οριζόντια swipes
        if (!isSwipingDown && diffX > Math.abs(diffY)) {
   ...
   setupEvents: () => {
    ...
    SM.container.addEventListener('touchstart', e => {
     ...
     SM.container.addEventListener('touchend', e => {
        ...
        if (e.target.closest('.slide-desc-container')) {
            // Αν το άγγιγμα ήταν στο κείμενο, δεν κάνουμε swipe, αλλά ΠΡΕΠΕΙ να επαναφέρουμε το auto-play
