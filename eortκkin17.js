  ...
  const DataEngine = {
    ...
    const MobileDateEngine = {
        ...
        calculateMovableWorldDays: function() {
            ...
            this.movableDays = {
                ...
                isLighthouseDay: (this.m === 8 && (this.d === Utils.getNthDayOfMonth(y, 8, 0, 3) || this.d === Utils.getNthDayOfMonth(y, 8, 0, 3) - 1)),
    ...
    const MobileHolidayEngine = {
        ...
        getNames: () => {
            ...
            } else if (MobileDateEngine.dateKey === "4-24") {
                if (!isMoved) names.push("Ελισάβετ");
            } else if (MobileDateEngine.dateKey === "4-25") {
                names.push(isMarkMoved ? "Νίκη" : "Μάρκος, Νίκη");
            } else {
                // 3. Υπόλοιπες Σταθερές (JSON)
...
// --- ΝΕΟ: ΜΗΧΑΝΗ ΑΝΑΖΗΤΗΣΗΣ (MOBILE) ---
    const MobileSearchEngine = {
        ...
        buildIndex: () => {
            const y = MobileDateEngine.today.getFullYear();
        ...
        init: async () => {
           ...
           // --- ΝΕΟ: ΕΞΥΠΝΗ ΤΗΛΕΜΕΤΑΦΟΡΑ (Time Travel) ---
            const animateToDate = (targetDate) => {
                ...
                const elements = eortBox.querySelectorAll('#main-icon-mobile, #eort-date-mobile, .info-box');
                ...
                setTimeout(() => {
                    ...
                    MobileUIEngine.renderHeader();
                    MobileUIEngine.renderHolidays();
                    MobileUIEngine.renderWeather(); // Κάνει update και τον καιρό!
