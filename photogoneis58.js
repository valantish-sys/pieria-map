...
window.startWatchdog = function() {
  ...
  loadingWatchdog = setTimeout(() => {
    ...
    // ⚡ ΑΥΤΟΜΑΤΟ ΗΛΕΚΤΡΟΣΟΚ ΣΤΟ ΠΑΡΑΣΚΗΝΙΟ (Χωρίς να πατήσει κάτι ο χρήστης!)
    if (typeof firebase !== 'undefined' && firebase.database) {
        console.log("Αυτόματο κλείσιμο και άνοιγμα σύνδεσης...");
        firebase.database().goOffline();
        setTimeout(() => firebase.database().goOnline(), 500);
    }
...
window.forceRetryConnection = function() {
  ...
  // Κρίσιμο: "Σκοτώνουμε" τις παλιές "κολλημένες" συνδέσεις για να ξεμπουκώσουν τα WebSockets
    ...
    // ⚡ ΜΑΓΙΚΟ ΚΟΛΠΟ ΓΙΑ ΑΡΓΑ PC: Κλείνουμε και ξανανοίγουμε βίαια τη σύνδεση της Firebase!
    if (typeof firebase !== 'undefined' && firebase.database) {
        firebase.database().goOffline(); // Κόβουμε τη νεκρή γραμμή 🔌
        
        // Δίνουμε στο PC μισό δευτερόλεπτο να πάρει "ανάσα"
        setTimeout(() => {
            firebase.database().goOnline(); // Ανοίγουμε μια ολοκαίνουρια γραμμή! ⚡
        ...
        // Fallback (αν δεν φορτώσει η βιβλιοθήκη)
        if (typeof initAlbumSettings === "function") {
            initAlbumSettings();
        }
    }
...
const initAlbumSettings = () => {
 ...
 if (!data.themes || !data.themes[currentViewId]) {
       currentViewId = initialViewId; // Ασφαλής μετάβαση στο πιο πρόσφατο διαθέσιμο θέμα
   }
...
// 🛡️ ΣΟΒΑΡΗ ΔΙΟΡΘΩΣΗ 4: Ακαριαίο καθάρισμα του DOM. Αλλιώς βλέπει "φαντάσματα" του παλιού άλμπουμ όσο περιμένει!
  ...
  // ΔΙΟΡΘΩΣΗ: Εμφάνιση loading state όσο αναμένουμε τα νέα δεδομένα από τη Firebase
  const emptyState = document.getElementById("empty-state");
  if (emptyState) {
      if (!emptyState.dataset.originalHtml) emptyState.dataset.originalHtml = emptyState.innerHTML;
      emptyState.innerHTML = `<div style="text-align: center; padding: 40px; color: #666;"><span style="font-size: 40px; animation: pulseWatchdog 1.5s infinite ease-in-out;">⏳</span><br><br>Οι αναμνήσεις φορτώνουν...</div>`;
      emptyState.style.display = "block";
  }
  
  const btnLoadMore = document.getElementById("btn-load-more");
  if (btnLoadMore) btnLoadMore.style.display = "none"; // Αποτροπή κλικ κατά την εναλλαγή θέματος
...
// 🚀 Το απόλυτο Query: Φέρνει ΜΟΝΟ όσες ανήκουν στο άλμπουμ ΚΑΙ είναι εγκεκριμένες (90% οικονομία δεδομένων)!
  activeQuery = dbPub.ref("album_photos").orderByChild("theme_status").equalTo(currentViewId + "_approved");

  activeQuery.on("value", snap => {
      let allFetched = [];
      snap.forEach(child => {
          let photoData = child.val(); photoData.id = child.key;
          allFetched.push(photoData); // Έφυγε το if, γιατί το Firebase μας έφερε ΗΔΗ τα σωστά!
      });
...
let lb = document.getElementById('album-dynamic-lightbox');
 if (lb && lb.style.display === 'flex' && window.albumPhotosList && window.albumPhotosList[currentLightboxIndex]) {
     ...
     window.albumPhotosList = photos; // Σωστό: Μόνο όσες είναι ορατές στο grid
 ...
 photos.forEach((photo, index) => {
    if (!photo || !photo.url) return; // Προστασία από κατεστραμμένες εγγραφές χωρίς URL
...
if (photos.length > 0) {
    ...
    if (btnLoadMore) btnLoadMore.style.display = hasMore ? "block" : "none";
  } else {
    if (grid) grid.style.display = "none";
    if (emptyState) {
...
// Αν έχει πιάσει το όριο τοπικά, προσπερνάμε τη χρονοβόρα κλήση στη βάση για να μην "παγώσει" η οθόνη
...
// ΠΡΟΣΘΗΚΗ: Καθολική μεταβλητή κλειδώματος
window.isUploadingPhoto = false;

window.uploadImage = async function(event) {
  // Κλείδωμα για προστασία από πολλαπλά Drag & Drop ή γρήγορα κλικ
  if (window.isUploadingPhoto) return;
  window.isUploadingPhoto = true;

  const file = event.target.files[0];
  if(!file) { window.isUploadingPhoto = false; return; }
  ...
  if (!isAllowed) {
    ...
    window.isUploadingPhoto = false; // Ξεκλείδωμα
    return;
  }
  if (!isAllowed) {
    document.getElementById('file-upload').value = '';
  ...
  if(!file.type.startsWith('image/')) {
    ...
    document.getElementById('file-upload').value = '';
    window.isUploadingPhoto = false; // <--- ΠΡΟΣΘΗΚΗ ΕΔΩ
    return;
  }
...
// 🛡️ ΣΟΒΑΡΗ ΔΙΟΡΘΩΣΗ 8: Πρόληψη μόνιμου "παγώματος" σε παλιά iPhone που δεν υποστηρίζουν createImageBitmap
 ...
 // Το createImageBitmap σέβεται αυτόματα την περιστροφή EXIF της κάμερας!
  ...
  canvas.toBlob(blob => {
        if (!blob) {
            Swal.fire({ icon: 'error', title: 'Σφάλμα Μνήμης', text: 'Η εικόνα είναι πολύ βαριά. Δοκιμάστε άλλη.', confirmButtonColor: '#e74c3c' });
          window.isUploadingPhoto = false; // <--- ΠΡΟΣΘΗΚΗ ΕΔΩ
            return;
        }
        ...
        .then(data => {
         ...
         const firebasePushPromise = dbPub.ref("album_photos").push({
              ...
              status: "pending",
              theme_status: activeThemeId + "_pending", // <--- ΝΕΟ ΠΕΔΙΟ
              timestamp: Date.now()
            });
...
window.openAlbumLightbox = function(index) {
    ...
    lb.style.cssText = "position: fixed !important; top: 0 !important; left: 0 !important; width: 100% !important; height: 100% !important; background: rgba(0,0,0,0.92) !important; backdrop-filter: blur(8px) !important; -webkit-backdrop-filter: blur(8px) !important; z-index: 2147483647 !important; display: none; justify-content: center !important; align-items: center !important; flex-direction: column; opacity: 0; transition: opacity 0.3s ease; touch-action: pinch-zoom !important; overscroll-behavior: none !important;";
        lb.innerHTML = `
 ...
 lb.addEventListener('touchend', e => {
           // ΠΡΟΣΤΑΣΙΑ: Ακύρωση του Swipe αν η οθόνη είναι μεγεθυμένη (Zoom), για να περιηγείται ο χρήστης ελεύθερα
           if (window.visualViewport && window.visualViewport.scale > 1.01) return;

           // ΔΙΟΡΘΩΣΗ: Ακύρωση του Swipe αν ο χρήστης δεν έχει σηκώσει ΑΚΟΜΑ όλα του τα δάχτυλα από το Zoom (Pinch release)
...
window.changeLightboxImage = function(step, event) {
    ...
    if(window.albumPhotosList.length <= 1) return;
...
window.updateLightboxContent = function() {
    document.querySelectorAll('.lightbox-arrow1').forEach(arrow => {
        arrow.style.display = (window.albumPhotosList && window.albumPhotosList.length > 1) ? 'block' : 'none';
    });
...
const initDragAndDrop = () => {
  ...
  if (albumBox && dragOverlay) {
   ...
   let dragCounter = 0;
    ...
    albumBox.addEventListener('dragenter', () => {
      dragCounter++;
      dragOverlay.classList.add('active');
    });
    
    albumBox.addEventListener('dragover', () => dragOverlay.classList.add('active'));
...
// Έλεγχος αν το DOM είναι ήδη έτοιμο, αλλιώς περιμένουμε (Συμβατότητα με defer/async scripts)
if (document.readyState === "loading") {
