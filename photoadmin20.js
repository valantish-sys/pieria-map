...
function fetchPhotosWithLimit() {
  ...
  photosListenerRef.on("value", snap => {
    ...
    // ΔΙΟΡΘΩΣΗ: Εξασφαλίζουμε ότι φορτώνονται ΠΑΝΤΑ ΟΛΕΣ οι εκκρεμότητες, ανεξάρτητα από το χρονικό όριο!
    db.ref("album_photos").orderByChild("status").equalTo("pending").once("value", pendingSnap => {
      let pendingMap = {};
      if (pendingSnap.exists()) pendingSnap.forEach(c => { pendingMap[c.key] = c.val(); });
      ...
      allPhotos.sort((a, b) => b.key.localeCompare(a.key)); // Ταξινόμηση χρονολογικά
 ...
 let statPending = document.getElementById('stat-pending-dash');
  ...
  if (adminLb && adminLb.style.display === 'flex' && adminLbPhotos.length > 0) {
      ...
      if (newIndex !== -1) {
          ...
          if (adminLbPhotos.length > 0) { adminLbIndex = 0; renderAdminLightbox(); }
