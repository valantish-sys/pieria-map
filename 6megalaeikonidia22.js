  ...
  const handleShare = async (e) => {
    ...
    try {
     ...
     const textarea = document.createElement('textarea');
      ...
      textarea.select();
      textarea.setSelectionRange(0, 99999); // ΔΙΟΡΘΩΣΗ 8: Απολύτως απαραίτητο κόλπο για να επιτρέψει την αντιγραφή το iOS
      try {
...
// --- Λειτουργία Text-to-Speech (Ανάγνωση Άρθρου) ---
  ...
  safeBind(DOM.aTts, 'click', () => {
    if (!speech) { alert('Η λειτουργία δεν υποστηρίζεται σε αυτή την εφαρμογή/περιηγητή.'); return; }
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          if (e && e.type === 'scroll') {
            ...
            // Εμφανίζεται ΜΟΝΟ αν ΔΕΝ το έχουμε κρύψει επειδή σκρολάραμε προς τα κάτω
  ...
  const initDraggableDial = () => {
   ...
   mainBtn.addEventListener('touchend', () => {
     ...
     // Υπολογίζουμε το πραγματικό bottom με βάση το ΣΤΑΘΕΡΟ screenH (όχι αυτό μετά από 400ms)
      ...
      setTimeout(() => { isDragging = false; hasMoved = false; }, 150);
...
// ==========================================
 ...
 // ΔΙΟΡΘΩΣΗ 6: Μεταβλητή μνήμης έξω από τη συνάρτηση (θυμάται την τελευταία λέξη)
  ...
  const handleSelection = () => {
    ...
    // ΔΙΟΡΘΩΣΗ: Αν ο χρήστης γράφει ή μαρκάρει κείμενο σε input/textarea, μην πετάς το λεξικό
    ...
    if (word.length >= 3 && word.length <= 30 && word.split(/\s+/).length <= 2) {
      if (selection.rangeCount === 0) return; // <-- ΠΡΟΣΘΗΚΗ
...
// --- Εξωτερικό Τηλεκοντρόλ Παύσης Κινήσεων (Με ενσωματωμένο "Κοριό" / Monkey Patch) ---
  ...
  if (iframe.dataset.url !== url) {
    ...
    iframe.contentWindow ? iframe.contentWindow.location.replace(url) : iframe.src = url;
}
   ...
   let hidePreviewTimer; // ΠΡΟΣΘΗΚΗ: Χρονόμετρο απόκρυψης
    ...
    iframe.dataset.url = ''; // <-- Προσθήκη
  iframe.dataset.url = ''; // <-- Προσθήκη
iframe.contentWindow ? iframe.contentWindow.location.replace('about:blank') : iframe.src = 'about:blank';
}
        }, 300);
      }, 150);
    });
