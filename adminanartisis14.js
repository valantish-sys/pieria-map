        ...
        imgHtml = `
          <div style="margin-bottom: 15px; text-align: center;">
            ...
            <div style="margin-top: 5px;">
              <button class="btn btn-danger" style="font-size:11px; padding:4px;" onclick="window.removePostVideo('${p.id}')">🗑️ Αφαίρεση Βίντεο</button>
            </div>
          </div>
        `;
      }
          ...
          Swal.fire({
             ...
             preConfirm: () => {
                  ...
                  if (!newTitle || !newContent) {
                      Swal.showValidationMessage('Ο Τίτλος και το Κείμενο είναι υποχρεωτικά!');
                      return false;
                  }
                  ...
                  return {
                      ...
                      content: escapeHTML(newContent).replace(/\n/g, '<br>'),
...
document.getElementById('rotate-blur-btn').style.display = 'inline-block';
  ...
  function handleMove(e) {
    ...
    let cx = startX + w / 2, cy = startY + h / 2;
      let rx = Math.max(0.1, Math.abs(w / 2)), ry = Math.max(0.1, Math.abs(h / 2));
  ...
  window.savePostBlurredImage = () => {
   ...
   .then(data => {
          if(data.secure_url) {
              const optimizedUrl = data.secure_url.replace('/upload/', '/upload/c_limit,w_1600,f_auto,q_auto/');
              db.ref('parent_posts/' + currentPostIdBlur).update({ imageUrl: optimizedUrl }).then(() => {
...
window.removePostVideo = (id) => {
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            title: 'Αφαίρεση Βίντεο;', text: 'Το κείμενο θα παραμείνει.', icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#e74c3c', confirmButtonText: 'Ναι, διαγραφή', cancelButtonText: 'Ακύρωση'
        }).then((result) => {
            if (result.isConfirmed) db.ref('parent_posts/' + id).update({ youtubeUrl: null });
        });
    } else {
        if(confirm('Αφαίρεση Βίντεο;')) db.ref('parent_posts/' + id).update({ youtubeUrl: null });
    }
};
...
window.removePostImage = (id) => {
 ...
 window.insertReadMore = () => {
   ...
   const text = textarea.value;
    const insertTxt = "\n[READ_MORE]\n";
    textarea.value = text.substring(0, start) + insertTxt + text.substring(end);
    textarea.selectionStart = textarea.selectionEnd = start + insertTxt.length;
    textarea.focus();
  };
  ...
  let img = post.imageUrl ? `<div style="text-align:center; margin-bottom:20px;"><img src="${post.imageUrl}" onload="window.print()" onerror="window.print()" style="max-width:100%; max-height:400px; border-radius:8px;"></div>` : '';
  ...
  const printWin = window.open('', '_blank');
    if (!printWin) {
        if (typeof Swal !== 'undefined') Swal.fire('Σφάλμα', 'Τα αναδυόμενα παράθυρα (pop-ups) είναι μπλοκαρισμένα. Παρακαλώ επιτρέψτε τα στον browser σας.', 'error');
        else alert('Τα pop-ups είναι μπλοκαρισμένα.');
        return;
    }
   ...
   printWin.document.close();
    if (!post.imageUrl) { setTimeout(() => { printWin.print(); }, 500); }
  };
  ...
  window.downloadImg = (url, id) => {
    ...
    .then(blob => {
        ...
        link.download = 'Image_' + id + '.' + ext;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    }).catch(() => {
        if (typeof Swal !== 'undefined') {
            Swal.fire({ title: 'Αποτυχία Λήψης', html: `Το δίκτυο εμπόδισε την αυτόματη λήψη. <a href="${url}" target="_blank">Πατήστε εδώ</a> για προβολή.`, icon: 'warning' });
        } else {
            prompt("Αντιγράψτε το URL για να δείτε την εικόνα:", url);
        }
      });
  };
 ...
 function escapeHTML(str) {
  ...
  escaped = escaped.replace(/&lt;\/?(div|p)[\s\/]*&gt;/gi, '<br>');
  escaped = escaped.replace(/&lt;(\/?)(b|i|u|strong|em|ul|li|br)(\s*\/?)&gt;/gi, '<$1$2$3>');
