...
// 5. Όταν το ποντίκι φεύγει από το link
 ...
 // Αν το ποντίκι φύγει εντελώς από το συννεφάκι, κλείστο
  ...
  // 2. Σκανάρουμε για Επικεφαλίδες (Μόνο H2 και H3)
   ...
   if (!heading.id) {
        // Δημιουργία μόνιμου ID βάσει του κειμένου του τίτλου (ώστε να μην αλλοιώνεται αν προστεθούν νέοι τίτλοι)
        const safeName = heading.textContent.trim().toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/^-|-$/g, '');
