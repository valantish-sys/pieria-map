  ...
  onValue(ref(db, 'answers'), (snapshot) => {
      try {
         ...
         masterGroups.sort((a, b) => b.latestTime - a.latestTime);
          ...
          // ΔΕΝ μηδενίζουμε το currentVisibleCount, ώστε να παραμένουν ανοιχτές 
          // οι κάρτες που έχει ήδη φορτώσει ο χρήστης με το "Load More".
          
          // Ελέγχουμε αν ο χρήστης έχει ενεργή αναζήτηση τη στιγμή της ενημέρωσης
          const searchInputEl = document.getElementById('search-input');
          const term = searchInputEl ? searchInputEl.value.toLowerCase().trim() : "";
          
          if (term === "") {
              renderGrid(masterGroups, false); 
          } else {
              // Διατηρούμε το φιλτράρισμα ώστε να μην του χαλάσουμε την αναζήτηση
 ...
 // Βοηθητική συνάρτηση που αφαιρεί αυτόματα τους τόνους
  ...
  if(searchInput) {
      searchInput.addEventListener('input', (e) => {
          ...
          if (term === "") {
              ...
              const filtered = masterGroups.filter(g => removeAccents(g.qData.text.toLowerCase()).includes(term));
              renderGrid(filtered, true);
          }
      });
  }
 ...
 window.goBack = () => {
      ...
      // Ομαλή μετακίνηση πίσω στην κορυφή του τοίχου με τις κάρτες
      viewGrid.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
