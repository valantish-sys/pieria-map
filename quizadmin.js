 import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-app.js";
  ...
  function renderAll() {
     ...
     // Έξυπνη ταξινόμηση: Πρώτα συγκρίνει το Σχολικό Έτος και μετά την Ημέρα
      daysArr.sort((a, b) => {
          const yearA = a.id.includes('schoolYear_') ? parseInt(a.id.split('schoolYear_')[1]) : 0;
          const yearB = b.id.includes('schoolYear_') ? parseInt(b.id.split('schoolYear_')[1]) : 0;
      ...
      daysArr.forEach(dayObj => {
         ...
         // ΝΕΟ: Κόβει σωστά τον αριθμό ακόμα κι αν το ID είναι 'year_2026_day_250'
          const dayNum = parseInt(dayId.split('day_')[1]);
          const qText = QUESTIONS_DB[dayNum % QUESTIONS_DB.length].text;
          ...
          ansArr.forEach(ans => {
              ...
              const cardBase = `
                  ...
                  <div class="card-text" id="txt-${ans.uid}">${safeAns}</div>
                  
                  <div class="edit-box" id="edit-${ans.uid}">
                      <textarea class="edit-area" id="input-${ans.uid}">${safeAns}</textarea>
                      <div id="err-${ans.uid}" class="edit-error">Το κείμενο δεν μπορεί να είναι κενό!</div>
                      <button class="btn btn-dark" onclick="saveEdit('${dayId}', '${ans.uid}')">💾 Αποθήκευση & Έγκριση</button>
                      <button class="btn" style="background:#95a5a6;" onclick="toggleEdit('${ans.uid}')">Ακύρωση</button>
                  </div>
              `;
              ...
              if(ans.status === 'pending') {
                  ...
                  htmlPen += `
                      <div class="ans-card pending">
                          ${cardBase}
                          <div class="actions-row" id="act-${ans.uid}">
                              <button class="btn btn-green" onclick="chStatus('${dayId}', '${ans.uid}', 'approved')">✔️ Έγκριση</button>
                              <button class="btn btn-blue" onclick="toggleEdit('${ans.uid}')">✏️ Επεξεργασία</button>
                  ...
                  htmlTra += `
                      <div class="ans-card trashed">
                          ...
                          <div class="actions-row" id="act-${ans.uid}">
                              ...
                              <button class="btn btn-red" id="delbtn-${ans.uid}" style="background:#000;" onclick="delForever('${dayId}', '${ans.uid}')">❌ Οριστική Διαγραφή</button>
                          </div>
                      </div>`;
              }
          });
...
// ΝΕΟ: Εμφάνιση/Απόκρυψη κουμπιών μαζικών ενεργειών αν υπάρχουν στοιχεία
  ...
  // ΝΕΟ: Έξυπνη διαγραφή με διπλό κλικ αντί για confirm() που μπλοκάρεται στο Sites
  window.delForever = (day, uid) => {
      const btn = document.getElementById(`delbtn-${uid}`);
      if(btn.innerText.includes("Σίγουρα;")) {
          ...
          setTimeout(() => { 
              if(document.getElementById(`delbtn-${uid}`)) { 
  ...
  window.toggleEdit = (uid) => {
      const eBox = document.getElementById(`edit-${uid}`);
      const tBox = document.getElementById(`txt-${uid}`);
      const aBox = document.getElementById(`act-${uid}`);
      const errBox = document.getElementById(`err-${uid}`);
      if(eBox.style.display === 'block') {
          eBox.style.display = 'none'; tBox.style.display = 'block'; aBox.style.display = 'flex';
          if(errBox) errBox.style.display = 'none';
      } else {
          eBox.style.display = 'block'; tBox.style.display = 'none'; aBox.style.display = 'none';
      }
  };

  window.saveEdit = (day, uid) => {
      const newTxt = document.getElementById(`input-${uid}`).value.trim();
      const errBox = document.getElementById(`err-${uid}`);
      if(!newTxt) {
          if(errBox) errBox.style.display = 'block'; // Εμφανίζει το μήνυμα αντί για alert
          return;
      }
      if(errBox) errBox.style.display = 'none';
