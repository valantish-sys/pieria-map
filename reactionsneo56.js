        ...
        init(mediaNodes) {
           ...
           let thumbSrc = 'https://via.placeholder.com/150x100/333333/FFFFFF?text=VIDEO';
                    ...
                    if (ytMatch && ytMatch[1]) {
                        ...
                        vidSrc = `https://www.youtube.com/embed/${ytMatch[1]}`;
                    }
         ...
         try {
          ...
          // Επανεκτέλεση των Scripts ώστε να δουλεύουν τα ενσωματωμένα Tweets & Instagram Posts
            ...
            const executeScriptsSequentially = async () => {
                ...
                // ΔΙΟΡΘΩΣΗ: Προσθήκη του await για να περιμένει πρώτα να κατέβουν τα scripts
           ...
           // Social Plugins & Embeds (Προσθήκη υποστήριξης για δυναμικά Tweets & Instagram)
                ...
                // ΔΙΟΡΘΩΣΗ: Αφαιρούμε το loading ώστε να ξεκλειδώσει το κουμπί σε περίπτωση που 
                // ο χρήστης επιστρέψει στη σελίδα με το πλήκτρο "Πίσω" (BFCache).
...
static processFetchedContent(fetchedBody, summaryWrap) {
      ...
      static close(post, btn, isPopState = false) {
         ...
         // ΔΙΟΡΘΩΣΗ: Στοχεύουμε ΟΛΑ τα iframes για να κλείνουμε με απόλυτη ασφάλεια τον ήχο
         // και από σχολικά εργαλεία (Google Drive, Canva, Facebook, Genially κ.ά.)
         wrapper.querySelectorAll('iframe').forEach(ifr => {
