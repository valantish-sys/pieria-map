(() => {
    ...
    function initWidget(prefix) {
       ...
       DOM.form?.addEventListener("submit", (e) => {
                ...
                // ΔΙΟΡΘΩΣΗ: Πραγματική αποστολή των δεδομένων στο παρασκήνιο για να μη χάνεται το email!
                if (DOM.form && DOM.form.action) {
                    fetch(DOM.form.action, {
                        method: DOM.form.method || 'POST',
                        body: new FormData(DOM.form),
                        mode: 'no-cors' // Απαραίτητο αν η φόρμα στέλνει π.χ. στο Blogger, Mailchimp κλπ
                    }).catch(() => {}); // Αν αποτύχει, το αγνοούμε για να συνεχίσει το παιχνίδι
                }
    ...
    checkReveal: () => {
      ...
      // --- Ανάκτηση δεδομένων Blogger API σε ΔΟΣΕΙΣ ---
        fetchPost: () => {
           ...
           // 1. Ασφαλής συνάρτηση που φέρνει ΜΙΑ φορά τα άρθρα, αποτρέποντας το άπειρο Loop.
            const fetchBatch = () => {
                fetch(`/feeds/posts/default?q=δράσεις&alt=json&max-results=250`)
                    .then(r => r.json())
                    .then(data => {
                ...
                if (actionPosts.length > 0) {
                  ...
                  let imgUrl = "";
                    if (randomPost.media$thumbnail?.url) {
                        // ΔΙΟΡΘΩΣΗ: Πιο ισχυρό Regex που «πιάνει» και τις νέες παραμέτρους εικόνων του Blogger
                        imgUrl = randomPost.media$thumbnail.url.replace(/\/s[0-9]+[^\/]*\//, "/s600/");
