(() => {
  ...
  const CONFIG = Object.freeze({
    schedule: [
     ...
     { start: "11:45", end: "12:25", name: "5η Διδακτική", type: "class", nextIsBreak: true }, // [FIX]
...
if (DOM.minimap) DOM.minimap.style.display = "flex";
      ...
      // --- ΝΕΟ: Δευτερόλεπτα το πρωί! ---
        if (diffSecs <= 60) {
            ...
            DOM.subEl.innerHTML = `Το κουδούνι χτυπάει σε 1 λεπτό.`;
        } else {
     ...
     AppManager.initGyro();
      ...
      setTimeout(() => {
         ...
         // --- ΝΕΟ: Εφέ Γραφομηχανής (Typewriter) ---
          ...
          let charIndex = 0;
          const msgChars = Array.from(targetMessage); // Ασφαλής διαχωρισμός των Emojis!
