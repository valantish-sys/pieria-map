const CONFIG = Object.freeze({
    ...
    feedLabelsUrl: "/feeds/posts/default/-/" + encodeURIComponent("Διαπαιδαγώγηση|Ψυχολογία|Σχολείο|Υγεία|Παιχνίδι|Γενικά") + "?alt=json&max-results=50",
      ...
      try {
        ...
        if (imgMatch && imgMatch[1]) {
          ...
          if (imageUrl.includes("blogger.googleusercontent.com") || imageUrl.includes("bp.blogspot.com")) {
            imageUrl = imageUrl.replace(/\/s[0-9]+(-b|-c|-w)?\//, '/s800/').replace(/=w[0-9]+-h[0-9]+(-c)?/, '=s800');
          }
  ...
  const ApiManager = {
    fetchData: async () => {
     try {
        ...
        if (labelsRes) ApiManager.processWeeklyPick(labelsRes);
  ...
  const SliderManager = {
    buildWidget: (suffix) => {
      ...
      STATE.sliderPosts.forEach((post, index) => {
        ...
        const loadingAttr = index === 0 ? 'fetchpriority="high"' : '';
      ...
      // --- ΞΕΧΩΡΙΣΤΟΣ "ΕΓΚΕΦΑΛΟΣ" ΓΙΑ ΚΑΘΕ SLIDER (Κινητό / PC) ---
      const localState = {
        currentIndex: 0,
        autoSlideTimer: null,
        isAnimating: false,
      ...
      const resetAutoSlide = () => {
        clearInterval(localState.autoSlideTimer);
        if (STATE.sliderPosts.length > 1) startAutoSlide();
      };

      if (STATE.sliderPosts.length > 1) {
        if (arrowPrev) {
          arrowPrev.classList.remove('hidden-arrow');
          arrowPrev.addEventListener("click", () => moveSlide(-1));
        }
        if (arrowNext) {
          arrowNext.classList.remove('hidden-arrow');
          arrowNext.addEventListener("click", () => moveSlide(1));
        }
        
        startAutoSlide();

        wrapper.addEventListener("mouseenter", () => clearInterval(localState.autoSlideTimer), { passive: true });
        wrapper.addEventListener("mouseleave", resetAutoSlide, { passive: true });
        ...
        wrapper.addEventListener("touchend", (e) => {
          const diff = localState.touchStartX - e.changedTouches[0].screenX;
          if (diff > 40) moveSlide(1);    
          else if (diff < -40) moveSlide(-1); 
          resetAutoSlide();
        }, { passive: true });
      } else {
        if (arrowPrev) arrowPrev.classList.add('hidden-arrow');
