...
let authPromise = new Promise((resolve) => {
  // 1. Βαλβίδα ασφαλείας: Αν το Firebase μπλοκαριστεί ή κολλήσει από Edge/Incognito, 
  // σε 2 δευτερόλεπτα του δίνουμε ένα προσωρινό ID (Guest) για να προχωρήσει!
  let fallbackTimer = setTimeout(() => {
    if (!currentUserUid) {
      currentUserUid = "guest_" + Date.now();
      resolve(currentUserUid);
    }
  }, 2000);

  // 2. Προσπάθεια κανονικής σύνδεσης (Χωρίς persistent αποθήκευση αν χρειαστεί)
  authPub.setPersistence(firebase.auth.Auth.Persistence.LOCAL)
    ...
    .then(() => {
      authPub.onAuthStateChanged(user => {
        if (user) {
          clearTimeout(fallbackTimer); // Ακυρώνουμε τον Guest, βρέθηκε κανονικό ID
          currentUserUid = user.uid;
          resolve(user.uid);
        } else {
          authPub.signInAnonymously().catch(err => {
            // Αν φάμε οριστικό μπλοκ, γινόμαστε Guest!
...
window.loadPhotos = function(isLoadMore = false) {
  ...
  // 1. ΔΙΟΡΘΩΣΗ: Αφαιρούμε το limitToLast(). 
  // Φέρνουμε όλες τις φώτο του θέματος και τοπικά φιλτράρουμε τις "approved". Έτσι δεν χάνουμε καμία!
 // 1. ΔΙΟΡΘΩΣΗ ΑΣΦΑΛΕΙΑΣ: Ζητάμε από τον Server ΑΥΣΤΗΡΑ ΚΑΙ ΜΟΝΟ τις εγκεκριμένες φωτογραφίες!
  activeQuery = dbPub.ref("album_photos")
    .orderByChild("status")
    .equalTo("approved");

  // 2. ΠΡΑΓΜΑΤΙΚΟ LIVE SYNC: Με το .on("value") ακούμε ταυτόχρονα για Νέες, Εγκρίσεις & Διαγραφές!
  activeQuery.on("value", snap => {
      let allFetched = [];
      snap.forEach(child => {
        // Επειδή φέραμε τις "approved" όλου του άλμπουμ, κρατάμε τοπικά ΜΟΝΟ αυτές του επιλεγμένου θέματος:
      ...
      // Τοπικό Pagination: Κόβουμε όσες πρέπει να φανούν με βάση το currentDbLimit
      ...
      // Διαγραφή από την οθόνη όσων διεγράφησαν από τον admin ή βγήκαν εκτός ορίου
      Array.from(grid.children).forEach(el => {
          if (!newDbIds.includes(el.dataset.id)) el.remove();
      });

      // Προσθήκη ή ενημέρωση των φωτογραφιών
      photos.forEach((photo, index) => {
        ...
        if (existingEl) {
          ...
          // Αν κάποια παλιά φώτο εγκρίθηκε αργότερα, μπαίνει Live εμβόλιμα στη σωστή θέση!
          if (grid.children[index] !== existingEl) {
              grid.insertBefore(existingEl, grid.children[index]);
          }
        } else {
          let optimizedUrl = photo.url.replace('/upload/', '/upload/q_auto,f_auto,w_600/');
          let captionHtml = photo.caption ? `<div class="polaroid-caption">${escapeHTML(photo.caption)}</div>` : "";
...
window.canUserUpload = async function() {
  if (!currentUserUid) {
    Swal.fire({ 
      ...
      text: 'Γίνεται προετοιμασία, παρακαλώ περιμένετε 1-2 δευτερόλεπτα...', 
      allowOutsideClick: false,
      showConfirmButton: false,
      didOpen: () => { Swal.showLoading(); } 
    });
    
    // Πλέον εδώ δεν υπάρχει περίπτωση να πετάξει σφάλμα! 
    // Είτε θα βρει Firebase κανονικά, είτε θα μπει ως Guest.
    await authPromise;
    Swal.close();
  }
  ...
  try {
    ...
    // Αν κοπεί η σύνδεση με τη βάση λόγω υπερβολικού privacy (πχ Edge Strict),
    // απλά αγνοούμε το όριο και τον αφήνουμε να ανεβάσει!
    console.warn("Αγνοήθηκε ο έλεγχος ορίου (Guest Mode).", e);
  }
 ...
 // Το createImageBitmap σέβεται αυτόματα την περιστροφή EXIF της κάμερας!
  window.createImageBitmap(file).then(img => {
     ...
     canvas.toBlob(blob => {
        ...
        // ΣΗΜΑΝΤΙΚΗ ΔΙΟΡΘΩΣΗ: Προσθέτουμε τη λέξη "photo.webp" (Δίνουμε όνομα στο αρχείο)
        formData.append("file", blob, "photo.webp");
        formData.append("caption", userCaption || ""); 
        
        // ⚠️ ΒΑΛΕ ΕΔΩ ΤΟ ΠΡΑΓΜΑΤΙΚΟ URL ΑΠΟ ΤΟ CLOUDFLARE ΠΡΟΣΕΚΤΙΚΑ ⚠️
        const WORKER_URL = "https://BALE-TO-DIKO-SOU-LINK-EDW.workers.dev"; // Χωρίς / στο τέλος!
        ...
        .then(async response => {
          // Διαβάζουμε την απάντηση πρώτα ως κείμενο (ώστε να μην σκάσει αν δεν είναι JSON)
          ...
          try {
            ...
            console.error("Μη αναμενόμενη απάντηση Server:", text);
            throw new Error(response.status === 404 ? "Λάθος URL του Worker! Δες τον κώδικά σου." : "Σφάλμα του Cloudflare (όχι JSON)");
          }

          // Αν ο Worker επέστρεψε μήνυμα λάθους
          if (!response.ok) {
            throw new Error(data.error || "Άγνωστο εσωτερικό σφάλμα Worker");
          }
        ...
        .catch(err => {
          ...
          if (finalErrorMsg === "Failed to fetch") {
             finalErrorMsg = "Αδυναμία σύνδεσης: Είτε το URL του Worker είναι λάθος, είτε το μπλοκάρει κάποιο AdBlocker!";
          }

          // ΤΩΡΑ, το Popup θα μας λέει ΑΚΡΙΒΩΣ τι έφταιξε!
          Swal.fire({ 
            icon: 'error', 
            title: 'Αποτυχία ❌', 
            text: finalErrorMsg, 
...
window.openAlbumLightbox = function(index) {
    ...
    if (!lb) {
        ...
        lb.addEventListener('touchend', e => {
            ...
            if (touchStartX - touchEndX > 50) window.changeLightboxImage(1, e); 
            if (touchEndX - touchStartX > 50) window.changeLightboxImage(-1, e); 
        }, {passive: true});
