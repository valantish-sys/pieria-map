(() => {
    ...
    const UIEngine = {
        ...
        init: () => {
            ...
            // ΑΛΛΑΓΗ 1: Προσθήκη delay όταν το ποντίκι φεύγει από το tooltip
        ...
        closeTooltip: () => {
            UIEngine.currentHoveredFrame = null; // ΑΛΛΑΓΗ 2: Μηδενίζουμε τη μνήμη hover ΜΟΝΟ όταν κλείνει το tooltip
    ...
    const CalendarEngine = {
               ...
               // Εδώ πλέον ΜΟΝΟ ΖΩΓΡΑΦΙΖΟΥΜΕ (Δεν βάζουμε event listeners)
                dayCellDidMount: (info) => {
                    ...
                    if (DataEngine.postsByDate[cellDateStr]) {
                        ...
                        dot.style.pointerEvents = 'none'; // ΑΛΛΑΓΗ 5: Το ποντίκι αγνοεί το dot και βλέπει μόνο το κελί!
        ...
        setupEvents: () => {
            ...
            DOM.container.addEventListener('mouseover', (e) => {
                const frame = e.target.closest('.fc-daygrid-day-frame');
                if (frame) {
                    // ΑΛΛΑΓΗ 3: Όσο το ποντίκι είναι στο κελί, ακυρώνουμε ΠΑΝΤΑ το κλείσιμο
                    clearTimeout(UIEngine.hideTimeout); 
            ...
            DOM.container.addEventListener('mouseout', (e) => {
                ...
                // ΑΛΛΑΓΗ 4: Ελέγχουμε αν το ποντίκι πήγε πάνω στο ανοιχτό tooltip
                const toTooltip = DOM.tooltip && DOM.tooltip.contains(e.relatedTarget);

                if (frame && !frame.contains(e.relatedTarget) && !toTooltip) {
                    // Πλέον δεν κάνουμε currentHoveredFrame = null εδώ, το κάνει η closeTooltip
                    UIEngine.hideTimeout = setTimeout(UIEngine.closeTooltip, CONFIG.tooltipDelay);
                }
            });
