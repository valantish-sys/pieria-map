...
const mediaNodes = Array.from(wrapper.querySelectorAll('img, iframe'));
    ...
    class ArticleEngine {
        ...
        static processFetchedContent(fetchedBody, summaryWrap) {
          ...
          // Ταχύτερος και ασφαλέστερος αλγόριθμος (αποτρέπει τα παγώματα και αφαιρεί τα άδεια HTML κενά)
            let summaryLength = summaryWrap.textContent.replace(/\s+/g, '').length;
            ...
            while (walker.nextNode()) {
                ...
                let nodeCleanLength = node.nodeValue.replace(/\s+/g, '').length;
                
                if (removedLength + nodeCleanLength <= summaryLength + 5) {
                    nodesToRemove.push(node);
                    removedLength += nodeCleanLength;
                } else {
                    let charsToCut = summaryLength - removedLength;
                    if (charsToCut > 0) {
                        let originalText = node.nodeValue;
                        let cleanCount = 0;
                        for(let i = 0; i < originalText.length; i++) {
                            if (!/\s/.test(originalText[i])) cleanCount++;
                            if (cleanCount > charsToCut) {
                                node.nodeValue = originalText.substring(i);
                                break;
                            }
                        }
                    }
            ...
            nodesToRemove.forEach(n => { 
                ...
                n.nodeValue = ""; 
                // Αφαίρεση άδειων στοιχείων για αποφυγή μεγάλων λευκών κενών
                if (parent && parent.textContent.trim() === '' && !parent.querySelector('img, iframe, video')) {
                    parent.remove();
                }
            });
         ...
         // Προστασία Media (Αφαίρεση διπλότυπων)
            fetchedBody.querySelectorAll('img, iframe').forEach(m => {
                // Εξάγουμε το βασικό ID/Όνομα αρχείου αγνοώντας τις παραμέτρους μεγέθους (π.χ. /s72-c/, =s1600)
                const getFilename = (url) => { try { return new URL(url).pathname.split('/').pop().split('=')[0]; } catch(e) { return url; } };
                const isDuplicate = Array.from(summaryWrap.querySelectorAll(m.tagName)).some(sm => getFilename(sm.src) === getFilename(m.src));
