...
function fetchPhotosWithLimit() {
  ...
  // Προσθήκη του .limitToLast() για να εφαρμόζεται πραγματικά το όριο στο κατέβασμα των δεδομένων
...
let hasThemes = false;
  ...
  for (let key in allThemes) {
   ...
   let safeName = t.name ? t.name.replace(/</g, "&lt;").replace(/>/g, "&gt;") : "";
    ...
    let btns = '';
 if (!isCurrent) btns += `<button class="btn btn-success" style="padding: 8px 12px; margin-right: 5px;" onclick="setActiveTheme('${key}', '${t.name.replace(/'/g, "\\'").replace(/"/g, '&quot;')}')">⭐ Ορισμός Ενεργό</button>`;
  ...
  if (editMode[photo.key]) {
      captionHtml = `
        <div style="width:100%; display:flex; flex-direction:column;">
          <!-- Προσθήκη maxlength="150" για να συμβαδίζει με τα DB rules -->
          <textarea id="edit-input-${photo.key}" class="caption-input" rows="2" maxlength="150" placeholder="Γράψε λεζάντα...">${safeCaption...}</textarea>
...
if (photo.status === "pending") {
 ...
 if (existingCard) {
      ...
      // 3. Επαναφορά εστίασης ΑΥΣΤΗΡΑ ΜΕΤΑ το appendChild (το οποίο προκαλεί την απώλεια focus)
      if (isFocused) {
...
function saveCaption(id) {
  ...
  db.ref("album_photos/" + id + "/caption").set(val).then(() => {
    ...
    // Σύλληψη της απόρριψης (validation rule failure) του Firebase
    showToast("Σφάλμα: Η λεζάντα δεν μπορεί να ξεπερνά τους 150 χαρακτήρες!", true);
  });
}
...
if (result.isConfirmed) {
    db.ref("album_photos").orderByChild("status").equalTo("trashed").once("value", snap => {
      if (!snap.exists()) return; // Αποτροπή εκτέλεσης κλήσης χωρίς δεδομένα

      let updates = {};
      snap.forEach(child => { updates[child.key] = null; });
      db.ref("album_photos").update(updates);
    });
...
async function bulkAction(type, action) {
 ...
 // Ο καθαρισμός ΠΡΕΠΕΙ να γίνει ΠΡΙΝ την εκτέλεση του update.
...
function bulkMoveSelected() {
 ...
 // Στόχευση στη γενική κλάση `.cb-bulk` για να δουλεύει σε όλες τις καρτέλες
  ...
  // Καθαρισμός επιλογών ΠΡΙΝ το update (για αποφυγή φαντασμάτων)
  selectedPhotos.forEach(cb => cb.checked = false); 
  document.querySelectorAll('input[id^="selectAll-"]').forEach(el => el.checked = false);
  if (themeSelect) themeSelect.value = ""; 

 db.ref().update(updates).then(() => {
    // Διαγράφηκε η κλήση της ανύπαρκτης μεταβλητής (ο καθαρισμός έχει ήδη γίνει πιο πάνω)
...
function toggleBlurMode(key) {
 ...
 blurCanvas = document.getElementById('blur-canvas');
    ...
    let actionsDiv = document.getElementById('admin-panel-lightbox-actions');
    if(actionsDiv) actionsDiv.innerHTML = `<span style="color:white; font-size:16px;">⏳ Φόρτωση εργαλείου...</span>`;
    if(!blurCanvas) return;
    
    blurCanvas.style.display = 'block';
    blurCtx = blurCanvas.getContext('2d');
    
    // Καθαρισμός του καμβά για αποφυγή εμφάνισης της προηγούμενης εικόνας (ghosting effect)
...
imgObj.onload = function() {
        // Κρύβουμε την εικόνα ΜΟΝΟ όταν φορτώσει η νέα στον καμβά!
        let imgEl = document.getElementById('admin-panel-lightbox-img');
