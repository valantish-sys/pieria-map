(() => {
   ...
   const CONFIG = Object.freeze({
                ...
                try {
                    ...
                    if (data.feed?.entry && data.feed.entry.length > 0) {
                     ...
                     const totalResults = parseInt(data.feed.openSearch$totalResults?.$t || 0, 10);
                       // ΔΙΟΡΘΩΣΗ: Το Blogger API δεν επιστρέφει πάντα 150 αποτελέσματα (π.χ. επιστρέφει 147) αν υπάρχουν πρόχειρα/διαγραμμένα.
                       // Το "length < maxResults" προκαλεί λανθασμένο τερματισμό και τεράστια απώλεια άρθρων!
    ...
    const UIEngine = {
      ...
      init: () => {
            // ΔΙΟΡΘΩΣΗ: Σε AJAX περιβάλλον, διαγράφουμε πλήρως τα παλιά DOM elements και καθαρίζουμε τα 
            // Global Listeners, ώστε να μη δημιουργούνται ζόμπι events (Memory Leak) που νεκρώνουν το Modal.
          ...
          UIEngine.overlay.addEventListener('touchmove', (e) => {
                // Εμποδίζει το scroll ΜΟΝΟ αν το παράθυρο είναι ενεργό. Αν βρίσκεται σε φάση κλεισίματος (fade-out), το επιτρέπει!
            ...
            // Επαναχρησιμοποίηση named functions στο window, για να μην στοιβάζονται ανώνυμα events 
             ...
             self.els.yearOverlay = document.createElement('div');
                ...
                self.calendar = new window.FullCalendar.Calendar(self.els.calendarEl, {
                    ...
                    datesSet: (info) => {
                       ...
                       // ΔΙΟΡΘΩΣΗ: Επιβολή Ονομαστικής με Array για να μην τυπώνουν τα iPhone "Ιανουαρίου".
                ...
                const spinInterval = setInterval(() => {
                    ...
                    if (spins >= maxSpins) {
                       ...
                       self.els.calendarEl.classList.remove('roulette-blur');
                        ...
                        // ΔΙΟΡΘΩΣΗ: Αν η ρουλέτα κληρώσει τον ΙΔΙΟ μήνα που ήδη κοιτάμε, το datesSet δεν πυροδοτείται!
                        // Χωρίς αυτό, η ταμπέλα μένει κολλημένη στο τελευταίο ψεύτικο κείμενο της ρουλέτας.
