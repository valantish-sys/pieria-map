...
window.showLivePreview = () => {
    ...
    document.getElementById('modal-content').innerText = document.getElementById('p-content').value.trim() || '[Κενό Κείμενο]';
...
window.submitParentPost = async () => {
    ...
    try {
        if (selectedFile) {
            if (typeof Swal !== 'undefined') Swal.fire({ title: 'Επεξεργασία & Ανέβασμα...', allowOutsideClick: false, showConfirmButton: false, didOpen: () => Swal.showLoading() });
