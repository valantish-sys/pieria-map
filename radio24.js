    ...
    setupPulse: () => {
     ...
     if (!document.getElementById('pulse-css')) {
      ...
      const checkAndConnect = () => {
          // ΔΙΟΡΘΩΣΗ: Ελέγχουμε ΟΧΙ ΜΟΝΟ αν το firebase υπάρχει, αλλά αν έχει φτιάξει το app!
          const isFirebaseReady = typeof firebase !== 'undefined' && firebase.apps && firebase.apps.length > 0;
          
          if (typeof window.radioDb !== 'undefined' || isFirebaseReady) {
              connectFirebase();
        ...
        pipBtn.addEventListener('click', async () => {
          ...
          isPipOpening = true;
           try { 
               ...
               // ΔΙΟΡΘΩΣΗ: 1. Ανοίγουμε πρώτα το PiP
               await video.requestPictureInPicture(); 

               // ΔΙΟΡΘΩΣΗ: 2. Βάζουμε παύση ΜΕΤΑ το άνοιγμα
