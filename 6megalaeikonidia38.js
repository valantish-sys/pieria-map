...
// 3. Το κουμπί "Εκτύπωση" ΜΕΣΑ στο μενού εκτελεί την Εκτύπωση:
  safeBind('btn-do-print', 'click', () => {
    DOM.shareMenu?.classList.remove('show');
    setTimeout(() => window.print(), 150); // ΠΡΟΣΘΗΚΗ: Αναμονή 150ms για να προλάβει ο browser να κρύψει το μενού οπτικά!
  });
...
if (iframe.dataset.url !== url) {
    ...
    // ΝΕΟ: Χρήση location.replace για να μην παγιδεύεται το κουμπί "Πίσω" του Browser!
    if (iframe.contentWindow) {
