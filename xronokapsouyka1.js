(() => {
    ...
    const DataEngine = {
        fetchPosts: async () => {
            try {
                // 1. Διαλέγουμε μια τυχαία ετικέτα από αυτές που έχεις βάλει στο CONFIG
                ...
                // 2. Κάνουμε μια ελαφριά κλήση για να μάθουμε το ΣΥΝΟΛΟ των αναρτήσεων (max-results=1)
                const metaUrl = `/feeds/posts/default/-/${encodedLabel}?alt=json&max-results=1`;
                const metaRes = await fetch(metaUrl);
                const metaData = await metaRes.json();

                // Διαβάζουμε τον συνολικό αριθμό αναρτήσεων που βρέθηκαν
                const totalPosts = parseInt(metaData.feed.openSearch$totalResults.$t, 10);
                if (totalPosts === 0) throw new Error("Δεν βρέθηκαν αναρτήσεις για αυτή την ετικέτα.");

                // 3. Βρίσκουμε ένα εντελώς τυχαίο index από το 1 έως το totalPosts
                const randomIndex = Math.floor(Math.random() * totalPosts) + 1;

                // 4. Κάνουμε τη 2η κλήση, ζητώντας ΜΟΝΟ αυτή την τυχαία ανάρτηση (start-index)
                const finalUrl = `/feeds/posts/default/-/${encodedLabel}?alt=json&max-results=1&start-index=${randomIndex}`;
                const finalRes = await fetch(finalUrl);
                const finalData = await finalRes.json();

                const entry = finalData.feed.entry[0];
                DataEngine.processSingleEntry(entry);
