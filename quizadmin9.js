...
let QUESTIONS_DB = [];
 ...
 onAuthStateChanged(auth, user => {
      if (user && !user.isAnonymous) {
          ...
          isDataLoaded = false; // ΝΕΟ: Επαναφορά της μνήμης για να λειτουργήσει το re-login
...
window.switchTab = (event, tabId) => {
      // ΝΕΟ: Εφαρμογή της ασπίδας και στην αλλαγή καρτέλας!
...
daysArr.sort((a, b) => {
    ...
    const yearB = getYear(b.id);
      ...
      const qIndex = savedQIndex !== null ? savedQIndex : Math.max(0, dayNum - 1) % qLength; 
       // ΝΕΟ: Αποτροπή εμφάνισης του μηνύματος λάθους σε ΟΛΕΣ τις παλιές κάρτες αν το JSON κοπεί από firewall.
       const qText = (QUESTIONS_DB[qIndex] && QUESTIONS_DB.length > 1) ? QUESTIONS_DB[qIndex].text : "Άγνωστη Ερώτηση";
