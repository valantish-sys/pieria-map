 ...
 // ==========================================
  ...
  const NavManager = {
    init: () => {
      ...
      // --- ΝΕΟ: Η ΟΡΙΣΤΙΚΗ ΛΥΣΗ ΓΙΑ ΤΟ GHOST HOVER & FOCUS ---
      const allDropdowns = document.querySelectorAll('.dropdown-content, .sub-dropdown-content');
      
      const cleanMenus = (activeLi = null) => {
        allDropdowns.forEach(d => {
          // Αν δεν είμαστε πάνω στο μενού που ανήκει αυτό το dropdown...
          if (!activeLi || (!activeLi.contains(d) && !d.contains(activeLi))) {
            d.style.display = 'none'; // ...Το κρύβουμε ΜΟΝΙΜΑ (χωρίς 50ms)
          } else {
            d.style.display = ''; // Αφήνουμε το CSS να λειτουργήσει για το ενεργό μενού
          }
        });

        // Σκοτώνει το "Ghost Focus" αν ο χρήστης έκανε δεξί κλικ (άνοιγμα σε νέα καρτέλα)
      ...
      // 2. Όταν το ποντίκι βγαίνει ΕΝΤΕΛΩΣ από το μενού: Επαναφορά
      nav.addEventListener('mouseleave', () => {
        // Καθαρίζουμε τα inline styles για να είναι έτοιμα τα CSS hover την επόμενη φορά
        allDropdowns.forEach(d => d.style.display = '');
        if (document.activeElement && nav.contains(document.activeElement)) {
            document.activeElement.blur();
        }
      }, PASSIVE);

      // 3. Click (Αφή / Mobile ή κλικ έξω από το μενού)
      ...
      // 4. Αλλαγή Καρτέλας: Τα μαζεύει όλα αν ο χρήστης βγει εντελώς από το παράθυρο
  ...
  const ReactionsManager = {
   ...
   handleVote: (btn) => {
      ...
      // ΠΡΟΣΘΗΚΗ: Αποθηκεύουμε το νούμερο ΠΡΙΝ βάλουμε το ".."
      ...
      .catch((error) => {
        ...
        // ΔΙΟΡΘΩΣΗ: Επαναφέρει το αρχικό νούμερο αντί για "0"!
