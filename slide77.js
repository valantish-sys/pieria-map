...
const REGEX = Object.freeze({
   // ΔΙΟΡΘΩΣΗ: Προσθήκη του 'live\/' για υποστήριξη σχολικών ζωντανών μεταδόσεων
...
const NavManager = {
        ...
        if (!ticking) {
          window.requestAnimationFrame(() => {
           ...
           // Ενημέρωση θέσης (το <= 0 αποτρέπει λάθη με το bounce του Safari στα iPhone)
            ...
            // Ξεκλειδώνει το μενού όταν το scroll σταματήσει, για να λειτουργεί το ποντίκι
   ...
   index: 0,
    ...
    init: () => {
     ...
     LM.el.addEventListener('touchstart', e => {
        if (e.touches.length > 1 || e.target.closest('.lightbox-thumbnails')) return;
  ...
  LM.el.addEventListener('touchend', (e) => {
        // ΠΡΟΣΘΗΚΗ: Ο έλεγχος `e.touches.length > 0` εγγυάται ότι ένα Swipe θα υπολογιστεί
        // ΑΥΣΤΗΡΑ ΚΑΙ ΜΟΝΟ όταν έχουν σηκωθεί ΟΛΑ τα δάχτυλα από την οθόνη, προστατεύοντας το zoom.
        if (e.touches.length > 0 || (e.changedTouches && e.changedTouches.length > 1) || e.target.closest('.lightbox-thumbnails')) return;
        ...
        if (isSwipingDown && diffY > 120) {
          ...
          // BUG FIX: Μηδενίζουμε ΠΑΝΤΑ το background και το transform αν δεν έκλεισε, 
          // αλλιώς αν το diffY γίνει < 0, η εικόνα "κολλάει" εκτός κέντρου!
   ...
   close: () => {
      ...
      // ΠΡΟΣΘΗΚΗ: Καθυστέρηση 300ms στον μηδενισμό των inline styles 
      // ώστε να μείνει η εικόνα κάτω μέχρι να εξαφανιστεί πλήρως το lightbox.
      setTimeout(() => {
        LM.el.style.backgroundColor = '';
...
const regexCss = new RegExp(`#${oldId}(?=[\\s\\:\\,\\[\\{>~\\.\\+\\)]|$)`, 'g');
        ...
        if (!widgetHtml) {
          ...
          let textStartX = 0, textStartY = 0;
              ...
              descContainer.addEventListener('touchend', (e) => {
                ...
                // Αλλαγή σελίδας ΜΟΝΟ αν είναι σκόπιμο οριζόντιο swipe και όχι κάθετο scroll
               if (Math.abs(diffX) > 40 && Math.abs(diffX) > Math.abs(diffY)) {
                  const action = diffX > 0 ? "next-snippet" : "prev-snippet";
                  descContainer.querySelector(`[data-action="${action}"]`)?.click();
                  // ΔΙΑΓΡΑΦΗ: Αφαιρέθηκε το e.stopPropagation() για να μπορεί 
                  // το Slider να λάβει το touchend και να ξεπαγώσει!
                }
              }, PASSIVE);
            }
  ...
  SM.slides[SM.index].el.classList.remove("active");
      ...
      SM.slides[SM.index].el.setAttribute("inert", ""); 
    ...
    SM.index = newIndex;
      ...
      if (reactDiv) ReactionsManager.observer.observe(reactDiv);
  ...
  resume: (force = false) => {
      const SM = window.SliderManager;
      SM.pause();
      // Προσθήκη προστασίας για αποτροπή Fatal Crash:
      if (!SM.slides || !SM.slides.length) return; 
 ...
 SM.container.addEventListener('touchstart', e => {
        if (e.target.closest('.widget-overlay')) {
            // ΔΙΟΡΘΩΣΗ: Παγώνει άμεσα τον χρόνο ώστε να διαβαστεί το widget με την ησυχία του
            STATE.isPausedByTitle = true;
            SM.pause();
            if (SM.progressBar) { SM.progressBar.style.transition = 'none'; SM.progressBar.style.width = '100%'; }
            return;
   ...
   SM.container.addEventListener('touchend', e => {
        if (e.target.closest('.widget-overlay')) return;
        
        // ΔΙΟΡΘΩΣΗ: Ακυρώνει το swipe ΜΟΝΟ αν το κείμενο έχει όντως σελιδοποίηση. 
        // Αλλιώς επιτρέπει στο χρήστη να σύρει ελεύθερα τη μικρή ανακοίνωση!
        if (e.target.closest('.slide-desc-container')?.querySelector('.snippet-pagination')) {
