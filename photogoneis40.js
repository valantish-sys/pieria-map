...
window.changeTheme = function() {
 ...
 // --- ΜΕΤΑ ---
  ...
  window.allFetchedAlbumPhotos = []; // 🛡️ ΔΙΟΡΘΩΣΗ: Άδειασμα της μνήμης για να μην καταρρεύσει το "Load More" στο νέο άλμπουμ!
...
window.canUserUpload = async function() {
  if (!currentUserUid) {
    ...
    await authPromise;
    clearTimeout(loadingTimer); // Αν η ταυτοποίηση έγινε άμεσα, ακυρώνουμε το άνοιγμα!
    if (Swal.isVisible()) Swal.close(); // Κλείνουμε με ασφάλεια μόνο αν άνοιξε όντως.
  }
  ...
  try {
    if (currentUserUid && !currentUserUid.startsWith("guest_")) {
      ...
      const snap = await Promise.race([
        dbPub.ref(`user_uploads/${currentUserUid}`).once('value'),
