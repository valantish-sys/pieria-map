       ...
       init: () => {
        ...
        showTooltip: (cellFrame, posts, isModal, isPC) => {
            ...
            if (isModal || !isPC) {
                ...
                if (topPos < window.scrollY + 10) topPos = rect.bottom + window.scrollY + 10; // Αν κόβεται πάνω, το δείχνει κάτω
    ...
    const CalendarWidget = (suffix) => {
        ...
        const self = {
            ...
            setupEvents: () => {
             ...
             let touchstartX = 0, startY = 0, isSwiping = false; // ΔΙΟΡΘΩΣΗ: Δήλωση της μεταβλητής ώστε να μην "σκάει" η JS!
                ...
                self.els.container.addEventListener('touchend', (e) => {
                    ...
                    // ΔΙΟΡΘΩΣΗ: Αν υπήρξε κίνηση (swipe) >10px, μπλοκάρουμε στιγμιαία το click
                    if (Math.abs(diffX) > 10 || Math.abs(diffY) > 10) {
                        isSwiping = true;
                        setTimeout(() => isSwiping = false, 50); // Επαναφορά αμέσως μετά για να λειτουργούν τα μελλοντικά κλικ
                    }
              ...
              self.els.container.addEventListener('click', (e) => {
                    if (isSwiping) { e.preventDefault(); return; } // Ακύρωση του Ghost Click!
