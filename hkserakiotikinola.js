  ...
  const FlipManager = {
    init: async () => {
      ...
      try {
        ...
        // Διαβάζουμε το JSON (ασχέτως αν έχει το όνομα Mob ή Desk)
    ...
    buildWidget: (suffix) => {
      const el = {
        ...
        // Το Desktop έχει extra wrapper για το hover
        flipCard: document.getElementById(suffix === "-desk" ? `my-flip-card${suffix}` : null)
      };
      ...
      const updateDOM = () => {
        ...
        if (nextFact) {
          if (suffix === "-desk") {
            el.fact.innerText = Utils.decodeHTML(nextFact);
          } else {
