...
const mediaNodes = Array.from(wrapper.querySelectorAll(`${bodySelectors}, .scrollable-article-area img, .scrollable-article-area iframe`));
      ...
      this.overlay = document.createElement('div');
            ...
            // Επιτρέπει το Pinch-to-Zoom, απενεργοποιώντας μόνο το native οριζόντιο swipe
            this.overlay.style.touchAction = 'pinch-zoom';
    ...
    class ArticleEngine {
        ...
        static async handleInteraction(btn) {
         ...
         try {
             ...
             // Επαναφορά των κρυμμένων εικόνων που έχουν παγιδευτεί από το Lazy Load του Blogger
                fetchedBody.querySelectorAll('img').forEach(img => {
                    const realSrc = img.getAttribute('data-src') || img.getAttribute('data-original-src');
                    if (realSrc && img.src !== realSrc) {
                        img.src = realSrc;
                        img.removeAttribute('srcset'); // Υποχρεωτική διαγραφή για να μην επιβάλει ο Browser τη θολή έκδοση!
                    }
                });
          ...
          // Επανεκτέλεση των Scripts ώστε να δουλεύουν τα ενσωματωμένα Tweets & Instagram Posts
                ...
                div.querySelectorAll('script').forEach(oldScript => {
                 ...
                 const newScript = document.createElement('script');
                    ...
                    // Εξαναγκασμός σειριακής (σύγχρονης) εκτέλεσης ώστε πολύπλοκα σχολικά widgets να τρέξουν με τη σωστή σειρά
                ...
                // Αντί για βίαιη ανακατεύθυνση (που θα πετάξει Error του Browser αν δεν έχει ίντερνετ), δίνουμε την ευκαιρία επανάληψης:
                btn.classList.remove('is-loading');
                btn.innerHTML = '<span class="icon">⚠️</span> ΣΦΑΛΜΑ ΔΙΚΤΥΟΥ! ΔΟΚΙΜΑΣΤΕ ΞΑΝΑ';
            }
        }
static processFetchedContent(fetchedBody, summaryWrap) {
      ...
      static close(post, btn, isPopState = false) {
         ...
         // Σταματάμε τα video iframes ΚΑΙ τις πλατφόρμες ήχου/podcasts, αφήνοντας ανέπαφα τα Social Media
    ...
    class FeedbackEngine {
            ...
            if (!this.observer) {
                this.observer = new IntersectionObserver((entries, obs) => {
                    ...
                    get(ref(db, 'reactions/' + safePostId)).then((snapshot) => {
                                // Διακόπτουμε την αντικατάσταση αν ο χρήστης ψηφίζει, Ή αν έχει ΗΔΗ ψηφίσει επιτυχώς σε αυτή τη συνεδρία
                                if (div.classList.contains('voted') || Array.from(div.querySelectorAll('span:last-child')).some(span => span.innerText === '..')) return;
     ...
     spanCount.innerText = "..";
          ...
          // Εκτελούμε τον καθολικό μετρητή ανεξάρτητα στο παρασκήνιο
            runTransaction(totalRef, (currentTotal) => { return (currentTotal || 0) + 1; }).catch(() => {});
         ...
         // Popstate για back button του Browser
            window.addEventListener('popstate', (e) => {
                ...
                if (document.body.classList.contains('lb-active')) {
                    AppLightbox.close();
                    return; // Αποτροπή εκτέλεσης του κλεισίματος του άρθρου
                }
