  ...
  window.SliderManager = {
    ...
    buildDOM: (entries) => {
      ...
      entries.forEach((entry, idx) => {
        ...
        // --- ΑΛΛΑΓΗ 1: Αν ο τίτλος έχει το [MENU], εμφανίζει τα Tabs ---
        if (title.includes("[MENU]")) {
            const customSlide = document.createElement('div'); // div για να δουλεύουν τα κουμπιά
            customSlide.className = `slide ${isPinned ? "pinned-contain" : ""}`;
            customSlide.setAttribute("aria-hidden", "true");
            
            customSlide.innerHTML = `
              <!-- ΕΠΙΚΟΛΛΗΣΕ ΕΔΩ ΟΛΟ ΤΟΝ ΚΩΔΙΚΑ ΣΟΥ (το <style> και το <div class="glass-tabs-wrapper">...</div>) -->
            `;
            
            fragment.appendChild(customSlide);
            SM.slides.push({ el: customSlide, imgs: [], chunks: [] });
            return; // Προχωράμε στην επόμενη ανάρτηση, χωρίς να φτιάξουμε κλασικό slide!
        }
        // --- ΤΕΛΟΣ ΑΛΛΑΓΗΣ 1 ---
    ...
    setupEvents: () => {
      ...
      SM.container.addEventListener('click', (e) => {
        ...
        // --- ΑΛΛΑΓΗ 3: Παύση του Slider αν κλικάρεις στα Tabs ---
        if (e.target.closest('.glass-tabs-wrapper')) {
      ...
      document.addEventListener('click', (e) => {
        if (STATE.isPausedByTitle && !e.target.closest('.slide-title') && !e.target.closest('.glass-tabs-wrapper')) {
      ...
      SM.container.addEventListener('touchstart', e => {
        if (e.target.closest('.slide-desc-container') || e.target.closest('.glass-tabs-wrapper')) return;
      ...
      SM.container.addEventListener('mousedown', e => {
        if (e.button !== 0 || STATE.isTouchDevice || e.target.closest('.slide-desc-container') || e.target.closest('.glass-tabs-wrapper')) return;
