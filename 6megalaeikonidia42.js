...
// --- Εξωτερικό Τηλεκοντρόλ Παύσης Κινήσεων (Με ενσωματωμένο "Κοριό" / Monkey Patch) ---
  const applySliderPatch = () => {
    ...
    // ΠΡΟΣΘΗΚΗ: Το resume() γίνεται ΜΟΝΟ αν το ζήτησε ρητά ο χρήστης. Αλλιώς ξεπαγώνει βίαια το Slider ενώ το παιδί το στοχεύει.
      ...
      } else if (arguments.length > 0 && arguments[0] === true) {
  ...
  setTimeout(() => applySliderPatch(false), 1500);
 ...
 // ΝΕΟ: Άμεση απόκρυψη του παλιού συννεφακίου αν το ποντίκι πήγε σε ΔΙΑΦΟΡΕΤΙΚΟ link!
      ...
      // Προστασία: Αν το συννεφάκι βγαίνει έξω δεξιά, σπρώξτο αριστερά
        // ΔΙΟΡΘΩΣΗ: Σύγκριση της ορατής θέσης (rect.left) με το ορατό πλάτος (innerWidth) για αποφυγή σφάλματος κατά το Zoom
...
if (window.location.hash) {
 ...
 // ΔΙΟΡΘΩΣΗ: Αντί για αυθαίρετο χρονόμετρο 1s, περιμένουμε με ασφάλεια να φορτώσει το DOM (σε αργά δίκτυα)
