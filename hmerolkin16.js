            ...
            CalendarEngine.calendar = new window.FullCalendar.Calendar(DOM.calendarEl, {
                ...
                dayCellDidMount: (info) => {
                    ...
                    if (DataEngine.postsByDate[cellDateStr]) {
                        ...
                        // ΑΣΦΑΛΕΙΑ: Βάζει την τελεία ΜΟΝΟ αν δεν υπάρχει ήδη!
                        ...
                        // ΑΣΦΑΛΕΙΑ: Βάζει το Emoji ΜΟΝΟ αν δεν υπάρχει ήδη!
                        if (!frame.querySelector('.day-indicator-emoji')) {
                            let indicator = document.createElement('div');
                            indicator.className = 'day-indicator-emoji'; // Του δίνουμε κλάση για να το βρίσκουμε
        ...
        init: () => {
            ...
            const waitForCalendar = setInterval(async () => {
                if (window.FullCalendar) {
                   ...
                   // ΝΕΟ: Τα κατεβάζει ΤΑΥΤΟΧΡΟΝΑ. Το Ημερολόγιο φορτώνει 50% πιο γρήγορα στο κινητό!
                    await Promise.all([
