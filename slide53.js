    ...
    init: () => {
      ...
      document.addEventListener('click', (e) => {
        ...
        if (!nav.contains(e.target)) {
          // 1. Κλικ ΕΞΩ από το μενού: Τα κλείνει όλα
          ...
          // 2. Κλικ ΜΕΣΑ στο μενού
          const clickedLi = e.target.closest('li'); 
          
          if (clickedLi) {
            allDropdowns.forEach(d => {
              // Ελέγχει αν το dropdown ανήκει στην επιλογή που πατήσαμε
              const isChild = clickedLi.contains(d); // π.χ. Ανοίγουμε ένα μενού
              const isParent = d.contains(clickedLi); // π.χ. Κάνουμε κλικ μέσα σε ήδη ανοιχτό μενού

              // Αν ΔΕΝ είναι ούτε το παιδί, ούτε ο γονιός, σημαίνει ότι είναι ένα "διπλανό" (άλλο) μενού. Το κλείνουμε!
              if (!isChild && !isParent) {
                d.style.display = 'none';
                setTimeout(() => { d.style.display = ''; }, 100);
              }
