 ...
 // Το createImageBitmap σέβεται αυτόματα την περιστροφή EXIF της κάμερας!
  window.createImageBitmap(file).then(img => {
     ...
     canvas.toBlob(blob => {
        ...
        // Στέλνουμε τη λεζάντα στον Worker για να τη βάλει στο Email!
        formData.append("caption", userCaption || ""); 

        // 🚨 ΒΑΛΕ ΕΔΩ ΤΟ URL ΤΟΥ ΔΙΚΟΥ ΣΟΥ CLOUDFLARE WORKER ΠΟΥ ΑΝΤΕΓΡΑΨΕΣ 🚨
        const WORKER_URL = "https://school-album-api.το-όνομά-σου.workers.dev"; 
        ...
        .then(data => {
          if(data.secure_url) {
            ...
            // Ο Worker ανέβασε τη φώτο (στα κρυφά) και έστειλε το email! 
            // Εμείς απλά γράφουμε το URL που μας έδωσε πίσω, στη Firebase.
            dbPub.ref("album_photos").push({
              url: data.secure_url,
              caption: userCaption || "",
              theme_id: activeThemeId,
              status: "pending", // Η Firebase το επιτρέπει λόγω του κανόνα σου!
              timestamp: Date.now()
            });
            
            // Το Email διεγράφη εντελώς από εδώ.
            
            // Καταγραφή ανεβάσματος για το Rate Limit (Και αυτό δουλεύει άψογα)
            if (currentUserUid) {
            ...
            Swal.fire({
              ...
              text: 'Μόλις εγκριθεί από τον διαχειριστή, θα εμφανιστεί!',
              confirmButtonColor: '#2ecc71'
            });
          } else {
            Swal.fire({ icon: 'error', title: 'Σφάλμα', text: 'Αποτυχία ανεβάσματος. Δοκιμάστε ξανά.', confirmButtonColor: '#e74c3c' });
          }
        })
        .catch(err => {
          console.error("Worker Error:", err);
          Swal.fire({ icon: 'error', title: 'Σφάλμα', text: 'Υπήρξε πρόβλημα επικοινωνίας με τον Server.', confirmButtonColor: '#e74c3c' });
        });
     }, "image/webp", 0.85);
 }).catch(err => {
