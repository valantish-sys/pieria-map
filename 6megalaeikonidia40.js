...
// Share Event
  ...
  safeBind('btn-do-print', 'click', () => {
    DOM.shareMenu?.classList.remove('show');
    window.print(); // Άμεση εκτέλεση (Σύγχρονα) για να μην μπλοκαριστεί από το iOS Safari
  });
...
const speech = new SpeechSynthesisUtterance();
    ...
    // ΠΡΟΣΘΗΚΗ: Δυναμική προσαρμογή της γλώσσας ανάγνωσης αν ο χρήστης έχει μεταφράσει το άρθρο!
  ...
  const fallbackElement = document.querySelector('.main-wrapper') || document.body;
const textToRead = article ? article.innerText : fallbackElement.innerText;
...
const applyLang = (lang, retryCount = 0) => {
 ...
 // --- ΕΙΔΙΚΗ ΔΙΑΧΕΙΡΙΣΗ ΓΙΑ ΕΛΛΗΝΙΚΑ (ΑΡΧΙΚΗ ΓΛΩΣΣΑ) ---
  if (lang === 'el') {
    ...
    // 2. Κάνουμε reload για καθαρή επαναφορά στην αρχική γλώσσα της σελίδας
...
// Αποθήκευση της διαδικασίας λήξης σε μεταβλητή
   const handleDragEnd = () => {
      if (!isDragging) {
...
let currentWordRequested = '';
  ...
  const fetchDefinition = async (word, x, y) => {
    ...
    // 3. Το safeX "εγκλωβίζει" το συννεφάκι για να μην βγει ΠΟΤΕ εκτός οθόνης!
      ...
      // Έλεγχος αν η λέξη είναι πολύ ψηλά, ώστε να μην κοπεί το συννεφάκι εκτός οθόνης
      const tHeight = glossaryTooltip.offsetHeight || 80;
      if (y - window.scrollY - offset - tHeight < 5) {
        glossaryTooltip.style.top = `${y + 35}px`; 
        glossaryTooltip.style.transform = `translate(-50%, 0)`; // Το εμφανίζουμε ΑΠΟ ΚΑΤΩ
      } else {
        glossaryTooltip.style.top = `${y - offset}px`; 
        glossaryTooltip.style.transform = `translate(-50%, -100%)`; // Το εμφανίζουμε ΑΠΟ ΠΑΝΩ
      }
    };
...
// --- Εξωτερικό Τηλεκοντρόλ Παύσης Κινήσεων (Με ενσωματωμένο "Κοριό" / Monkey Patch) ---
 ...
 const url = link.href;
  ...
  // ΠΡΟΣΘΗΚΗ: Έλεγχος ώστε να μπλοκάρονται μόνο οι άγκυρες της ΙΔΙΑΣ σελίδας (όχι links προς άλλα άρθρα)
      const isSamePageAnchor = url.includes('#') && url.split('#')[0] === window.location.href.split('#')[0];
...
if (iframe.dataset.url !== url) {
    // Απόλυτη διαγραφή και αναδημιουργία του iframe για να μην ρυπαίνεται το ιστορικό του χρήστη!
    ...
    iframe = newIframe; // Σύνδεση της μεταβλητής με το νέο καθαρό στοιχείο
  }
