...
function previewParentImage(event) {
    ...
    if (selectedFile) {
     ...
     // 🍏 Υποστήριξη iPhone (HEIC) & WEBP με νέα μηνύματα
  ...
  try {
       ...
       const newPostRef = window.dbAnartiseis.ref("parent_posts").push();
        ...
        let slowUploadTimer = setTimeout(() => {
            isUploadSlow = true;
            if (typeof Swal !== 'undefined') Swal.fire({ icon: 'info', title: 'Αργό Ίντερνετ ⏳', text: 'Το ίντερνετ καθυστερεί, αλλά η αποθήκευση συνεχίζεται στο παρασκήνιο...', showConfirmButton: false });
        }, 10000); // 10 δευτερόλεπτα ανοχή

      firebasePushPromise.catch(() => {}); // ⚡ UX (Από το Άλμπουμ): Βγάζουμε το await. Το Firebase συγχρονίζει αθόρυβα στο background!
        ...
        // 1. Μήνυμα Επιτυχίας (κλείνει αυτόματα το loading Swal)
        if (typeof Swal !== 'undefined') {
            Swal.fire({ icon: 'success', title: 'Επιτυχία!', text: 'Η ανάρτηση στάλθηκε για έγκριση.', timer: 2500, showConfirmButton: false });
        } else {
       ...
       // 🔓 Ξεκλείδωμα στοιχείων και καθαρισμός μνήμης σε περίπτωση αποτυχίας!
        document.getElementById('p-image').disabled = false;
        document.getElementById('p-author').readOnly = false;
        document.getElementById('p-title').readOnly = false;
        document.getElementById('p-content').readOnly = false;
       ...
       document.getElementById('p-image').value = '';
        ...
        // 🧠 ΠΡΟΣΤΑΣΙΑ ΜΝΗΜΗΣ: Καθάρισμα του Blob URL της εικόνας για να μην τρώει μνήμη RAM
        const imgPrev = document.getElementById('p-img-preview');
        if (imgPrev && imgPrev.src && imgPrev.src.startsWith('blob:')) {
            URL.revokeObjectURL(imgPrev.src);
            imgPrev.src = '';
