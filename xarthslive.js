...
aiginio: { title: "Αιγίνιο 🌾", desc: "Ο εύφορος κάμπος της βόρειας Πιερίας. Ένας τόπος γεμάτος ζωντανή παράδοση, ιστορικές βυζαντινές εκκλησιές και φυσική ομορφιά κοντά στους προστατευμένους υδροβιότοπους.", img: "https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEglKzTrLdlgz15emRwy5PSkfbrqcWgy0-5ZK-FWCwsctBqaZ7Ro4Q_WmWoAItjYl14RHKnnKcnARxP35QeiUK1PLCqE5QGX7yPzCr-3QOyll6AFMX8QypFpVMDvgE2oenzCW3ydLADTf2rJq6W2PnY60RfnAgRvn8aSNI04nSX62zbtgrRwg2MZKwMc2i0/w400-h223/2026-07-01%2014_10_17-2025-09-25%2018_01_46-World%20of%20Warcraft.png" },
  ...
  function showPlace1(placeId, event) {
    document.getElementById('tt-title-1').innerHTML = placesData1[placeId].title;
    document.getElementById('tt-desc-1').innerHTML = placesData1[placeId].desc;
    
    // ΠΡΟΣΘΗΚΗ: Εμφάνιση / Απόκρυψη εικόνας
    const imgEl = document.getElementById('tt-img-1');
    if (placesData1[placeId].img) {
      imgEl.src = placesData1[placeId].img; 
      imgEl.style.display = 'block'; 
    } else {
      imgEl.style.display = 'none'; 
    }

    document.getElementById('tolkien-tooltip-1').classList.add('active');
    if(event) event.stopPropagation();
  }

  function hidePlaceMap1() {
    document.getElementById('tolkien-tooltip-1').classList.remove('active');
}

  document.addEventListener('click', function(event) {
    const tooltip1 = document.getElementById('tolkien-tooltip-1');
    if (tooltip1.classList.contains('active') && !tooltip1.contains(event.target)) {
        hidePlaceMap1();
    }
});
// === ΗΧΗΤΙΚΟ ΕΦΕ ΚΟΥΜΠΙΟΥ (Συνολικά 12 δευτερόλεπτα με Fade Out) ===
const oceanAudio = new Audio("https://res.cloudinary.com/drx2a5ane/video/upload/v1783095191/solarmusic-ocean-waves-112906._ee1tbg.mp3");
let audioTimeout;
let fadeInterval;

let mapState = 3; // Ξεκινάμε από το 3 για να κάνει την αρχική οργάνωση σωστά
let isSoundOn1 = true;

function toggleMute1() {
    isSoundOn1 = !isSoundOn1;
    document.getElementById('sound-btn-1').innerHTML = isSoundOn1 ? "🔊" : "🔇";
    
    // Αν το πατήσει για σίγαση ενώ παίζει ήδη ήχος, τον κόβουμε αμέσως
    if (!isSoundOn1) {
        oceanAudio.pause();
...
function toggleBanners(isFirstLoad) {
    hidePlaceMap1(); 

    // Καθαρισμός προηγούμενων χρονομέτρων
    clearTimeout(audioTimeout);
    clearInterval(fadeInterval);

    // ΝΕΟ: Ελέγχει αν ο ήχος είναι ενεργοποιημένος πριν παίξει
    if (isSoundOn1 && !isFirstLoad) {
        ...
        }, 6000); // 12 δευτερόλεπτα (ή 10000 αν θες 10)
    }
    const groupA = [
        'peristasi', 'olympus', 'katerini', 'platamonas', 'aiginio', 
        'pydna', 'elatochori', 'litochoro', 'kolindros', 'kontariotissa', 'milia'
    ];
    
    const groupB = [
        'alykes', 'livadi', 'ryakia', 'paralia', 'dion', 
        'agiosdimitrios', 'panteleimonas', 'leptokarya', 'anoskotina', 'korinos'
    ];
    
    const button = document.getElementById('toggle-btn');
    const map1Area = document.getElementById('map-area');
const allBanners = map1Area.querySelectorAll('.t-banner');

    if (mapState === 1) {
        // ΠΑΤΗΜΑ 1: Δείχνουμε το Group B (visible), Κρύβουμε το Group A (hidden)
        allBanners.forEach(banner => {
            const onClickAttr = banner.getAttribute('onclick') || '';
            const isGroupB = groupB.some(place => onClickAttr.includes(place));
        ...
        // ΠΑΤΗΜΑ 2: Τα δείχνουμε ΟΛΑ (visible παντού)
        allBanners.forEach(banner => {
            banner.classList.remove('hidden-group');
            banner.classList.add('visible-group');
        });
        ...
        // ΠΑΤΗΜΑ 3 (ή αρχική φόρτωση): Δείχνουμε μόνο το Group A
        allBanners.forEach(banner => {
            const onClickAttr = banner.getAttribute('onclick') || '';
            const isGroupA = groupA.some(place => onClickAttr.includes(place));
...
// Τρέχουμε τη συνάρτηση ΑΠΕΥΘΕΙΑΣ (χωρίς DOMContentLoaded) για να είναι 100% συμβατό με το Blogger
toggleBanners(true);
// ✨ ΑΥΤΟΜΑΤΟ PRELOADING ΟΛΩΝ ΤΩΝ ΕΙΚΟΝΩΝ ΤΟΥ 1ου ΧΑΡΤΗ ΓΙΑ ΑΚΑΡΙΑΙΑ ΕΜΦΑΝΙΣΗ
window.addEventListener('load', function() {
    Object.values(placesData1).forEach(place => {
        if (place.img) {
