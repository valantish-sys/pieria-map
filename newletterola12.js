    ...
    const ScratchManager = {
        initCanvas: () => {
           ...
           setTimeout(() => {
                ...
                // ΝΕΟ: Ακυρώνει την εξαφάνιση αν ο χρήστης ζήτησε νέο παιχνίδι πολύ γρήγορα!
      ...
      draw: (e) => {
       ...
       checkReveal: () => {
           ...
           // Ελέγχουμε 1 στα 16 pixels (ελαφρύτερο για τη μνήμη του κινητού)
            ...
            if ((clearPixels / totalToCheck) * 100 > 480) { 
                isRevealed = true;
...
if (startIndex + entries.length <= totalResults && entries.length > 0) {
    ...
    processPosts(allEntries);
        ...
        const processPosts = (entries) => {
           ...
           // Αν τα άρθρα υπάρχουν ήδη στη μνήμη, τα φορτώνουμε ακαριαία χωρίς νέα κλήση στο API!
            if (cachedEntries) {
                processPosts(cachedEntries);
            } else {
                fetchBatch(1);
            }
        }
