    ...
    (() => {
        ...
        const Utils = {
            ...
            getPostPath: (container) => {
                ...
                try { return (linkElem && linkElem.href && linkElem.href !== "#") ? new URL(linkElem.href, window.location.origin).pathname : null; } 
                catch(e) { return null; }
            }
        };
// ==========================================
    ...
    class LightboxEngine {
        ...
        bindEvents() {
            ...
            containerDiv.addEventListener('touchstart', e => {
                if (e.touches.length > 1) return; // Αγνοεί pinch-to-zoom
                this.touchStartX = e.changedTouches[0].screenX;
            }, {passive: true});
            
            containerDiv.addEventListener('touchend', e => {
                if (e.changedTouches && e.changedTouches.length > 1) return; // Αγνοεί pinch-to-zoom
...
// ==========================================
    ...
    class FeedbackEngine {
        static scan() {
            // 1. Ασφαλής δημιουργία του Observer ΜΕΣΑ στη συνάρτηση (Συμβατό με όλα τα κινητά!)
            if (!this.observer) {
                this.observer = new IntersectionObserver((entries, obs) => {
                    entries.forEach(entry => {
                        if (entry.isIntersecting) {
                            const div = entry.target;
                            obs.unobserve(div); // Το διαβάζουμε 1 φορά και τέλος!
                            ...
                            get(postRef).then((snapshot) => {
                                ...
                                const l = div.querySelector('.count-love');
                                const f = div.querySelector('.count-funny');
                                const w = div.querySelector('.count-wow');
                                if (l) l.textContent = d.love || 0;
                                if (f) f.textContent = d.funny || 0;
                                if (w) w.textContent = d.wow || 0;
                            }).catch(console.error);
                        }
                    });
            ...
            // 2. Σάρωση των άρθρων (Και αυτών που φορτώνουν με Infinite Scroll)
            document.querySelectorAll(CONFIG.SELECTORS.ARTICLE_CONTAINER).forEach(container => {
                ...
                this.observer.observe(div); // Μπαίνει στον "έξυπνο" μετρητή που φτιάξαμε πάνω
            });
        }
       ...
       static handleReaction(btn) {
            ...
            // 1. Διορθώθηκε το Query για να στοχεύει αυστηρά το νούμερο:
            const span = btn.querySelector(`.count-${type}`) || btn.querySelector('span:last-child');
            
            // 2. Φυλάμε τον παλιό αριθμό με ασφάλεια:
            const prevCountNumber = parseInt(span.textContent, 10) || 0;
            span.textContent = "..";
            ...
            .then((txResult) => {
                ...
                // 3. ΥΠΟΛΟΓΙΣΜΟΣ ΝΕΟΥ ΝΟΥΜΕΡΟΥ: Αν το snapshot αποτύχει, χρησιμοποιούμε τον φυλαγμένο αριθμό + 1
                let newCount = prevCountNumber + 1; 
                if (txResult && txResult.committed && txResult.snapshot.exists()) {
                    newCount = txResult.snapshot.val();
                }
                
                // Εγγυημένη ενημέρωση του DOM
            ...
            .catch((error) => {
                ...
                // Σε περίπτωση σφάλματος, επαναφέρουμε τον παλιό αριθμό
                span.textContent = prevCountNumber;
            });
        }
    }
   const AppController = {
        init: () => {
            // Αρχική Σάρωση: ΠΡΩΤΑ το άρθρο, ΜΕΤΑ τα Emoji
            ArticleEngine.scan();
            FeedbackEngine.scan();
            ...
            document.addEventListener('newPostsLoaded', () => {
                ArticleEngine.scan();  // 1ο: Φτιάχνει το Κουμπί Read More
                FeedbackEngine.scan(); // 2ο: Φτιάχνει τα Emoji ακριβώς από κάτω!
            });
            
            // Popstate για back button του Browser
            window.addEventListener('popstate', ArticleEngine.closeAllOpened);
          // Λαμβάνει σήμα αν ψηφίσει ο χρήστης στο Slider και ανανεώνει το άρθρο!
            window.addEventListener('reactionSync', (e) => {
                const { safePostId, type, newCount } = e.detail;
                document.querySelectorAll(`.smart-feedback-buttons[data-postid="${safePostId}"]`).forEach(btnContainer => {
                    const mainWrap = btnContainer.closest('.smart-feedback-container');
                    if (mainWrap) mainWrap.classList.add('voted');
                    btnContainer.style.opacity = "0.8";
                    btnContainer.style.pointerEvents = 'none';
                    const targetSpan = btnContainer.querySelector(`.smart-feedback-btn[data-type="${type}"] span:last-child`);
                    if (targetSpan && targetSpan.textContent !== "..") targetSpan.textContent = newCount;
                });
            });
        },
