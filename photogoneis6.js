      ...
      if (!isLoadMore) {
        ...
        newPhotoListener.on("child_added", newSnap => {
            ...
            if (photoData.status === "approved" && !window.albumPhotosList.some(p => p.id === newSnap.key)) {
                ...
                let captionHtml = photoData.caption ? `<div class="polaroid-caption">${escapeHTML(photoData.caption)}</div>` : "";
                ...
                Array.from(grid.children).forEach((el, idx) => el.onclick = () => window.openAlbumLightbox(idx));
