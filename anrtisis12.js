...
const compressImageLocally = async (file) => {
    return new Promise((resolve, reject) => { 
     ...
     // 🛡️ ΑΣΦΑΛΕΙΑ 2: Παράκαμψη συμπίεσης σε παλιές συσκευές ώστε να ΜΗΝ αποκλείονται οι μαθητές/γονείς!
...
function previewParentImage(event) {
    ...
    if (selectedFile) {
       ...
       if (!isImage && !isHeicOrWebp) {
            ...
            // 🧹 Καθαρισμός του "φαντάσματος" της παλιάς εικόνας από την οθόνη
...
function initParentForm(retries = 0) {
    ...
    if (typeof firebase === 'undefined' || !firebase.database) {
        if (retries > 50) return; // 🛑 Ακύρωση αναμονής μετά από 5 δευτερόλεπτα
...
async function submitParentPost() {
  ...
  let imageUrl = null;
    let slowUploadTimer; // ⏱️ Δήλωση έξω από το try για να έχουμε πρόσβαση οπουδήποτε!
  ...
  try {
      ...
      // ⚡ Ξεκινάμε την εγγραφή στη βάση δεδομένων (ΜΙΑ ΦΟΡΑ)
    ...
    let isUploadSlow = false;
        slowUploadTimer = setTimeout(() => { // Χωρίς το let εδώ!
            isUploadSlow = true;
      ...
      // 🔒 ΠΡΟΣΤΑΣΙΑ ΔΕΔΟΜΕΝΩΝ: Αναμονή με όριο 15 δευτερολέπτων για να μην κολλήσει η φόρμα (π.χ. από Firewall)
        const firebaseTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('firebase_timeout')), 15000));
      ...
      // 1. Μήνυμα Επιτυχίας (Προσαρμοσμένο βάσει της εικόνας αναμνήσεων)
        ...
        clearTimeout(slowUploadTimer); // 🛑 ΑΚΥΡΩΣΗ του χρονομέτρου σε περίπτωση σφάλματος!
        console.error(error);
       ...
       if (typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Σφάλμα', text: errorMsg });
        ...
        // 🔓 ΠΡΟΣΤΕΘΗΚΕ: Ξεκλείδωμα όλων των πεδίων και επαναφορά του κουμπιού για να ξαναπροσπαθήσει!
        document.getElementById('p-image').disabled = false;
        document.getElementById('p-author').readOnly = false;
        document.getElementById('p-title').readOnly = false;
        document.getElementById('p-content').readOnly = false;
...
const initCharCounters = () => {
  ...
  for (const [id, max] of Object.entries(limits)) {
        const el = document.getElementById(id);
        if (!el || el.dataset.counterInit) continue; // 🛡️ Προστασία: Αν υπάρχει ήδη μετρητής, τον προσπερνάει
