(() => {
   ...
   const Utils = {
      cleanText: (htmlStr) => {
            ...
            const spacedStr = htmlStr.replace(/<\/?(p|div|br|h[1-6]|li|blockquote)[^>]*>/gi, ' ');
        ...
        fetchPosts: async () => {
            ...
            try {
              ...
              const totalPosts = parseInt(metaData.feed.openSearch$totalResults.$t, 10);
               ...
               // Το API του Blogger δεν επιτρέπει start-index πάνω από 500. Επιβάλλεται περιορισμός 
                // για να μην καταρρέει η αναζήτηση στα μεγάλα blogs.
                const maxAllowedIndex = Math.min(totalPosts, 500);
                const randomIndex = Math.floor(Math.random() * maxAllowedIndex) + 1;
                const finalUrl = `/feeds/posts/default/-/${encodedLabel}?alt=json&max-results=1&start-index=${randomIndex}`;
                const finalRes = await fetch(finalUrl);
                const finalData = await finalRes.json();

              // Ασφαλής έλεγχος με optional chaining. Αν δεν υπάρχει καθόλου έγκυρο άρθρο, αναγκάζουμε 
                // τη ρίψη σφάλματος για να ενεργοποιηθεί ο μηχανισμός Fallback αντί να κολλήσει το UI.
                const entry = finalData.feed?.entry?.[0] || metaData.feed?.entry?.[0];
                if (!entry) throw new Error("Δεν βρέθηκε έγκυρο άρθρο.");
