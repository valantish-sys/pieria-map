...
window.addEventListener('beforeunload', (e) => {
    if (isSubmitting) return;
    const contentText = document.getElementById('p-content').innerText.trim();
    const titleText = document.getElementById('p-title').value.trim();
    
    // Εάν έχουν γράψει έστω και λίγο, το σώζουμε αυτόματα και εμφανίζουμε προειδοποίηση στην έξοδο (αν την υποστηρίζει ο browser)
    if (contentText.length > 0 || titleText.length > 0) {
        window.saveDraft();
        e.preventDefault();
        e.returnValue = ''; 
    }
