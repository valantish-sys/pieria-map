(function($) {
    ...
    // --- SENIOR MASTER LOGIC: Time-Based LocalStorage & Referrer Check ---
      ...
      var lastSeen = null;
      try {
          lastSeen = localStorage.getItem('introLastSeenDate');
      } catch (e) {
          // Αποτροπή crash αν η πρόσβαση στο localStorage είναι μπλοκαρισμένη
      }
  ...
  // Ανανεώνουμε το χρόνο αν κάνει περιήγηση, ώστε να μην πεταχτεί ξαφνικά αν αλλάξει tab μετά από 2 ώρες
          try { localStorage.setItem('introLastSeenDate', now.toString()); } catch(e) {}
     ...
     // Αν έφτασε εδώ, σημαίνει ότι θα δει το intro. Αποθηκεύουμε την τωρινή ώρα.
      ...
      // ΚΡΙΣΙΜΟΣ ΕΛΕΓΧΟΣ: Αν το HTML του Intro δεν υπάρχει σε αυτή τη σελίδα, σταματάμε αμέσως!
      if ($('#epic-intro-overlay').length === 0) return;
...
var isMobile = (window.innerWidth <= 1300) || isTouch;
    ...
    // 2. Parallax με RequestAnimationFrame (Απόλυτη 60fps απόδοση)
      ...
      let $parallaxContainer = $("#intro-magic-canvas-container"); // Caching του στοιχείου μία φορά στην αρχή

      function updateParallax() {
        var moveX = (mouseX * 4 - 2); 
        var moveY = (mouseY * 4 - 2);
        // Άμεση εφαρμογή του CSS στο αποθηκευμένο στοιχείο, αποφεύγοντας την αναζήτηση στο DOM
        $parallaxContainer.css("transform", "translate3d(" + moveX + "%, " + moveY + "%, 0)");
        requestRef = null;
      }

    if (!isMobile) {
          $(document).on("mousemove.intro", function(e) {
            // Αφαιρέθηκε η περιττή και εξαιρετικά "βαριά" αναζήτηση στο DOM (DOM Thrashing)
  ...
  // 3. Cinematic Έξοδος & Memory Cleanup
      // Αλλαγή σε .one() ώστε το event να καταγραφεί αυστηρά μόνο 1 φορά
      $('#intro-enter-school-btn').one('click', function(e) {
        e.preventDefault(); 
        try { sessionStorage.setItem('introSeen', 'true'); } catch(e) {} // Προσθήκη προστασίας και για το sessionStorage
        
        // ---- ΕΦΑΡΜΟΓΗ ΔΙΟΡΘΩΣΗΣ #4 ----
        // ΑΚΑΡΙΑΙΟΣ τερματισμός του Parallax για αποφυγή frame drops κατά το FadeOut
        ...
        setTimeout(function() {
          $('#epic-intro-overlay').fadeOut(1000, function() {
            ...
            // (Οι εντολές του Parallax αφαιρέθηκαν από εδώ γιατί μπήκαν ψηλά στο κλικ)

            // Τερματισμός του WebGL context και απελευθέρωση πόρων πριν τη διαγραφή του DOM
            try { $('#intro-magic-canvas').ripples('destroy'); } catch(e) {}
