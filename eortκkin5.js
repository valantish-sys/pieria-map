  ...
  const DataEngine = {
        ...
        renderWeather: async () => {
            ...
            try {
                ...
                for(let i=0; i<3; i++) {
                    ...
                    const morningFeelsLike = Math.round(data.hourly.apparent_temperature[baseIndex + 8]); 
                    ...
                    if (i === 0) {
                        ...
                        if (widgetBox) {
                            widgetBox.classList.remove('bg-weather-sun', 'bg-weather-rain', 'bg-weather-snow');
                            if ([71,73,75,77].includes(mainCodeRaw)) widgetBox.classList.add('bg-weather-snow');
                            else if ([51,53,55,61,63,65,80,81,82,95,96,99].includes(mainCodeRaw)) widgetBox.classList.add('bg-weather-rain');
                            else if ([0,1,2,3].includes(mainCodeRaw)) widgetBox.classList.add('bg-weather-sun');
                        }
                    }
                        ...
                        <div class="c-day-card ${i===0 ? 'today' : ''}">
                            <div class="c-day-inner">
                                <div class="c-day-front">
                                    ...
                                    <div style="font-size: 10px; color: #64748b; margin-top: 4px; font-weight: 700;">Αίσθηση 8πμ: <span style="color:#a90e0e;">${morningFeelsLike}°</span></div>
                                </div>
    ...
    const MobileAppController = {
        init: async () => {
            ...
            // ΝΕΟ: Event Listeners για τα κουμπιά
    ...
    if (document.readyState === "loading") {
      ...
      // --- ΝΕΟ: Προσθήκη Αφής (Swipe) για αλλαγή ημέρας ---
            let touchStartX = 0;
            ...
            // Καταγράφουμε πού σηκώθηκε το δάχτυλο και υπολογίζουμε
            eortBox.addEventListener('touchend', (e) => {
                const touchEndX = e.changedTouches[0].screenX;
                const diffX = touchStartX - touchEndX;

                // Αν το σύρσιμο είναι πάνω από 40 pixels (για να μην πιάνει τυχαία αγγίγματα)
                if (Math.abs(diffX) > 40) {
                    if (diffX > 0) {
                        // Σύρσιμο αριστερά -> Επόμενη μέρα (όπως γυρνάμε σελίδα)
                        MobileDateEngine.changeDay(1);
                    } else {
                        // Σύρσιμο δεξιά -> Προηγούμενη μέρα
                        MobileDateEngine.changeDay(-1);
                    }
                    MobileUIEngine.renderHeader();
                    MobileUIEngine.renderHolidays();
