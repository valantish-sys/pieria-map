...
window.stopWatchdog = function() {
  ...
  const emptyState = document.getElementById("empty-state");
  if (emptyState && emptyState.innerHTML.includes("🦊")) {
     if (emptyState.dataset.originalHtml) {
...
const initAlbumSettings = () => {
 ...
 // --- ΜΕΤΑ ---
   // 🛡️ Αν το άλμπουμ διαγράφηκε από τον Admin όσο το βλέπουμε, το data.themes[currentViewId] 
   // γίνεται undefined. Επιστρέφουμε στο activeThemeId για να μην κρασάρει με TypeError η σελίδα!
...
window.renderPhotos = function(isLoadMore) {
 ...
 if (lb && lb.style.display === 'flex' && window.albumPhotosList && window.albumPhotosList[currentLightboxIndex]) {
     ...
     if (newIndex !== -1) currentLightboxIndex = newIndex;
 } else {
     window.albumPhotosList = allFetched; 
 }
 ...
 // --- ΜΕΤΑ ---
  ...
  try {
    if (currentUserUid && !currentUserUid.startsWith("guest_")) {
      // 🛡️ ΔΙΟΡΘΩΣΗ: Προσθήκη Timeout 3s. Αν η Firebase κρεμάσει επ' άπειρον, πάει στο catch και επιτρέπει το upload!
     // --- ΜΕΤΑ ---
    ...
    canvas.toBlob(blob => {
       ...
       // 🛡️ ΣΟΒΑΡΗ ΔΙΟΡΘΩΣΗ 3: Προστασία από "Αιώνιο Πάγωμα". Αν κολλήσει το ίντερνετ πάνω από 15s, ακυρώνεται!
        ...
        .then(data => {
          ...
          // --- ΜΕΤΑ ---
            // 🛡️ Αφαίρεση της if(!isUploadSlow). Ο χρήστης ΠΡΕΠΕΙ να ενημερωθεί 
            // αν τελικά η φωτογραφία ανέβηκε, ακόμα κι αν το δίκτυο άργησε!
            Swal.fire({
                icon: 'success',
...
window.openAlbumLightbox = function(index) {
    ...
    <div style="position: relative; display: flex; flex-direction: column; align-items: center;">
       ...
       lb.addEventListener('touchend', e => {
            ...
            // Διαφορετικά, σε iPhone, ο χρήστης καταλάθος θα πατήσει "Επιστροφή" σελίδας!
