 ...
 // Το createImageBitmap σέβεται αυτόματα την περιστροφή EXIF της κάμερας!
  window.createImageBitmap(file).then(img => {
     ...
     canvas.toBlob(blob => {
        ...
        .then(async response => {
          // Διαβάζουμε την απάντηση ΠΡΩΤΑ ως απλό κείμενο.
          const text = await response.text(); 
          let data;
          try {
             data = JSON.parse(text);
          } catch(e) {
             throw new Error(`Ο Server απάντησε με λάθος μορφή (${response.status}): ${text.substring(0, 80)}...`);
          }
          
          if (!response.ok) {
             throw new Error(data.error || "Άγνωστο σφάλμα από τον Worker");
          }
          return data;
        })
        .then(data => {
          if(data.secure_url) {
            ...
            // Επιτυχία! Γράφουμε στη Firebase
        ...
        .catch(err => {
          console.error("Αναλυτικό Λάθος:", err);
          // ΕΔΩ ΕΙΝΑΙ ΤΟ ΜΥΣΤΙΚΟ: Εμφανίζει το πραγματικό λάθος στην οθόνη!
          Swal.fire({ 
            icon: 'error', 
            title: 'Βρήκαμε το Πρόβλημα:', 
            text: err.message === "Failed to fetch" ? "Ο Browser μπλόκαρε την αποστολή (CORS)." : err.message, 
