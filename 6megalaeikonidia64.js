...
const closeAllMenus = (exceptNode = null) => {
  ...
  // ΠΡΟΣΘΗΚΗ: Έξυπνο Auto-Focus στο πεδίο κειμένου
      ...
      if (isOpening) {
        if (inputField) {
            // ΔΙΟΡΘΩΣΗ: Προσθήκη preventScroll για αποφυγή ανεξέλεγκτης αναπήδησης (Scroll Jump) στο iOS
...
safeBind('btn-do-print', 'click', () => {
    ...
    // Η εκτύπωση ΠΡΕΠΕΙ να καλείται άμεσα για να μην την μπλοκάρει το iOS/Mac
    window.print(); 
    
    setTimeout(() => { if (DOM.shareMenu) DOM.shareMenu.style.display = ''; }, 1000); 
  });
  ...
  safeBind(DOM.aPrint, 'click', () => {
    ...
    setTimeout(() => { if (DOM.aMenu) DOM.aMenu.style.display = ''; }, 1000); 
  });
...
const applySliderPatch = (forceResume = false) => {
 ...
 // ΠΡΟΣΘΗΚΗ: Έλεγχος ώστε να μπλοκάρονται μόνο οι άγκυρες της ΙΔΙΑΣ σελίδας (αγνοώντας παραμέτρους όπως ?m=1 ή fbclid)
      ...
      try {
         ...
         // ΔΙΟΡΘΩΣΗ: Θεωρούμε "ίδια σελίδα" οποιοδήποτε URL έχει το ίδιο path, ΑΝΕΞΑΡΤΗΤΩΣ αν έχει hash (#).
          isSamePageAnchor = cUrl.pathname === lUrl.pathname && cUrl.origin === lUrl.origin;
      } catch (e) {
...
window.handleSearchFromLib = (e) => {
 ...
 const sPop = document.getElementById('search-pop');
  if (sPop) {
   ...
   if (inputField) {
            // ΔΙΟΡΘΩΣΗ: Προσθήκη preventScroll για αποφυγή ανεξέλεγκτης αναπήδησης στο iOS
            inputField.focus({ preventScroll: true }); 
            setTimeout(() => inputField.focus({ preventScroll: true }), 100); 
        }
