...
window.toggleDictation = () => {
    ...
    if (!recognition) {
       ...
       recognition.onend = () => {
            if (isRecording) { setTimeout(() => { try { recognition.start(); } catch(e) { stopDictation(); } }, 300); } 
            else { stopDictation(); }
        };
    }
...
window.handleYouTubeInput = () => {
  ...
  if (match && match[1]) {
        ...
        if (imgPrev.src && imgPrev.src.startsWith('blob:')) URL.revokeObjectURL(imgPrev.src);
...
const initPostDragAndDrop = () => {
 ...
 dropZone.addEventListener('drop', e => {
        if (isSubmitting) return;
        if (document.getElementById('p-youtube').value) {
            if (typeof Swal !== 'undefined') Swal.fire({ toast: true, position: 'top-end', icon: 'warning', title: 'Έχετε ήδη επιλέξει βίντεο!', showConfirmButton: false, timer: 3000 });
            return;
        }
...
window.addEventListener('paste', e => {
 ...
 if (pastedText && /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)\S+$/i.test(pastedText.trim())) {
        if (e.target && (e.target.id === 'p-content' || e.target.id === 'p-title' || e.target.id === 'p-author')) {
            return; // Επιτρέπει την κανονική επικόλληση
        }
        e.preventDefault();
  ...
  const contentArea = document.getElementById('p-content');
    if (e.target && contentArea && contentArea.contains(e.target) && pastedText) {
        e.preventDefault();
        const currentLen = contentArea.innerText.length;
    ...
    if (file) {
        ...
        if (!document.getElementById('p-youtube').value) {
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'warning', title: 'Έχετε ήδη επιλέξει βίντεο!', showConfirmButton: false, timer: 3000 });
        }
    }
});
})();
