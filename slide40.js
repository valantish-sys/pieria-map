  ...
  window.SliderManager = {
    ...
    buildDOM: (entries) => {
      ...
      entries.forEach((entry, idx) => {
        ...
        // --- ΑΛΛΑΓΗ 1: Αν έχει [MENU], πάρε τον κώδικα κατευθείαν από το Blogger ---
        if (title.includes("[MENU]")) {
            ...
            // Παίρνουμε τον ΑΥΤΟΥΣΙΟ κώδικα της ανάρτησης (HTML + CSS) και τον βάζουμε στο slide
            customSlide.innerHTML = entry.content?.$t || "";
            
            fragment.appendChild(customSlide);
            SM.slides.push({ el: customSlide, imgs: [], chunks: [] });
            return; // Προχωράμε στην επόμενη ανάρτηση χωρίς να τρέξουμε τον κλασικό κώδικα (extractData)!
        }
        // --------------------------------------------------------------------------
    ...
    setupEvents: () => {
      ...
      SM.container.addEventListener('click', (e) => {
        ...
        // --- ΑΛΛΑΓΗ 2: Σταματάμε το Slider όταν πατάς στα Tabs ---
