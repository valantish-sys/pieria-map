    ...
    // Β. ΓΥΡΟΣΚΟΠΙΚΟ PARALLAX (Κινητά/Tablets)
      ...
      if (window.DeviceOrientationEvent && container) {
        ...
        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            // ΔΙΟΡΘΩΣΗ: Ζητάμε άδεια ΜΟΝΟ αν ο χρήστης κάνει κλικ στο ίδιο το ραδιόφωνο!
...
// Η συνάρτηση που κάνει την πραγματική σύνδεση
      const connectFirebase = () => {
       ...
       // ΔΙΟΡΘΩΣΗ: Ορίζουμε τη συνάρτηση ΕΞΩ από το συμβάν για να μπορούμε να τη διαγράψουμε σωστά!
         ...
         // ΔΙΟΡΘΩΣΗ: Συνδέουμε τη λειτουργία επανασύνδεσης για να ενεργοποιείται όταν η σελίδα ξαναγίνεται ορατή!
    ...
    setupSwipeSupport: () => {
     ...
     let isVerticalScrolling = false; // ΝΕΟ
      ...
      let snapBackTimeout = null; // ΝΕΟ: Αποθηκεύουμε το χρονόμετρο επαναφοράς
      const SWIPE_THRESHOLD = 90;

  const handleDragStart = (e) => {
        if (isAnimatingSwipe) return;
        clearTimeout(snapBackTimeout); // Ακυρώνει τη βίαιη επαναφορά αν ο χρήστης ξαναπιάσει την κάρτα
    
        if (e.target.closest('.loop-btn, #toggle-more-btn, audio, .extra-tracks-wrapper, input')) return;
        
        container.dataset.isSwiping = 'true'; // Ειδοποιούμε το Γυροσκόπιο ότι κάνουμε Swipe!
   ...
   // 7Β. Η βοηθητική λογική για Swipe & Loop (Εύρεση τραγουδιού)
    ...
    const els = JukeboxManager.dom;
      ...
      // ΔΙΟΡΘΩΣΗ: Σεβόμαστε και συγχρονίζουμε τον στόχο της έντασης με τη μνήμη του browser
      if (els.player.volume >= 0 && els.player.volume <= 1) {
