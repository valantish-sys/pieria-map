...
function renderPhotos() {
 ...
 let themeFilterEl = document.getElementById("theme-filter");
  ...
  allPhotos.forEach(photo => {
    ...
    let safeCaption = photo.caption ? photo.caption.replace(/'/g, "&#39;").replace(/"/g, "&quot;") : "";
    let captionHtml = "";
    ...
    if (editMode[photo.key]) {
      ...
      let dispCaption = photo.caption ? `💬 "${photo.caption}"` : "(Χωρίς λεζάντα)";
  ...
  if (photo.status === "pending") {
      ...
      if (themeFilter === "all" || photo.theme_id === themeFilter) {
        ...
        buttonsHtml = `<button class="btn btn-outline btn-block" onclick="deletePhoto('${photo.key}', true)">🗑️ Απόσυρση (Στον Κάδο)</button></div>`;
      } else {
        isVisible = false;
      }
 ...
 const pendingBadge = document.getElementById("pending-badge");
  ...
  if (statPending) {
    ...
    if(statApp) statApp.innerText = approvedCount;
