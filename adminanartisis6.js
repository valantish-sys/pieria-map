      ...
      if (p.imageUrl) {
        imgHtml = `
          ...
          <div style="display:flex; gap:10px; margin-top:5px; margin-bottom:15px;">
            <button class="btn" style="flex:1; font-size:11px; padding:4px; background:#f39c12; color:white; border:none; border-radius:3px; cursor:pointer;" onclick="window.openBlurTool('${p.id}', '${p.imageUrl}')">🎨 Εργαλείο Θόλωσης</button>
...
window.updatePostStatus = (id, status) => {
 ...
 window.removePostImage = (id) => {
    if (confirm('Σίγουρα θέλετε να αφαιρέσετε την εικόνα; Το κείμενο της ανάρτησης θα παραμείνει.')) {
        // Χρησιμοποιούμε update({ imageUrl: null }) που είναι πιο αξιόπιστο από το .remove()
        db.ref('parent_posts/' + id).update({ imageUrl: null }).then(() => {
            if (typeof Swal !== 'undefined') {
                Swal.fire({icon: 'success', title: 'Διαγράφηκε', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000});
            } else {
                alert('Η εικόνα διαγράφηκε επιτυχώς!');
            }
        }).catch(err => {
            console.error("Σφάλμα διαγραφής:", err);
            alert("Υπήρξε πρόβλημα κατά τη διαγραφή. Ελέγξτε την κονσόλα.");
        });
    }
};
