...
{ text: "Αν μπορούσατε να δημιουργήσετε μια ολοκαίνουργια γιορτή, τι ακριβώς θα γιορτάζαμε και πώς;", emoji: "🎉🎈" },
 ...
 const dailyQ = QUESTIONS_DB[dayOfYear % QUESTIONS_DB.length];
  ...
  // ΝΕΟ: ΕΞΥΠΝΟΣ ΥΠΟΛΟΓΙΣΜΟΣ ΣΧΟΛΙΚΟΥ ΕΤΟΥΣ (Σεπτέμβριος - Ιούνιος) και στο κινητό!
  const schoolYear = today.getMonth() >= 8 ? today.getFullYear() : today.getFullYear() - 1;
  const dayKey = `schoolYear_${schoolYear}_day_${dayOfYear}`;
  ...
  if (w) {
     ...
     // Καταργήσαμε τον έλεγχο - Η φόρμα στο κινητό ανοίγει πάντα!
      enableForms();
      ...
      function showVotedUI() {
          ...
          // ΝΕΟ: Εξαφανίζουμε το ταμπελάκι αν ο χρήστης έχει ήδη ψηφίσει
          const earlyViewBtn = document.getElementById('early-view-btn-mobile');
      ...
      function enableForms() {
          ...
          // ΝΕΟ: Ενεργοποιούμε το κλικ στο ταμπελάκι
          const earlyViewBtn = document.getElementById('early-view-btn-mobile');
          if (earlyViewBtn) {
          ...
          if (btnOpen) {
              btnOpen.onclick = () => { 
                  ...
                  // ΝΕΟ: Κρύβουμε το ταμπελάκι μόλις πατήσει "Απαντήστε" (για να καθαρίσει η οθόνη)
                  if (earlyViewBtn) earlyViewBtn.style.display = 'none';
              };
          }
         ...
         if (btnSubmit) {
              btnSubmit.onclick = async () => {
                  ...
                  // 3. Συνεχίζει κανονικά το διάβασμα των πεδίων...
                  const age = document.getElementById('quiz-age-mobile').value.trim().substring(0, 3);
                  const text = document.getElementById('quiz-answer-mobile').value.trim().substring(0, 500);
                 ...
                 try {
                      // 1. Στέλνουμε τα δεδομένα στη βάση (Firebase)
                      ...
                      // 2. Καθαρίζουμε τα πεδία (έτοιμα για την επόμενη φορά)
                      document.getElementById('quiz-age-mobile').value = '';
                      document.getElementById('quiz-answer-mobile').value = '';
                      
                      // 3. Η Λίστα της Επιβράβευσης (Level Up & Παιχνιδιάρικα)
                      ...
                      // Διαλέγουμε ΕΝΑ μήνυμα στην τύχη
                      const randomMsg = rewardMessages[Math.floor(Math.random() * rewardMessages.length)];
                      
                      if (successMsg) {
                          successMsg.innerText = randomMsg; // Βάζουμε το τυχαίο κείμενο
                          successMsg.style.display = 'block';
                      }

                      // 4. Η Χορογραφία του Συμμαζέματος (Μετά από 3.5 δευτερόλεπτα)
                      // Αυξήσαμε τον χρόνο για να προλάβουν να χαρούν το μήνυμα στην οθόνη του κινητού!
                      setTimeout(() => { 
                          if (successMsg) successMsg.style.display = 'none';
                          
                          // Επαναφέρουμε το κουμπί Αποστολής για την επόμενη φορά
                          btnSubmit.innerText = "🚀 Αποστολή";
                          btnSubmit.disabled = false;
                          
                          // Κλείνουμε εντελώς τη φόρμα για να μην πιάνει χώρο στην οθόνη του κινητού
                          if (form) form.style.display = 'none';
                          
                          // Εμφανίζουμε ξανά το κουμπί "Απαντήστε" & το ταμπελάκι
