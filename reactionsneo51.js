...
const mediaNodes = Array.from(wrapper.querySelectorAll(`${bodySelectors}, .scrollable-article-area img, .scrollable-article-area iframe`));
   ...
   // Δίνουμε 50% μεγαλύτερη ανοχή (πολλαπλασιάζοντας με το 1.5) στην καμπύλη 
            ...
            if (Math.abs(diffY) > Math.abs(diffX) * 1.5) {
                // Αποτροπή κλεισίματος όταν ο χρήστης έχει κάνει ζουμ (scale > 1) και περιηγείται στην εικόνα!
         ...
         // Σταματάμε τα video iframes ΚΑΙ τις πλατφόρμες ήχου/podcasts, αφήνοντας ανέπαφα τα Social Media
                wrapper.querySelectorAll('iframe[src*="youtube"], iframe[src*="youtu"], iframe[src*="vimeo"], iframe[src*="dailymotion"], iframe[src*="spotify"], iframe[src*="soundcloud"], iframe[src*="anchor.fm"], iframe[src*="podcasters"]').forEach(ifr => {
                    if (ifr.hasAttribute('src')) {
                        ...
                        ifr.removeAttribute('src'); // Διαγραφή του src: σταματά τον ήχο χωρίς να προσθέτει "κενές" σελίδες στο ιστορικό!
                    }
                });
...
mainContainer.dataset.justVoted = "true";
         ...
         .catch((error) => {
                ...
                delete mainContainer.dataset.justVoted; // Απελευθέρωση για να δουλέψει ξανά το Lazy Load
                spanCount.innerText = "!";
            });
        }
    }
