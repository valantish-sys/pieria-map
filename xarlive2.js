...
aiginio: { title: "Αιγίνιο 🌾", desc: "Ο εύφορος κάμπος της βόρειας Πιερίας. Ένας τόπος γεμάτος ζωντανή παράδοση, ιστορικές βυζαντινές εκκλησιές και φυσική ομορφιά κοντά στους προστατευμένους υδροβιότοπους.", img: "https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEglKzTrLdlgz15emRwy5PSkfbrqcWgy0-5ZK-FWCwsctBqaZ7Ro4Q_WmWoAItjYl14RHKnnKcnARxP35QeiUK1PLCqE5QGX7yPzCr-3QOyll6AFMX8QypFpVMDvgE2oenzCW3ydLADTf2rJq6W2PnY60RfnAgRvn8aSNI04nSX62zbtgrRwg2MZKwMc2i0/w400-h223/2026-07-01%2014_10_17-2025-09-25%2018_01_46-World%20of%20Warcraft.png" },
  ...
  const AudioManager = {
   toggle: (btnEl) => {
      ...
      if (!STATE.isSoundOn) {
        ...
        AudioManager.play(); // Εκκίνηση της αναπαραγωγής ξανά
      }
    },
    play: () => {
     ...
     // Συνάρτηση για ασφαλή εκκίνηση του Fade-out
      const startTimers = () => {
        clearTimeout(STATE.audioTimeout); // Αποτροπή πολλαπλών χρονομέτρων σε απανωτά κλικ
        clearInterval(STATE.fadeInterval);
        
        STATE.audioTimeout = setTimeout(() => {
          STATE.fadeInterval = setInterval(() => {
            if (STATE.audio.volume > 0.05) {
      ...
      // Χειρισμός Promise: Το Fade-out ξεκινά ΜΟΝΟ αφού το τραγούδι φορτώσει και αρχίσει να παίζει
      const playPromise = STATE.audio.play();
      if (playPromise !== undefined) {
        playPromise.then(startTimers).catch(() => { /* Σιωπηρή αποτυχία */ });
      } else {
        startTimers(); // Εφεδρική λύση για πολύ παλιούς browsers
      }
    },
    stop: () => {
 ...
 const TooltipManager = {
    get el() { return document.getElementById('tolkien-tooltip-1'); },
    get titleEl() { return document.getElementById('tt-title-1'); },
    get descEl() { return document.getElementById('tt-desc-1'); },
    get imgEl() { return document.getElementById('tt-img-1'); },
    ...
    // Ενημέρωση περιεχομένου
      ...
      // Υποχρεωτικός μηδενισμός του scroll για να εμφανίζεται το κείμενο πάντα από την αρχή
      TooltipManager.descEl.scrollTop = 0;
  ...
  toggle: (isFirstLoad = false) => {
      ...
      if (!allBanners.length) return; // Διακοπή ΜΟΝΟ αν δεν υπάρχουν τα λάβαρα
      ...
      if (STATE.mapState === 1) {
        ...
        if (button) button.innerHTML = "✨ Εμφάνιση όλων μαζί!";
        STATE.mapState = 2;
        ...
        if (button) button.innerHTML = "🔄 Επαναφορά στην αρχική!";
        STATE.mapState = 3;
      } else {
        // Κατάσταση 3: Εμφάνιση Group A (Αρχική)
        MapManager.applyVisibility(allBanners, CONFIG.groupA);
        if (button) button.innerHTML = "🗺️ Ανακάλυψε τα υπόλοιπα!";
        STATE.mapState = 1;
      }
    },
    ...
    applyVisibility: (banners, activeGroup) => {
      ...
      // Εξάγει το ID αντέχοντας διπλά εισαγωγικά (από CMS) και μετατρέποντας τυχόν κεφαλαία σε πεζά
        const rawId = banner.dataset.place || (banner.getAttribute('onclick') || '').match(/['"]([^'"]+)['"]/)?.[1];
        const placeId = rawId ? rawId.toLowerCase() : null;
...
// Διαβάζει δυναμικά ποιο λάβαρο πατήθηκε
   ...
   preloadImages: () => {
      ...
      // Αν το DOM και το περιεχόμενο έχουν ήδη φορτώσει, εκτέλεσε άμεσα. Αλλιώς, περίμενε το 'load'
      if (document.readyState === 'complete') {
        loadImages();
      } else {
        window.addEventListener('load', loadImages, { passive: true });
      }
    }
  };
