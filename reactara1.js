...
class FeedbackEngine {
            static scan() {
                document.querySelectorAll(CONFIG.SELECTORS.ARTICLE_CONTAINER).forEach(container => {
                    ...
                    // FIREBASE FIX: Αφαίρεση απαγορευμένων χαρακτήρων (. # $ [ ])
                    ...
                    // 1Η ΑΛΛΑΓΗ: Φόρτωση δεδομένων σε Πραγματικό Χρόνο
            ...
            runTransaction(reactionRef, (currentCount) => {
                ...
                // 2. ΝΕΟ: Καταγραφή ψήφου στο Γενικό Σύνολο!
                const totalRef = ref(db, 'stats/total_reactions');
                runTransaction(totalRef, (currentTotal) => {
                    return (currentTotal || 0) + 1;
                }).catch(e => console.error("Δεν ενημερώθηκε το σύνολο", e));

            }).catch(() => {
                btnContainer.style.pointerEvents = 'auto'; // Αν αποτύχει, το ξανανοίγουμε
                span.innerText = "!";
            });
        }
// ==========================================
