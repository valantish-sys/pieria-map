document.addEventListener("DOMContentLoaded", () => {
 ...
 // Τοπική Μνήμη (Local Storage) - Ασφαλής Ανάγνωση (Safe Parse)
  const safeJSON = (key) => {
    ...
    catch (e) { return []; } // Αν κάτι πάει λάθος, δεν κρασάρει αλλά επιστρέφει κενό πίνακα
  };
  ...
  function createBookCard(book) {
    ...
    favBtn.addEventListener('click', (e) => {
     ...
     if (favorites.includes(book.book_url)) {
        ...
        if (showFavsOnly) {
          card.remove(); // Αφαιρεί την κάρτα ομαλά χωρίς να μηδενίζει το Scroll!
          filteredBooks = filteredBooks.filter(b => b.book_url !== book.book_url);
          currentIndex--; // Διορθώνει τον μετρητή του Infinite Scroll
...
function openModal(book) {
    activeBook = book;
    modalImg.onerror = function() { this.src = errorImg; }; // Πρόληψη για νεκρά URLs εικόνων
 ...
 // Προσθήκη Fallback (text || "") ώστε να αποτραπεί η κατάρρευση αν ο τίτλος απουσιάζει
