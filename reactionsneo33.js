...
(() => {
    ...
    class LightboxEngine {
        ...
        open(clickedElement) {
            ...
            const wrapper = clickedElement.closest('.fetched-content-wrapper, .post-body, .entry-content');
if (!wrapper) return;
const mediaNodes = Array.from(wrapper.querySelectorAll('img, iframe'));
    ...
    const AppController = {
        init: () => {
            // Αρχική Σάρωση
            FeedbackEngine.scan(); // 1ο: Φτιάχνει τα Emoji για να μπουν από πάνω
            ArticleEngine.scan();  // 2ο: Φτιάχνει το Read More Button για να μπει από κάτω!
        ...
        setupGlobalDelegation: () => {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (lightboxImg) {
                    e.preventDefault(); e.stopPropagation();
                    AppLightbox.open(lightboxImg); // Περνάμε ολόκληρο το στοιχείο (DOM element)
                    return;
                }
