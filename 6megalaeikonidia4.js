...
if (DOM.shareMenu?.classList.contains('show') && !DOM.shareMenu.contains(t) && !t.closest('#share-master-fab, #share-master-fab-mob')) DOM.shareMenu.classList.remove('show');
  ...
  initMobileFabs();
  // ==========================================
  // ΝΕΟ: ΜΑΓΝΗΤΙΚΟ DRAGGABLE SPEED DIAL (AssistiveTouch)
  // ==========================================
  const initDraggableDial = () => {
    ...
    // Πιάνουμε τα Touch events πάνω στο κουμπί
    mainBtn.addEventListener('touchstart', (e) => {
      // Δεν επιτρέπουμε σύρσιμο αν το μενού είναι ήδη ανοιχτό
      if (DOM.speedDialMenu?.classList.contains('is-open')) return;
      ...
      dialWrapper.style.transition = 'none'; // Ακαριαία κίνηση, χωρίς CSS delay
      
      // Αποδεσμεύουμε το κουμπί από το 'bottom' και το 'left' του CSS σου
      dialWrapper.style.setProperty('bottom', 'auto', 'important');
      dialWrapper.style.setProperty('right', 'auto', 'important');
      dialWrapper.style.setProperty('left', `${initialLeft}px`, 'important');
      dialWrapper.style.setProperty('top', `${initialTop}px`, 'important');
    }, { passive: true });
    ...
    mainBtn.addEventListener('touchmove', (e) => {
      ...
      // Πρέπει να κουνηθεί > 10px για να θεωρηθεί Drag (προστασία από απλό "ταπ")
      ...
      if (isDragging) {
        if (e.cancelable) e.preventDefault(); // Σταματάει το scroll της σελίδας όσο σέρνεις!
    ...
    }, { passive: false }); // To false είναι υποχρεωτικό για να δουλέψει το preventDefault

    mainBtn.addEventListener('touchend', () => {
      if (!isDragging) {
        dialWrapper.style.transition = ''; // Αν ήταν απλό πάτημα, επαναφέρουμε το CSS
        return;
      }

      // Ενεργοποιούμε την ομαλή κίνηση για το μαγνητικό snap
      ...
      // Μαγνητικός έλεγχος: Βρίσκεται στο Δεξί ή στο Αριστερό μισό της οθόνης;
      const isRight = centerX > screenW / 2;
      const snapX = isRight ? screenW - rect.width - 15 : 15;
      
      // Προστασία Υ-άξονα: Μην πάει κάτω από την οθόνη (κρύβει μενού) ή πολύ ψηλά
      const snapY = Math.max(80, Math.min(rect.top, screenH - rect.height - 80));

      dialWrapper.style.setProperty('left', `${snapX}px`, 'important');
      dialWrapper.style.setProperty('top', `${snapY}px`, 'important');

      // CSS κλάση για αντιστροφή των Labels & Tooltip όταν είναι δεξιά!
      dialWrapper.classList.toggle('is-snapped-right', isRight);

      setTimeout(() => { isDragging = false; }, 100);
      
      // Καθαρίζουμε το transition μετά την κίνηση (0.4s) για να μην μπλέκεται με hover
      setTimeout(() => {
         if (!isDragging) dialWrapper.style.setProperty('transition', 'transform 0.8s ease, opacity 0.8s ease', 'important');
      }, 400);
    });
