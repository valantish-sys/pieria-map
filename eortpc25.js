...
const SearchEngine = {
        index: [],
        isBuiltForYear: null,
        // ΠΡΟΣΘΗΚΗ: Αντικαθιστά και το τελικό "ς" με "σ"
       ...
       search: (query) => {
            ...
            // Σπάμε το query του χρήστη σε ανεξάρτητες λέξεις (αγνοώντας πολλαπλά κενά)
            const words = q.split(/\s+/).filter(word => word.length > 0);
            
            // Επιστρέφουμε αποτελέσματα ΜΟΝΟ αν ΟΛΕΣ οι λέξεις (.every) υπάρχουν στο index
            return SearchEngine.index.filter(item => 
                    ...
                    if (i === 0) {
                        ...
                        if (widgetBox) {
                          ...
                          const currentHour = new Date().getHours();
                            // Έξυπνο Night Mode: Το 4μηνο Νοέμβριος-Φεβρουάριος σκοτεινιάζει από τις 17:30
                            const currentMonth = new Date().getMonth() + 1;
                            const isWinterSeason = (currentMonth >= 11 || currentMonth <= 2);
                            const nightStartHour = isWinterSeason ? 18 : 20;
                            const isNight = (currentHour >= nightStartHour || currentHour < 6);
                           ...
                           if ([71,73,75,77,85,86].includes(currentCode)) {
                            ...
                            } else if ([51,53,55,56,57,61,63,65,66,67,80,81,82,95,96,99].includes(currentCode)) {
                        ...
                        if (advisorBox) {
                            ...
                            if (hourNow >= 2 && hourNow < 6) {
                               ...
                               // 3. ΛΕΙΤΟΥΡΓΙΑ "ΑΥΡΙΟ" (16:00 το απόγευμα έως 01:59 το βράδυ)
                                    ...
                                    // ΛΥΣΗ: Δυναμικό λεκτικό. Μετά τα μεσάνυχτα το "Αύριο" είναι παραπλανητικό.
                                    const textDay = (hourNow < 6) ? "Το πρωί" : "Αύριο";
                                   ...
                                   for (let h = 6; h <= 14; h++) {
                                    ...
                                    if (hasSnow) {
                                        icon = "⛄"; msg = `${textDay} περιμένουμε χιόνι! Ετοίμασε ζεστά ρούχα 🧅!`; 
                                        borderColor = "#1e6cff"; bgColor = "rgba(30, 108, 255, 0.1)";
                                    } else if (hasRain) {
                                        icon = "☔"; msg = `${textDay} δίνει βροχή! Μην ξεχάσεις την ομπρέλα σου!`; 
                                        borderColor = "#3b82f6"; bgColor = "rgba(59, 130, 246, 0.1)";
                                    } else if (checkTemp < 7) {
                                        icon = "🧣"; msg = `${textDay} θα έχει παγωνιά! Ετοίμασε σκούφο και γάντια!`; 
                                        borderColor = "#0ea5e9"; bgColor = "rgba(14, 165, 233, 0.1)";
                                    } else if (checkTemp > 21) {
                                        icon = "☀️"; msg = `${textDay} θα κάνει ζέστη! Μην ξεχάσεις το παγούρι σου!`; 
                                        borderColor = "#f59e0b"; bgColor = "rgba(245, 158, 11, 0.1)";
                                    } else {
                                        icon = "🌤️"; msg = `${textDay} φαίνεται ιδανικός καιρός για παιχνίδι!`; 
                                        borderColor = "#10b981"; bgColor = "rgba(16, 185, 129, 0.1)";
                                    }
                                }
            ...
            if (searchIcon && searchInput && searchResults) {
                ...
                searchIcon.addEventListener('click', (e) => {
                    ...
                    if (searchBox.classList.contains('active')) {
                        ...
                        searchInput.value = '';
                        searchInput.blur(); // ΛΥΣΗ: Κλείνει εξαναγκαστικά το ψηφιακό πληκτρολόγιο
              ...
              // Κλικ στο αποτέλεσμα!
                searchResults.addEventListener('click', (e) => {
                    ...
                    if (item && item.getAttribute('data-time')) {
                        ...
                        searchInput.value = '';
                        searchInput.blur(); // ΠΡΟΣΘΗΚΗ: Αφαιρεί το focus / Κλείνει το πληκτρολόγιο
                        searchResults.classList.remove('show');
                        if (groupWrap) groupWrap.classList.remove('searching');
