(() => {
   "use strict";
    ...
    const triggerFullscreen = (iframeId) => {
       ...
       const fallbackOpen = () => { if (fIframe.src) window.open(fIframe.src, '_blank'); };
       ...
       // ΔΙΟΡΘΩΣΗ Α: Ενεργοποιούμε το κλικ στα κουμπιά ΑΜΕΣΩΣ! Έτσι δουλεύουν ακόμα και αν το Adblocker μπλοκάρει το YT API.
        ...
        const setupPlayers = () => {
            ...
            platforms.forEach(p => {
                ...
                // ΔΙΟΡΘΩΣΗ Β: Προσθήκη enablejsapi=1 αν λείπει (αλλιώς το onStateChange νεκρώνει και το κουμπί δεν θα κρυφτεί ποτέ αυτόματα)
