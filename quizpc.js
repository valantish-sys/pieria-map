(function() {
    ...
    if (path === '/' || path === '/index.html') {
        var wrapper = document.getElementById('pc-quiz-wrapper');
        if (wrapper) {
...
{ text: "Αν μπορούσατε να δημιουργήσετε μια ολοκαίνουργια γιορτή, τι ακριβώς θα γιορτάζαμε και πώς;", emoji: "🎉🎈" },
    ...
    document.getElementById("q-text-pc").innerText = dailyQ.text;
    document.getElementById("q-emoji-pc").innerHTML = dailyQ.emoji;
})();

// --- 3. Άνοιγμα/Κλείσιμο Φόρμας στο PC ---
window.toggleQuizPC = function(event) {
    var trigger = document.getElementById('quiz-trigger-pc');
    var form = document.getElementById('quiz-form-pc');
    ...
    if (form && trigger) {
        ...
        setTimeout(function() {
            document.addEventListener('click', closeQuizPC);
        }, 100);
    }
};

function closeQuizPC(event) {
    var form = document.getElementById('quiz-form-pc');
    var trigger = document.getElementById('quiz-trigger-pc');
    
    if (form && trigger && !event.target.closest('#quiz-form-pc')) {
        form.style.display = "none";
        trigger.style.display = "block";
        document.removeEventListener('click', closeQuizPC);
    }
}

// --- 4. Αυτόματο Κλείσιμο μετά την Υποβολή (Με επαναφορά θέσης για να φαίνεται το μήνυμα) ---
var formLoadCountPC = 0;

window.iframeLoadedPC = function() {
    formLoadCountPC++;
    
    // Η 1η φόρτωση είναι όταν ανοίγει η φόρμα. Η 2η είναι όταν πατηθεί "Υποβολή"
    if (formLoadCountPC === 2) {
        // Βρίσκουμε το iframe και του λέμε να έρθει στο top: 0 για να φανεί το μήνυμα της Google
        var iframe = document.getElementById('google-form-iframe-pc');
        if (iframe) {
            iframe.style.top = "0px";
        }

        // Περιμένουμε 6 δευτερόλεπτα (6000ms) και μετά κλείνουμε το gadget
        setTimeout(function() {
            var form = document.getElementById('quiz-form-pc');
            var trigger = document.getElementById('quiz-trigger-pc');
            ...
            // Επαναφέρουμε το iframe στην κρυφή του θέση (-335px) για την επόμενη φορά
            if (iframe) {
            ...
            formLoadCountPC = 0; 
        }, 8000); // 8000ms = 6 δευτερόλεπτα αναμονή
    }
};
