...
LM.lastFocus?.focus(); 
  ...
  window.SliderManager = {
    ...
    try {
      ...
      // --- ΝΕΟ: FETCH ΑΡΘΡΩΝ ΓΟΝΕΩΝ ΑΠΟ FIREBASE ΓΙΑ ΤΟ SLIDER ---
        try {
            // Το Slider είναι συνδεδεμένο στη βάση των αντιδράσεων. 
            // Άρα κατεβάζουμε τα άρθρα από τη βάση "anartiseis" πανεύκολα με REST API fetch!
            const fbRes = await fetch('https://anartiseis-7cad1-default-rtdb.europe-west1.firebasedatabase.app/parent_posts.json');
            ...
            if (fbData && !fbData.error) {
                ...
                for (const key in fbData) {
                    ...
                    if (post.status === 'approved') {
                        ...
                        cleanText = cleanText.replace(/<o:p>.*?<\/o:p>/g, '').replace(/\[READ_MORE\]/g, ' ');
                        ...
                        // ΠΡΟΣΤΑΣΙΑ: Αν η ημερομηνία λείπει ή είναι σπασμένη, βάζουμε τη σημερινή για να μη κρασάρει η ταξινόμηση
                        let safeTimestamp = post.timestamp ? Number(post.timestamp) : Date.now();
                        if (isNaN(safeTimestamp)) safeTimestamp = Date.now();

                        fbPosts.push({
                            isParentPost: true,
                            firebaseId: key,
                            published: { $t: new Date(safeTimestamp).toISOString() },
                            link: [{ rel: "alternate", href: "#" }],
                            title: { $t: "👨‍👩‍👧‍👦 " + (post.title || "") },
                            content: { $t: imgHtml + cleanText }
                        });
                    }
                }
                entries = [...entries, ...fbPosts].sort((a, b) => new Date(b.published?.$t || 0).getTime() - new Date(a.published?.$t || 0).getTime());
            }
        } catch(fbErr) { console.warn("Slider Firebase fetch error:", fbErr); }
        // -----------------------------------------------------------
...
const { imgs, text: fullText, widgetHtml } = Utils.extractData(postContent);
      ...
      const slide = Object.assign(document.createElement(widgetHtml ? 'div' : 'a'), {
        ...
        if (!widgetHtml) {
          slide.href = entry.isParentPost ? "javascript:void(0)" : href;
          slide.rel = "noopener";
          if (entry.isParentPost) slide.dataset.firebaseId = entry.firebaseId;
        }
...
const regexCss = new RegExp(`#${oldId}(?=[\\s\\:\\,\\[\\{>~\\.\\+\\)]|$)`, 'g');
   ...
   let originalPostId = "";
        ...
        if (entry.isParentPost) {
            safePostId = "parent_post_" + entry.firebaseId;
            originalPostId = safePostId;
        } else if (href && href !== "#") {
            try { 
                originalPostId = new URL(href, window.location.origin).pathname; 
                safePostId = originalPostId.replace(/[\.\#\$\[\]\/]/g, '_');
            } catch(e) {}
        }

        if (safePostId?.length > 3) {
            let isVoted = false;
          ...
          reactDiv.dataset.postid = originalPostId;
 ...
 resume: (force = false) => {
   ...
   // --- ΝΕΟ: Scroll στη ροή όταν πατάμε άρθρο γονέων στο slider ---
        const parentSlide = e.target.closest('.slide[data-firebase-id]');
        if (parentSlide && !e.target.closest('.floating-btn, [data-action="zoom"], .slider-arrow, .snippet-btn, .slide-title, .widget-overlay')) {
            e.preventDefault();
            const targetEl = document.getElementById('fb-post-' + parentSlide.dataset.firebaseId);
            if (targetEl) {
                targetEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
                // Εφέ highlight για να δει ο χρήστης πού πήγε
                targetEl.style.transition = "background-color 0.5s ease";
                targetEl.style.backgroundColor = "rgba(30,108,255,0.15)";
                setTimeout(() => targetEl.style.backgroundColor = "transparent", 1500);
                
                // Αν είναι κλειστό, το ανοίγουμε μετά το scroll
                const readBtn = targetEl.querySelector('.local-read-more');
                const postEl = targetEl.querySelector('.post-outer');
                if (readBtn && postEl && !postEl.classList.contains('is-expanded')) {
