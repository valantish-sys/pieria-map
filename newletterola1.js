    ...
    function initWidget(prefix) {
        ...
        init: () => {
          ...
          if (path !== '/' && path !== '/index.html') return;
            ...
            // ΑΦΑΙΡΕΘΗΚΕ ΤΟ display = 'block'. Αφήνουμε το CSS να κρίνει ποιο widget πρέπει να φαίνεται.
      ...
      stopDraw: () => { 
       ...
       checkReveal: () => {
        ...
        fetchPost: () => {
           ...
           // Αλλαγή από 'summary' σε 'default' για να έρθει η πλήρης HTML μορφή
            fetch('/feeds/posts/default?q=δράσεις&alt=json&max-results=500')
                .then(r => r.json())
                .then(data => {
                    ...
                    if (actionPosts.length > 0) {
                      ...
                      } else if (randomPost.content?.$t) { // Ψάχνουμε πλέον στο content που διατηρεί τα tags
