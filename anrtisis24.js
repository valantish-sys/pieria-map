...
window.escapeHTML = (str) => {
 ...
 // Προστέθηκε το "p" για να επιτρέπονται και οι κανονικές παράγραφοι
...
window.addEventListener('beforeunload', (e) => {
    // Μπλοκάρουμε την έξοδο ΜΟΝΟ αν γίνεται υποβολή της ανάρτησης εκείνη τη στιγμή
    if (isSubmitting) {
        e.preventDefault();
        e.returnValue = ''; 
    }
...
const initPostDragAndDrop = () => {
    ...
    const contentArea = document.getElementById('p-content'); // προσθήκη
    if (!dropZone || !fileInput) return;

    // ΔΙΟΡΘΩΣΗ: Αποτροπή ρίψης (Drop) αρχείου μέσα στο πεδίο κειμένου, που καταστρέφει το περιεχόμενο.
    if (contentArea) {
...
const initCharCounters = () => {
    ...
    for (const [id, max] of Object.entries(limits)) {
     ...
     el.dataset.counterInit = 'true';
        ...
        // ΔΙΟΡΘΩΣΗ: Φυσικό μπλοκάρισμα της πληκτρολόγησης όταν ο χρήστης ξεπεράσει το όριο 
        if (id === 'p-content') {
          el.addEventListener('keydown', (e) => {
                const allowedKeys = ['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'];
                if (el.innerText.length >= max && !allowedKeys.includes(e.key) && !e.ctrlKey && !e.metaKey) {
                    // Αν ο χρήστης ΔΕΝ έχει μαρκάρει κείμενο προς αντικατάσταση, τότε τον μπλοκάρουμε
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
        initPostDragAndDrop();
        loadDraft(); // Πρώτα φορτώνουμε το προσχέδιο...
        initCharCounters(); // ...και μετά μπαίνουν οι μετρητές
    });
} else {
    initPostDragAndDrop(); 
    loadDraft(); // Πρώτα φορτώνουμε το προσχέδιο...
...
window.addEventListener('paste', e => {
    ...
    // 1. Αν έγινε επικόλληση συνδέσμου YouTube
 ...
 // 2. ΔΙΟΡΘΩΣΗ: Επικόλληση καθαρού κειμένου (χωρίς χρώματα/στυλ) μέσα στο πεδίο κειμένου
    if (e.target && e.target.id === 'p-content' && pastedText) {
        ...
        if (charsAllowed > 0) {
            ...
            // Ενημέρωση χρήστη αν το κείμενο περικόπηκε με SweetAlert
        ...
        return; // Σταματάμε εδώ για να μην προχωρήσει στον έλεγχο εικόνας
    }

    // 3. ΔΙΟΡΘΩΣΗ: Επικόλληση Εικόνας (π.χ. Print Screen / Snipping Tool)
    ...
    if (file && !document.getElementById('p-youtube').value) {
        e.preventDefault(); // ΚΡΙΣΙΜΟ: Αποτρέπει τον browser από το να κολλήσει την εικόνα ως κώδικα Base64 μέσα στο κείμενο
        const dataTransfer = new DataTransfer();
        dataTransfer.items.add(file);
        const fileInput = document.getElementById('p-image');
        fileInput.files = dataTransfer.files;
