...
// --- 1. ΟΜΑΛΗ ΜΕΤΑΒΑΣΗ (FADE IN/OUT) - BULLETPROOF ---
   ...
   // --- 4. ΖΩΝΤΑΝΟΣ ΠΑΛΜΟΣ (ΑΛΗΘΙΝΟ FIREBASE PRESENCE) ---
    ...
    setupPiP: () => {
       // Ελέγχουμε αν υπάρχει PiP ΚΑΙ αν ο browser υποστηρίζει captureStream (αλλιώς το κρύβουμε)
        ...
        // Συγχρονισμός Σελίδας -> PiP
        JukeboxManager.dom.player.addEventListener('play', () => { if (video.paused) video.play().catch(()=>{}); });
        JukeboxManager.dom.player.addEventListener('pause', () => { if (!video.paused) video.pause(); });
      ...
      // 2. Σκοτώνουμε πάντα την παλιά λούπα πριν φτιάξουμε νέα!
      ...
      try { 
          ...
          // 3. Αν σκάσει/μπλοκαριστεί από τον browser, να μην τρώει μνήμη!
          clearInterval(drawInterval);
    ...
    playTrack: (button, url, name) => {
     ...
     if (button && button.classList.contains('track-active')) {
          if (els.player.paused) {
             // Αν το τραγούδι είχε τελειώσει (ended), γύρισέ το στην αρχή!
             if (els.player.ended) {
      ...
      const executeChange = () => {
       ...
       if (typeof JukeboxManager.setupMediaSession === 'function') JukeboxManager.setupMediaSession(name);
        ...
        // ΚΡΙΣΙΜΟ: Σκοτώνουμε το παλιό fade που τρέχει στο παρασκήνιο πριν μηδενίσουμε την ένταση,
        // αλλιώς θα ακουστεί ηχητική έκρηξη.
        clearInterval(STATE.fadeInterval);
        ...
        els.player.play().then(() => {
          ...
          // Αν ο χρήστης ακύρωσε την αναπαραγωγή αλλάζοντας γρήγορα τραγούδι (AbortError),
          // ΑΓΝΟΗΣΕ ΤΟ! Αν δεν το αγνοήσεις, θα σβήσεις το Visualizer του ΝΕΟΥ τραγουδιού!
     ...
     // Άμεση εκτέλεση χωρίς καθυστέρηση. Τα κινητά μπλοκάρουν το επόμενο .play() 
      // αν συμβεί ασύγχρονα μετά από Timeout/Promise. 
      executeChange();
    },
   ...
   handleTrackEnd: () => {
      ...
      // Η επιλογή "Ένα τραγούδι" (2) αφαιρέθηκε από εδώ γιατί την 
      // διαχειρίζεται αψεγάδιαστα ο browser μέσω του els.player.loop
